...
function je(t, e, n) {
  We();
  const s = document.querySelector("sa11y-control-panel").shadowRoot, o = s.getElementById("panel-alert"), a = s.getElementById("panel-alert-text"), i = s.getElementById("panel-alert-preview"), l = s.getElementById("close-alert"), d = s.getElementById("skip-button");
  o.classList.add("active"), a.innerHTML = t;
  ...
  function r() {
    We(), (d.hasAttribute("disabled") ? s.getElementById("toggle") : d).focus();
  }
  l.addEventListener("click", r), o.onkeydown = (c) => {
    (c || window.event).key === "Escape" && o.classList.contains("active") && r();
  };
}
const u = /* @__PURE__ */ (function() {
  ...
  function n(c) {
    ...
    const h = c.checks.QA_DOCUMENT.sources, A = 'a[href$=".doc"], a[href$=".docx"], a[href*=".doc?"], a[href*=".docx?"], a[href$=".ppt"], a[href$=".pptx"], a[href*=".ppt?"], a[href*=".pptx?"], a[href^="https://drive.google.com/file"], a[href^="https://docs.google."], a[href^="https://sway."]';
    h ? e.documentSources = `${A}, ${h}` : e.documentSources = A;
    const b = c.checks.EMBED_VIDEO.sources, y = 'video, [src*="Video"], [src*="video"], [src*="watch"], [src*="youtube.com"], [src*="vimeo.com"], [src*="panopto.com"], [src*="wistia.com"], [src*="dailymotion.com"], [src*="brightcove.com"], [src*="vidyard.com"]';
    if (b) {
    ...
    if (E) {
      ...
      e.AudioSources = v;
    const L = c.checks.EMBED_DATA_VIZ.sources, g = '[src*="datastudio"], [src*="tableau"], [src*="lookerstudio"], [src*="powerbi"], [src*="qlik"]';
    if (L) {
  ...
  function o(c, m, h) {
    ...
    try {
      ...
      A.length > 0 ? A.forEach((b) => {
        u.Root.areaToCheck.push(b);
      }) : console.error(`Sa11y: The target root (${c}) does not exist.`);
    } catch {
      s.areaToCheck.length = 0;
    }
    s.areaToCheck.length === 0 && e.headless === !1 && (je(f.sprintf("MISSING_ROOT", c)), s.areaToCheck.push(document.body));
    try {
      const A = document.querySelectorAll(m);
      A.length > 0 ? A.forEach((b) => {
        u.Root.Readability.push(b);
      }) : console.error(
    ...
    s.Readability.length === 0 && e.headless === !1 && (s.areaToCheck.length === 0 ? s.Readability.push(document.body) : (s.Readability = s.areaToCheck, setTimeout(() => {
      const { readabilityDetails: A, readabilityToggle: b } = u.Panel, y = b?.getAttribute("aria-pressed") === "true", E = u.Panel.readability.querySelector("#readability-alert");
      if (A && y && !E) {
  ...
  function r(c) {
    ...
    if (p.Container = ["#wpadminbar", "#wpadminbar *", ...m], c.containerIgnore) {
      const h = c.containerIgnore.split(",").map((A) => A.trim());
    ...
    p.Contrast = [
      ...
      "svg desc",
      ...m
    ], c.contrastIgnore && (p.Contrast = c.contrastIgnore.split(",").map((h) => h.trim()).flatMap((h) => [h, `${h} *`]).concat(p.Contrast)), p.Readability = ["nav li", '[role="navigation"] li', ...m], c.readabilityIgnore && (p.Readability = c.readabilityIgnore.split(",").map((h) => h.trim()).flatMap((h) => [h, `${h} *`]).concat(p.Readability)), p.Headings = c.headerIgnore ? c.headerIgnore.split(",").map((h) => h.trim()) : [], p.HeaderSpan = c.headerIgnoreSpan ? c.headerIgnoreSpan.split(",").map((h) => h.trim()) : [], p.Outline = c.outlineIgnore ? c.outlineIgnore.split(",").map((h) => h.trim()) : [], p.Images = [
      'img[role="presentation"]:not(a img[role="presentation"]), img[aria-hidden="true"]:not(a img[aria-hidden="true"])'
    ], c.imageIgnore && (p.Images = c.imageIgnore.split(",").map((h) => h.trim()).concat(p.Images)), p.Links = [".anchorjs-link"], c.linkIgnore && (p.Links = c.linkIgnore.split(",").map((h) => h.trim()).concat(p.Links)), p.LinkSpan = c.linkIgnoreSpan ? c.linkIgnoreSpan.split(",").map((h) => h.trim()) : [];
  }
  return {
    initializeRoot: o,
    Root: s,
...
function Z(t, e, n) {
  ...
  const o = u.Exclusions.Container.join(", "), a = n?.join(", ") || "", i = a ? `, ${a}` : "";
  let l = [];
  return s.flat().filter(Boolean)?.forEach((d) => {
    const p = d?.querySelectorAll("[data-sa11y-has-shadow-root]"), r = p ? ", [data-sa11y-has-shadow-root]" : "", c = Array.from(
      d.querySelectorAll(`:is(${t}${r}):not(${o}${i})`)
    );
    if (p.length) {
      const m = [];
      if (c.forEach((h, A) => {
...
const D = /* @__PURE__ */ (function() {
  const e = {};
  function n(a) {
    e.Everything = Z("*", "root", u.Exclusions.Sa11yElements), e.Contrast = e.Everything.filter((r) => !u.Exclusions.Contrast.some(
      (m) => r.matches(m)
    ) && !u.Exclusions.Contrast.includes(r)), e.Images = e.Everything.filter(
      (r) => r.tagName === "IMG" && !u.Exclusions.Images.some((c) => r.matches(c))
      ...
      !u.Exclusions.Links.some((c) => r.matches(c))
    ), e.Headings = Z(
      ...
      a.ignoreContentOutsideRoots || a.fixedRoots ? "root" : "document",
      u.Exclusions.Headings
    ), e.HeadingOverrideStart = /* @__PURE__ */ new WeakMap(), e.HeadingOverrideEnd = /* @__PURE__ */ new WeakMap(), a.initialHeadingLevel && a.initialHeadingLevel.forEach((r) => {
      const c = Z(
      ...
      (r) => u.Exclusions.Headings.some((c) => r.matches(c))
    ), e.ExcludedOutlineHeadings = e.Headings.filter(
      (r) => u.Exclusions.Outline.some((c) => r.matches(c))
    ...
    e.CustomErrorLinks = i.length ? e.Links.filter(
      (r) => i.split(",").some((c) => r.matches(c.trim()))
    ) : [];
    const l = (r) => u.Root.Readability.some((c) => c.contains(r)) && !u.Exclusions.Readability.some((c) => r.matches(c));
    e.Readability = [
      ...e.Paragraphs.filter(l),
      ...e.Lists.filter(l)
    ];
    const d = a.checks.QA_NESTED_COMPONENTS.sources || '[role="tablist"], details';
    e.NestedComponents = e.Everything.filter((r) => r.matches(d)), e.TabIndex = e.Everything.filter(
    ...
    ), e.Videos = e.iframes.filter((r) => r.matches(u.Global.VideoSources)), e.Audio = e.iframes.filter((r) => r.matches(u.Global.AudioSources)), e.Visualizations = e.iframes.filter(
      (r) => r.matches(u.Global.VisualizationSources)
    ), e.EmbeddedContent = e.iframes.filter(
      (r) => !r.matches(u.Global.AllEmbeddedContent)
    );
  ...
  const s = {};
  function o() {
    s.Array = Z("sa11y-annotation", "document"), s.Array.forEach((a, i) => {
...
function Ne(t, e = !1) {
  ...
  return n.innerHTML = t, ["script", "style", "noscript", "iframe", "form"].forEach((o) => {
    const a = n.getElementsByTagName(o);
    for (; a.length > 0; )
      a[0].parentNode.removeChild(a[0]);
  }), Array.from(n.getElementsByTagName("*")).forEach((o) => {
...
function qe(t, e) {
  ...
  const s = t.cloneNode(!0), o = s.querySelectorAll(n);
  let a = o.length;
  for (; a--; )
    o[a].remove();
  return s;
...
function Ys(t) {
  ...
  t.addEventListener("keydown", (o) => {
    const a = o.key === "Tab" || o.keyCode === 9, i = t.getRootNode();
    a && (o.shiftKey ? i.activeElement === n && (s.focus(), o.preventDefault()) : i.activeElement === s && (n.focus(), o.preventDefault()));
  });
}
function ts(t, e) {
  t.forEach((n) => {
...
function ss(t) {
  ...
  if (s)
    return n(s);
  const o = t.closest("picture")?.querySelector("source[srcset]")?.getAttribute("srcset"), a = e(o);
  return n(a || t.getAttribute("src"));
}
const js = (t) => new Promise((e, n) => {
  const s = new FileReader();
  s.onloadend = () => {
    let { result: o } = s;
    const a = t?.type?.startsWith("image/") ? t.type : "image/png";
    o.startsWith("data:application/octet-stream") && (o = o.replace("data:application/octet-stream", `data:${a}`)), e(o);
  }, s.onerror = n, s.readAsDataURL(t);
});
function as(t, e = !1) {
  const n = t.element, s = Ne(t.htmlPath), o = Se(s, 600), a = `<pre><code>${ct(o)}</code></pre>`, i = (r) => {
    const c = Q(r), m = Se(c, 100);
    return c.length ? Y(m) : a;
  }, d = {
    SPAN: i,
    P: i,
    A: (r) => {
      const c = Q(r), m = Se(c, 100);
      return c.length > 1 && r.href && !r.hasAttribute("role") ? `<a href="${$e(r.href)}">${Y(m)}</a>` : a;
    },
    IMG: (r) => {
      const c = r.closest("a[href]"), m = r.alt ? `alt="${Y(r.alt)}"` : "alt", h = ss(r);
      function A(y) {
        return c ? `<a href="${$e(c.href)}" rel="noopener noreferrer"><img src="${y}" ${m}/></a>` : `<img src="${y}" ${m}/>`;
      }
      if (e)
        return new Promise((y) => {
          if (h)
            if (new URL(h, window.location.origin).origin === window.location.origin)
              fetch(h).then((v) => v.blob()).then((v) => js(v)).then((v) => {
              ...
              const v = h.startsWith("data:image/") ? h : $e(h);
              y(A(v));
            }
          else
            y(a);
        });
      const b = h.startsWith("data:image/") ? h : $e(h);
      return h ? A(b) : a;
    },
    IFRAME: (r) => {
      const c = r.src, m = r.title ? r.title : "", A = r.getAttribute("aria-label") || "";
      if (c) {
        const b = A || m;
        return `<iframe src="${$e(c)}" aria-label="${Y(b)}"></iframe>`;
      }
      return a;
    },
    AUDIO: () => Ne(t.htmlPath),
...
function is(t, e, n = [], s) {
  let o = "";
  const a = n?.length ? n.join(",") : "", i = Array.isArray(s) ? s : null, l = (r = "") => i ? i.reduce((c, m) => c.replace(m, ""), r) : r;
  t.childNodes.forEach((r) => {
    r.nodeType === Node.TEXT_NODE && (o += l(r.textContent)), r.nodeType === Node.ELEMENT_NODE && (a && r.matches(a) || pn(r) || (o += l(Q(r))));
  });
  const d = /[\p{Emoji_Presentation}\p{Extended_Pictographic}]/gu;
  let p = o.replace(d, "");
  return p = Ae(p).toLowerCase(), p === "x" ? !1 : p.length !== 0 && !e.toLowerCase().includes(p);
}
function qs(t) {
...
function rs(t, e) {
  let n = 0;
  const s = (o) => {
    if (!["ArrowUp", "ArrowDown", "Space"].includes(o.code))
      return;
    if (o.code === "Space") {
      e[n].click(), o.preventDefault();
      return;
    }
    e[n].setAttribute("tabindex", -1);
    let i;
    o.code === "ArrowDown" ? (i = n + 1, n === e.length - 1 && (i = 0)) : o.code === "ArrowUp" && (i = n - 1, n === 0 && (i = e.length - 1)), e[i].setAttribute("tabindex", 0), e[i].focus(), n = i, o.preventDefault();
  };
...
class Nn extends HTMLElement {
  ...
  connectedCallback() {
    ...
    n.innerHTML = Xs + mt, e.appendChild(n);
    const s = document.createElement("div");
    s.setAttribute("id", "dialog"), s.setAttribute("tabindex", "-1");
    const o = window.location, a = "https://forms.gle/sjzK9XykETaoqZv99", i = `## Error Description
\`\`\`javascript
${this.error.stack}
\`\`\`
...
## Comments
`, d = `https://github.com/ryersondmp/sa11y/issues/new?title=Bug%20report&body=${encodeURIComponent(i)}`;
    s.innerHTML = `
      <button class="close-btn" aria-label="${f._("ALERT_CLOSE")}"></button>
      <h2>${f._("ERROR")}</h2>
      <p>${f.sprintf("CONSOLE_ERROR", a, d)}</p>
      <p class="error">${ct(this.error.stack)}<br><br>Version: 4.4.0 <br> URL: ${o}</p>
...
function Js(t, e, n) {
  if (t === !0) {
    ...
    const o = Vs(async () => {
      s !== window.location.href && (R.getItem("sa11y-panel") === "Closed" || !R.getItem("sa11y-panel") ? e() : (n(!1), await e()), s = window.location.href);
    }, 250);
    window.addEventListener("click", o), window.addEventListener("keydown", o);
  }
}
function Zs(t, e) {
  const n = JSON.parse(localStorage.getItem("sa11y-dismissed") || "[]"), s = window.location.pathname, o = (m, h) => h.key.includes(m.dismiss) && h.href === s && (m.type === "warning" || m.type === "good"), a = (m, h) => typeof h.dismissAll == "string" && m.dismissAll === h.dismissAll && h.href === s, i = t.filter(
    (m) => n.some((h) => o(m, h))
  ), l = t.filter(
    (m) => n.some((h) => a(m, h))
  ), d = [...i, ...l], p = [
    ...new Map(d.map((m) => [m.dismiss, m])).values()
  ], r = p.length, c = t.filter(
    (m) => !p.some(
      (h) => h.dismiss === m.dismiss && (m.type === "warning" || m.type === "good")
    )
  );
  return r ? (u.Panel.dismissButton.classList.add("active"), u.Panel.dismissTooltip.innerText = f.sprintf("PANEL_DISMISS_BUTTON", r), e.object.setContent(f.sprintf("PANEL_DISMISS_BUTTON", r))) : u.Panel.dismissButton.classList.remove("active"), { dismissedIssues: n, updatedResults: c, dismissCount: r, dismissedResults: p };
}
let tn, Pt;
const ea = async (t, e, n, s) => {
  let o = JSON.parse(R.getItem("sa11y-dismissed"));
  ...
  if (i.hidden = !1, a.tagName === "BUTTON" && a.hasAttribute("data-sa11y-dismiss")) {
    const l = parseInt(a.getAttribute("data-sa11y-dismiss"), 10), d = e.find((p) => p.id === l);
    if (o === null && (setTimeout(() => je(f._("DISMISS_REMINDER")), 0), o = []), d.dismiss) {
      const p = a.hasAttribute("data-sa11y-dismiss-all") ? d.dismissAll : "", r = {
        key: d.dismiss,
        href: window.location.pathname,
        ...p ? { dismissAll: p } : {}
      }, c = Z(`[data-sa11y-annotation='${d.id}']`, "root"), m = c[0] ? c[0].getAttribute("data-sa11y-position") : 0;
      R.setItem("sa11y-latest-dismissed", m), R.setItem("sa11y-dismiss-item", JSON.stringify(r)), o.push(r), R.setItem("sa11y-dismissed", JSON.stringify(o)), R.removeItem("sa11y-dismiss-item");
  ...
  s.hidden = !0;
  const o = t.filter((a) => a.href !== window.location.pathname);
  R.setItem("sa11y-dismissed", JSON.stringify(o)), u.Panel.dismissButton.classList.remove("active"), n(!1), await e();
};
function na(t, e, n, s) {
  u.Global.dismissAnnotations && (Pt = (a) => ea(a, t, n, s), document.querySelector("sa11y-tooltips").shadowRoot.addEventListener("click", Pt), u.Panel.panel.addEventListener("click", Pt)), tn = () => ta(e, n, s), u.Panel.dismissButton?.addEventListener("click", tn);
}
function sa() {
  u.Panel.panel?.removeEventListener("click", Pt), u.Panel.dismissButton?.removeEventListener("click", tn);
}
function aa() {
...
function un() {
  const t = /* @__PURE__ */ new Date(), e = String(t.getDate()).padStart(2, "0"), n = String(t.getMonth() + 1).padStart(2, "0"), s = t.getFullYear(), o = (/* @__PURE__ */ new Date()).toLocaleString(), a = `${n}-${e}-${s}`, i = document.querySelector("head title"), l = !i || i.textContent.trim().length === 0, d = l ? "" : i.textContent, p = window.location.href;
  return { date: o, numericDate: a, titleCheck: l, metaTitle: d, pageURL: p };
}
async function oa(t, e) {
  const n = t.filter((y) => y.type === "error"), s = t.filter((y) => y.type === "warning"), o = { error: n.length, warning: s.length, dismiss: e.length };
  async function a(y, E) {
    ...
    if (!(y.length > 0))
      return "";
    let _ = `<h2>${L}</h2>`, I = `<ol class="${E}">`, S = "</ol>";
    E === "dismissed" && (I = `<details><summary>${f.sprintf("PANEL_DISMISS_BUTTON", o.dismiss)}</summary><ol>`, S = "</details>"), _ += I;
    const x = y.map(async (k) => {
      let M = "";
      if (k.element) {
        const $ = ["IMG", "IFRAME", "AUDIO", "VIDEO"], w = await as(k, !0);
        $.includes(k.element.tagName) ? M = `<li><strong>${f._("PREVIEW")}:</strong> ${w}</li><li><strong>${f._("ELEMENT")}:</strong> <pre><code>${ct(k.htmlPath)}</code></pre></li>` : M = `<li><strong>${f._("ELEMENT")}:</strong> <pre><code>${ct(k.htmlPath)}</code></pre></li>`;
      }
      const U = k.cssPath ? `<li><strong>${f._("PATH")}:</strong> <pre><code>${k.cssPath}</code></pre></li>` : "";
      return `<li>${k.content} <ul>${M}${U}</ul></li>`;
    }), C = await Promise.all(x);
    return _ += C.join(""), _ += S, _;
  }
  const i = await a(n, "error"), l = await a(s, "warning"), d = await a(e, "dismissed"), p = un(), r = p.titleCheck ? "" : `<dt>${f._("PAGE_TITLE")}</dt><dd>${p.metaTitle}</dd>`, c = o.error !== 0 ? `<dt>${f._("ERRORS")}</dt><dd>${o.error}</dd>` : "", m = o.warning !== 0 ? `<dt>${f._("WARNINGS")}</dt><dd>${o.warning}</dd>` : "", h = o.dismiss !== 0 ? `<dt>${f._("DISMISSED")}</dt><dd>${o.dismiss}</dd>` : "";
  return `
      <!DOCTYPE html>
...
async function la(t, e) {
  const n = await oa(t, e), s = un(), o = new Blob([n], { type: "text/html" }), a = document.createElement("a"), i = s.titleCheck ? "" : `_${s.metaTitle.trim().replace(/ /g, "")}`;
  a.href = window.URL.createObjectURL(o), a.download = `Sa11y_${s.numericDate + i}.html`, document.body.appendChild(a), a.click(), setTimeout(() => {
    document.body.removeChild(a), window.URL.revokeObjectURL(a.href);
  }, 100);
}
function ca(t) {
  const e = un(), n = t.filter((r) => r.type === "warning" || r.type === "error").map((r) => {
    const { type: c, content: m, htmlPath: h, cssPath: A } = r, b = m.replaceAll(/<span\s+class="visually-hidden"[^>]*>.*?<\/span>/gi, "").replaceAll('<hr aria-hidden="true">', " | ").replaceAll(/"/g, '""'), y = zs(String(b)), E = Ws(y), v = {
      Title: `"${e.metaTitle}"`,
      URL: `"${e.pageURL}"`,
      Type: `"${String(c)}"`,
      Issue: `"${E}"`,
    ...
    return A && (v.Path = `"${A}"`), v;
  }), s = Object.keys(n[0]), o = `${s.join(",")}
${n.map((r) => s.map((c) => r[c]).join(",")).join(`
`)}`, a = new Uint8Array([239, 187, 191]), i = new Blob([a, o], { type: "text/csv;charset=utf-8;" }), l = window.URL.createObjectURL(i), d = document.createElement("a");
  d.href = l, d.href = window.URL.createObjectURL(i);
...
class Sn extends HTMLElement {
  connectedCallback() {
    ...
    M.setAttribute("id", "container"), M.setAttribute("role", "region"), M.setAttribute("data-sa11y-version", "4.4.0"), M.setAttribute("lang", f._("LANG_CODE")), M.setAttribute("aria-label", f._("CONTAINER_LABEL")), M.setAttribute("dir", u.Global.langDirection), i === "top-left" || i === "top-right" ? M.innerHTML = `
        ${A}
        ...
        <div id="panel" class="${i}" part="panel">
          ...
          ${S}
          ${k}
        </div>`, this.shadowRoot.appendChild(M), u.initializePanelSelectors();
  }
}
function ha(t, e) {
  u.Global.showMovePanelToggle && (u.Panel.movePanelToggle.onclick = async () => {
    const l = R.getItem("sa11y-position"), [d, p] = l.includes("top") ? ["top-right", "top-left"] : ["right", "left"], r = l === d ? p : d;
    R.setItem("sa11y-position", r), [d, p].forEach((c) => {
      u.Panel.toggle.classList.replace(c, r), u.Panel.panel.classList.replace(c, r);
    }), u.Panel.movePanelToggle.setAttribute(
      "aria-pressed",
      l === d ? "true" : "false"
    );
  }), u.Global.developerPlugin ? u.Panel.developerToggle.onclick = async () => {
    R.getItem("sa11y-developer") === "On" ? (R.setItem("sa11y-developer", "Off"), u.Panel.developerToggle.textContent = `${f._("OFF")}`, u.Panel.developerToggle.setAttribute("aria-pressed", "false"), e(!1), await t()) : (R.setItem("sa11y-developer", "On"), u.Panel.developerToggle.textContent = `${f._("ON")}`, u.Panel.developerToggle.setAttribute("aria-pressed", "true"), e(!1), await t());
  } : R.setItem("sa11y-developer", "Off"), u.Readability.Plugin && (u.Panel.readabilityToggle.onclick = async () => {
    R.getItem("sa11y-readability") === "On" ? (R.setItem("sa11y-readability", "Off"), u.Panel.readabilityToggle.textContent = `${f._("OFF")}`, u.Panel.readabilityToggle.setAttribute("aria-pressed", "false"), u.Panel.readability.classList.remove("active"), e(!1), await t()) : (R.setItem("sa11y-readability", "On"), u.Panel.readabilityToggle.textContent = `${f._("ON")}`, u.Panel.readabilityToggle.setAttribute("aria-pressed", "true"), u.Panel.readability.classList.add("active"), e(!1), await t());
  }, R.getItem("sa11y-readability") === "On" && u.Panel.readability.classList.add("active"));
  const n = window.matchMedia("(prefers-color-scheme: dark)"), { themeToggle: s } = u.Panel, { html: o } = u.Global, a = (l) => {
  ...
  }, i = R.getItem("sa11y-theme") || (n.matches ? "dark" : "light");
  a(i), n.addEventListener("change", () => {
    a(n.matches ? "dark" : "light");
  }), s.onclick = () => {
    const d = (R.getItem("sa11y-theme") || (n.matches ? "dark" : "light")) === "dark" ? "light" : "dark";
    a(d);
    ...
    l >= 1 && l <= 4 ? window.matchMedia("(forced-colors: active)").matches ? je(f._("COLOUR_FILTER_HIGH_CONTRAST")) : (document.body.setAttribute("data-sa11y-filter", d[l - 1]), u.Panel.colourFilterIcon.setAttribute("aria-label", p[l - 1]), ts(
...
const kn = () => {
  u.Panel.outlineToggle.classList.add("active"), u.Panel.outline.classList.add("active"), u.Panel.outlineToggle.setAttribute("aria-expanded", "true"), R.setItem("sa11y-outline", "Opened"), gt(u.Panel.outlineList, u.Panel.outlineContent), Z("sa11y-heading-label", "document").forEach((n) => {
    n.hidden = !1;
  });
  const e = new CustomEvent("sa11y-build-heading-outline");
  document.dispatchEvent(e);
}, Qt = () => {
  u.Panel.outline.classList.remove("active"), u.Panel.outlineToggle.classList.remove("active"), u.Panel.outlineToggle.setAttribute("aria-expanded", "false"), R.setItem("sa11y-outline", "Closed"), Z("sa11y-heading-label", "document").forEach((e) => {
    e.hidden = !0;
  });
}, wn = () => {
  u.Panel.imagesToggle.classList.add("active"), u.Panel.images.classList.add("active"), u.Panel.imagesToggle.setAttribute("aria-expanded", "true"), R.setItem("sa11y-images", "Opened"), gt(u.Panel.imagesList, u.Panel.imagesContent);
  const t = new CustomEvent("sa11y-build-image-outline");
  document.dispatchEvent(t);
}, Yt = () => {
  u.Global.showImageOutline && (u.Panel.imagesToggle.classList.remove("active"), u.Panel.images.classList.remove("active"), u.Panel.imagesToggle.setAttribute("aria-expanded", "false"), R.setItem("sa11y-images", "Closed"));
}, Cn = () => {
  u.Panel.settingsToggle.classList.add("active"), u.Panel.settings.classList.add("active"), u.Panel.settingsToggle.setAttribute("aria-expanded", "true"), R.setItem("sa11y-settings", "Opened"), gt(
    u.Panel.settingsContent,
    u.Panel.settingsContent,
    f._("SETTINGS")
  );
}, jt = () => {
  u.Panel.settings.classList.remove("active"), u.Panel.settingsToggle.classList.remove("active"), u.Panel.settingsToggle.setAttribute("aria-expanded", "false"), R.setItem("sa11y-settings", "Closed");
};
function ga() {
  ...
  }), R.getItem("sa11y-outline") === "Opened" && kn(), u.Global.showImageOutline && (u.Panel.imagesToggle.addEventListener("click", () => {
    u.Panel.imagesToggle.getAttribute("aria-expanded") === "true" ? Yt() : (wn(), Qt(), jt()), u.Panel.imagesHeader.focus();
  }), R.getItem("sa11y-images") === "Opened" && wn()), u.Panel.settingsToggle.addEventListener("click", () => {
    u.Panel.settingsToggle.getAttribute("aria-expanded") === "true" ? jt() : (Cn(), Qt(), Yt()), u.Panel.settingsHeader.focus();
  }), R.getItem("sa11y-settings") === "Opened" && Cn(), u.Panel.skipToPageIssues.addEventListener("click", () => {
  ...
  t.length !== 0 && t.forEach((e) => {
    e.addEventListener("keydown", (n) => {
      ...
      let o;
      n.key === "ArrowRight" ? o = (s + 1) % t.length : o = (s - 1 + t.length) % t.length, t[o].focus();
    });
  });
}
function ma(t, e, n) {
  const s = () => {
    ...
    if (n.showTitleInPageOutline) {
      ...
      if (!l || l.textContent.trim().length === 0)
        ...
        const d = Q(l);
        i = `<li><span class="badge">${f._("TITLE")}</span> ${Y(d)}</li>`;
      }
      o.push(i);
    }
    e.forEach((l, d) => {
      const { element: p, headingLevel: r, text: c, type: m, dismissedHeading: h, isWithinRoot: A } = l, b = pn(p), y = b === !0 ? `<span class="hidden-icon"></span><span class="visually-hidden">${f._("HIDDEN")}</span>` : "", E = n.showHinPageOutline === !0 || n.showHinPageOutline === 1 ? "H" : "";
      let v;
      m === "error" && A === !0 ? (v = `
            <li class="outline-${r}">
      ...
      if (g.id = `sa11y-h${d}`, b) {
        ...
        S?.insertAdjacentElement("beforebegin", g), S?.setAttribute("data-sa11y-parent", `h${d}`);
      } else
        L?.insertAdjacentElement("beforebegin", g);
      const _ = document.createElement("span");
      _.classList.add("heading-label"), _.innerHTML = `H${r}`, L.shadowRoot.appendChild(_), R.getItem("sa11y-outline") === "Opened" && (L.hidden = !1);
    }), u.Panel.outlineList.innerHTML = e.length === 0 ? `${i || ""} <li>${f._("PANEL_NO_HEADINGS")}</li>` : o.join(" "), setTimeout(() => {
      ...
      l.forEach((d, p) => {
        d.addEventListener("click", () => {
          ...
          r && (r.scrollIntoView({
            ...
            block: "center"
          }), Dt(r.parentNode || r)), We(), (!r || r.hasAttribute("data-sa11y-parent")) && je(f._("NOT_VISIBLE"));
        });
      }), rs(u.Panel.outlineList, l);
    }, 0), document.removeEventListener("sa11y-build-heading-outline", s);
  };
  R.getItem("sa11y-outline") === "Opened" && s(), document.addEventListener("sa11y-build-heading-outline", s);
}
const nt = (t, e) => {
  const n = (a) => {
    if (a === "none")
  ...
  ), o = n(window.getComputedStyle(t, ":after").getPropertyValue("content"));
  return `${s}${e}${o}`;
}, Pn = (t) => {
  for (let e = 0; e < 1e3; e++) {
    if (t.nextSibling())
  ...
  if (!e && n)
    return n.split(/\s+/).filter((o) => o.trim()).map((o) => {
  ...
  if (!t.children.length)
    return o = nt(t, t.textContent), !o.trim() && t.hasAttribute("title") ? t.getAttribute("title") : o;
  function a(A, b, y) {
  ...
  const i = a(t), l = ["noscript", "style", "script", "video", "audio"], d = [...e, ...l].join(", "), p = d ? t.querySelectorAll(d) : [];
  let r = !1, c = !1, m = 0, h = !0;
  for (; i.nextNode() && h; ) {
    m += 1;
    ...
    if (A.shadowRoot) {
      ...
      for (let v = 0; v < E.length; v++) {
        const L = E[v];
        (!d || !L.closest(d)) && (o += Te(L, e, n + 1));
      }
    }
    if (A.nodeType === Node.TEXT_NODE) {
      A.parentNode.tagName !== "SLOT" && (o += ` ${A.nodeValue}`);
      continue;
    }
    if (r && !A.closest("a") && (c === o && (o += r), r = !1, c = !1), A.ariaHidden === "true" && !(n && m < 3)) {
      Pn(i) || (h = !1);
      continue;
    }
    const y = ke(A, n);
    ...
    switch (A.tagName) {
      case "IMG":
        A.hasAttribute("alt") && A.role !== "presentation" && (o += A.getAttribute("alt"));
        break;
      case "SVG":
        ...
        break;
      case "A":
        A.hasAttribute("title") ? (r = A.getAttribute("title"), c = o) : (r = !1, c = !1), o += nt(A, "");
        break;
      case "INPUT":
        o += nt(i.currentNode, ""), i.currentNode.hasAttribute("title") && (r = i.currentNode.getAttribute("title"));
        break;
      case "SLOT": {
  ...
  return r && !c && (o += ` ${r}`), o = o.replace(/[\uE000-\uF8FF]/gu, ""), !o.trim() && t.hasAttribute("title") ? t.getAttribute("title") : o;
}, Aa = (t) => {
  const { src: e } = t.element, n = u.Global.ignoreEditImageURL.some((r) => e.includes(r)), s = u.Global.ignoreEditImageClass.some(
    (r) => t.element.classList.contains(r)
  );
  if (n || s)
    return "";
  const o = u.Global.relativePathImageSRC || window.location.host, a = e.split(o)[1] || "", i = u.Global.relativePathImageID;
  let l;
  i.length && t.element.classList.length && t.element.classList.forEach((r) => {
    if (r.startsWith(i)) {
      const [c] = r.match(/\d+/) || [];
      l = c;
    }
  });
  const d = o && i.length ? u.Global.editImageURLofCMS + l : u.Global.editImageURLofCMS + a, p = (r) => r.includes(window.location.host) || r.startsWith(o);
  return (i.length && l !== void 0 || !i) && p(e) ? `<div class="edit-block"><a href="${encodeURI(d)}" tabindex="-1" target="_blank" rel="noopener noreferrer" class="edit">${f._("EDIT")}</a></div>` : "";
};
function ba(t, e, n) {
  const s = () => {
    const o = [];
    e.forEach((a, i) => {
      ...
      const { element: d, type: p, developer: r, dismissedImage: c } = a, m = ke(d) === "noAria" ? ct(d.getAttribute("alt")) : ke(d), h = pn(d);
      if (h) {
        const C = ht(d, "display", "none"), k = document.createElement("sa11y-image-anchor");
        k.setAttribute("data-sa11y-parent", `image${i}`), (C?.previousElementSibling || C?.parentNode)?.insertAdjacentElement("beforebegin", k);
      } else
        d.setAttribute("data-sa11y-image", i);
      const A = R.getItem("sa11y-developer"), y = (A === "Off" || A === null) && (p === "error" || p === "warning") && r === !0, E = ss(a.element), v = u.Global.editImageURLofCMS ? Aa(a) : "", L = d.hasAttribute("alt") && m === "" ? `<div class="badge">${f._("DECORATIVE")}</div>` : "", g = n.altPlaceholder?.some(
        (C) => m.toLowerCase().startsWith(C.toLowerCase())
      ), _ = n.imageWithinLightbox ? `a[href]:not(${n.imageWithinLightbox})` : "a[href]", I = d.closest(_) ? `<div class="badge"><span class="link-icon"></span><span class="visually-hidden">${f._("LINKED")}</span></div>` : "", S = h === !0 ? `<div class="badge"><span class="hidden-icon"></span><span class="visually-hidden">${f._("HIDDEN")}</span></div>` : "";
      let x;
      if (p === "error" && !y) {
    ...
    }), u.Panel.imagesList.innerHTML = o.length === 0 ? `<li class="no-images">${f._("NO_IMAGES")}</li>` : o.join(" "), setTimeout(() => {
  ...
  R.getItem("sa11y-images") === "Opened" && s(), document.addEventListener("sa11y-build-image-outline", s);
}
function ya(t, e, n) {
...
function Ea(t, e) {
  const n = t + e;
  n === 0 ? (u.Panel.notifCount.innerText = "", u.Panel.notifText.innerText = "", u.Panel.notifBadge.style.display = "none") : e > 0 && t === 0 ? (u.Panel.notifBadge.classList.add("notification-badge-warning"), u.Panel.notifCount.innerText = `${e}`, u.Panel.notifText.innerText = `${f._("WARNINGS_FOUND")}`) : (u.Panel.notifBadge.classList.remove("notification-badge-warning"), u.Panel.notifCount.innerText = `${n}`, u.Panel.notifText.innerText = f._("TOTAL_FOUND")), R.getItem("sa11y-panel") === "Opened" || n === 0 ? u.Panel.notifBadge.style.display = "none" : u.Panel.notifBadge.style.display = "flex";
}
function va(t, e, n) {
  let s = e, o = n;
  return t.forEach((a, i) => {
    const l = t[i].type;
    l === "error" ? s += 1 : l === "warning" && (o += 1);
  }), { error: s, warning: o };
}
var ee = "top", ce = "bottom", de = "right", te = "left", fn = "auto", At = [ee, ce, de, te], ze = "start", dt = "end", La = "clippingParents", ls = "viewport", st = "popper", Ta = "reference", On = /* @__PURE__ */ At.reduce(function(t, e) {
  return t.concat([e + "-" + ze, e + "-" + dt]);
}, []), cs = /* @__PURE__ */ [].concat(At, [fn]).reduce(function(t, e) {
  return t.concat([e, e + "-" + ze, e + "-" + dt]);
}, []), Ia = "beforeRead", _a = "read", xa = "afterRead", Na = "beforeMain", Sa = "main", ka = "afterMain", wa = "beforeWrite", Ca = "write", Pa = "afterWrite", Oa = [Ia, _a, xa, Na, Sa, ka, wa, Ca, Pa];
function ye(t) {
...
function Ra(t) {
  ...
  Object.keys(e.elements).forEach(function(n) {
    var s = e.styles[n] || {}, o = e.attributes[n] || {}, a = e.elements[n];
    !le(a) || !ye(a) || (Object.assign(a.style, s), Object.keys(o).forEach(function(i) {
      var l = o[i];
...
function Da(t) {
  ...
  return Object.assign(e.elements.popper.style, n.popper), e.styles = n, e.elements.arrow && Object.assign(e.elements.arrow.style, n.arrow), function() {
    Object.keys(e.elements).forEach(function(s) {
      var o = e.elements[s], a = e.attributes[s] || {}, i = Object.keys(e.styles.hasOwnProperty(s) ? e.styles[s] : n[s]), l = i.reduce(function(d, p) {
        return d[p] = "", d;
      }, {});
      !le(o) || !ye(o) || (Object.assign(o.style, l), Object.keys(a).forEach(function(d) {
        o.removeAttribute(d);
      }));
    });
  };
...
var Re = Math.max, Mt = Math.min, Ve = Math.round;
function an() {
...
function Qe(t, e, n) {
  ...
  var s = t.getBoundingClientRect(), o = 1, a = 1;
  e && le(t) && (o = t.offsetWidth > 0 && Ve(s.width) / t.offsetWidth || 1, a = t.offsetHeight > 0 && Ve(s.height) / t.offsetHeight || 1);
  var i = Me(t) ? ae(t) : window, l = i.visualViewport, d = !ps() && n, p = (s.left + (d && l ? l.offsetLeft : 0)) / o, r = (s.top + (d && l ? l.offsetTop : 0)) / a, c = s.width / o, m = s.height / a;
  return {
    width: c,
    height: m,
    top: r,
...
function Ba(t) {
  ...
  var o = Ht(t);
  for (hn(o) && (o = o.host); le(o) && ["html", "body"].indexOf(ye(o)) < 0; ) {
    var a = Ie(o);
...
function bt(t) {
  for (var e = ae(t), n = Rn(t); n && Ma(n) && Ie(n).position === "static"; )
    n = Rn(n);
  return n && (ye(n) === "html" || ye(n) === "body" && Ie(n).position === "static") ? e : n || Ba(t) || e;
}
function mn(t) {
...
var Ha = function(e, n) {
  ...
  })) : e, hs(typeof e != "number" ? e : gs(e, At));
};
function Ua(t) {
  var e, n = t.state, s = t.name, o = t.options, a = n.elements.arrow, i = n.modifiersData.popperOffsets, l = be(n.placement), d = mn(l), p = [te, de].indexOf(l) >= 0, r = p ? "height" : "width";
  if (!(!a || !i)) {
    var c = Ha(o.padding, n), m = gn(a), h = d === "y" ? ee : te, A = d === "y" ? ce : de, b = n.rects.reference[r] + n.rects.reference[d] - i[d] - n.rects.popper[r], y = i[d] - n.rects.reference[d], E = bt(a), v = E ? d === "y" ? E.clientHeight || 0 : E.clientWidth || 0 : 0, L = b / 2 - y / 2, g = c[h], _ = v - m[r] - c[A], I = v / 2 - m[r] / 2 + L, S = it(g, I, _), x = d;
    n.modifiersData[s] = (e = {}, e[x] = S, e.centerOffset = S - I, e);
  }
}
function $a(t) {
  var e = t.state, n = t.options, s = n.element, o = s === void 0 ? "[data-popper-arrow]" : s;
  o != null && (typeof o == "string" && (o = e.elements.popper.querySelector(o), !o) || us(e.elements.popper, o) && (e.elements.arrow = o));
}
const Fa = {
  name: "arrow",
  enabled: !0,
...
function Dn(t) {
  var e, n = t.popper, s = t.popperRect, o = t.placement, a = t.variation, i = t.offsets, l = t.position, d = t.gpuAcceleration, p = t.adaptive, r = t.roundOffsets, c = t.isFixed, m = i.x, h = m === void 0 ? 0 : m, A = i.y, b = A === void 0 ? 0 : A, y = typeof r == "function" ? r({
    x: h,
    y: b
  }) : {
    x: h,
  ...
  h = y.x, b = y.y;
  var E = i.hasOwnProperty("x"), v = i.hasOwnProperty("y"), L = te, g = ee, _ = window;
  if (p) {
    var I = bt(n), S = "clientHeight", x = "clientWidth";
    if (I === ae(n) && (I = we(n), Ie(I).position !== "static" && l === "absolute" && (S = "scrollHeight", x = "scrollWidth")), I = I, o === ee || (o === te || o === de) && a === dt) {
      g = ce;
      var C = c && I === _ && _.visualViewport ? _.visualViewport.height : (
        // $FlowFixMe[prop-missing]
        I[S]
      );
      b -= C - s.height, b *= d ? 1 : -1;
    }
    if (o === te || (o === ee || o === ce) && a === dt) {
      L = de;
      var k = c && I === _ && _.visualViewport ? _.visualViewport.width : (
        // $FlowFixMe[prop-missing]
        I[x]
      );
      h -= k - s.width, h *= d ? 1 : -1;
  ...
  if (h = U.x, b = U.y, d) {
    var $;
    return Object.assign({}, M, ($ = {}, $[g] = v ? "0" : "", $[L] = E ? "0" : "", $.transform = (_.devicePixelRatio || 1) <= 1 ? "translate(" + h + "px, " + b + "px)" : "translate3d(" + h + "px, " + b + "px, 0)", $));
  }
  return Object.assign({}, M, (e = {}, e[g] = v ? b + "px" : "", e[L] = E ? h + "px" : "", e.transform = "", e));
}
function za(t) {
  var e = t.state, n = t.options, s = n.gpuAcceleration, o = s === void 0 ? !0 : s, a = n.adaptive, i = a === void 0 ? !0 : a, l = n.roundOffsets, d = l === void 0 ? !0 : l, p = {
...
function Qa(t) {
  var e = t.state, n = t.instance, s = t.options, o = s.scroll, a = o === void 0 ? !0 : o, i = s.resize, l = i === void 0 ? !0 : i, d = ae(e.elements.popper), p = [].concat(e.scrollParents.reference, e.scrollParents.popper);
  return a && p.forEach(function(r) {
    r.addEventListener("scroll", n.update, wt);
  }), l && d.addEventListener("resize", n.update, wt), function() {
    a && p.forEach(function(r) {
      r.removeEventListener("scroll", n.update, wt);
    }), l && d.removeEventListener("resize", n.update, wt);
  };
}
const Ya = {
  name: "eventListeners",
...
function Xa(t, e) {
  var n = ae(t), s = we(t), o = n.visualViewport, a = s.clientWidth, i = s.clientHeight, l = 0, d = 0;
  if (o) {
...
function Ja(t) {
  var e, n = we(t), s = An(t), o = (e = t.ownerDocument) == null ? void 0 : e.body, a = Re(n.scrollWidth, n.clientWidth, o ? o.scrollWidth : 0, o ? o.clientWidth : 0), i = Re(n.scrollHeight, n.clientHeight, o ? o.scrollHeight : 0, o ? o.clientHeight : 0), l = -s.scrollLeft + bn(t), d = -s.scrollTop;
  return Ie(o || n).direction === "rtl" && (l += Re(n.clientWidth, o ? o.clientWidth : 0) - a), {
    width: a,
    height: i,
    x: l,
    y: d
...
function yn(t) {
  var e = Ie(t), n = e.overflow, s = e.overflowX, o = e.overflowY;
  return /auto|scroll|overlay|hidden/.test(n + o + s);
}
function ms(t) {
  return ["html", "body", "#document"].indexOf(ye(t)) >= 0 ? t.ownerDocument.body : le(t) && yn(t) ? t : ms(Ht(t));
}
function rt(t, e) {
  var n;
  e === void 0 && (e = []);
  var s = ms(t), o = s === ((n = t.ownerDocument) == null ? void 0 : n.body), a = ae(s), i = o ? [a].concat(a.visualViewport || [], yn(s) ? s : []) : s, l = e.concat(i);
  return o ? l : (
...
function Bn(t, e, n) {
  return e === ls ? rn(Xa(t, n)) : Me(e) ? Za(e, n) : rn(Ja(we(t)));
}
function ei(t) {
  var e = rt(Ht(t)), n = ["absolute", "fixed"].indexOf(Ie(t).position) >= 0, s = n && le(t) ? bt(t) : t;
  return Me(s) ? e.filter(function(o) {
    return Me(o) && us(o, s) && ye(o) !== "body";
  }) : [];
}
function ti(t, e, n, s) {
  var o = e === "clippingParents" ? ei(t) : [].concat(e), a = [].concat(o, [n]), i = a[0], l = a.reduce(function(d, p) {
    var r = Bn(t, p, s);
    return d.top = Re(r.top, d.top), d.right = Mt(r.right, d.right), d.bottom = Mt(r.bottom, d.bottom), d.left = Re(r.left, d.left), d;
  }, Bn(t, i, s));
  return l.width = l.right - l.left, l.height = l.bottom - l.top, l.x = l.left, l.y = l.top, l;
}
function As(t) {
  var e = t.reference, n = t.element, s = t.placement, o = s ? be(s) : null, a = s ? Ye(s) : null, i = e.x + e.width / 2 - n.width / 2, l = e.y + e.height / 2 - n.height / 2, d;
  switch (o) {
    case ee:
      d = {
        x: i,
...
function pt(t, e) {
  e === void 0 && (e = {});
  var n = e, s = n.placement, o = s === void 0 ? t.placement : s, a = n.strategy, i = a === void 0 ? t.strategy : a, l = n.boundary, d = l === void 0 ? La : l, p = n.rootBoundary, r = p === void 0 ? ls : p, c = n.elementContext, m = c === void 0 ? st : c, h = n.altBoundary, A = h === void 0 ? !1 : h, b = n.padding, y = b === void 0 ? 0 : b, E = hs(typeof y != "number" ? y : gs(y, At)), v = m === st ? Ta : st, L = t.rects.popper, g = t.elements[A ? v : m], _ = ti(Me(g) ? g : g.contextElement || we(t.elements.popper), d, r, i), I = Qe(t.elements.reference), S = As({
    reference: I,
    element: L,
    placement: o
  }), x = rn(Object.assign({}, L, S)), C = m === st ? x : I, k = {
    top: _.top - C.top + E.top,
    bottom: C.bottom - _.bottom + E.bottom,
    left: _.left - C.left + E.left,
    right: C.right - _.right + E.right
  }, M = t.modifiersData.offset;
  if (m === st && M) {
    var U = M[o];
    Object.keys(k).forEach(function($) {
      var w = [de, ce].indexOf($) >= 0 ? 1 : -1, O = [ee, ce].indexOf($) >= 0 ? "y" : "x";
      k[$] += U[O] * w;
    });
  }
  return k;
...
function ni(t, e) {
  e === void 0 && (e = {});
  var n = e, s = n.placement, o = n.boundary, a = n.rootBoundary, i = n.padding, l = n.flipVariations, d = n.allowedAutoPlacements, p = d === void 0 ? cs : d, r = Ye(s), c = r ? l ? On : On.filter(function(A) {
    return Ye(A) === r;
  }) : At, m = c.filter(function(A) {
    return p.indexOf(A) >= 0;
  });
  m.length === 0 && (m = c);
  var h = m.reduce(function(A, b) {
    return A[b] = pt(t, {
      placement: b,
      boundary: o,
      rootBoundary: a,
...
function ai(t) {
  ...
  if (!e.modifiersData[s]._skip) {
    for (var o = n.mainAxis, a = o === void 0 ? !0 : o, i = n.altAxis, l = i === void 0 ? !0 : i, d = n.fallbackPlacements, p = n.padding, r = n.boundary, c = n.rootBoundary, m = n.altBoundary, h = n.flipVariations, A = h === void 0 ? !0 : h, b = n.allowedAutoPlacements, y = e.options.placement, E = be(y), v = E === y, L = d || (v || !A ? [Ot(y)] : si(y)), g = [y].concat(L).reduce(function(j, q) {
      return j.concat(be(q) === fn ? ni(e, {
        placement: q,
        boundary: r,
        rootBoundary: c,
        padding: p,
      ...
      var M = g[k], U = be(M), $ = Ye(M) === ze, w = [ee, ce].indexOf(U) >= 0, O = w ? "width" : "height", G = pt(e, {
        placement: M,
        boundary: r,
        rootBoundary: c,
        altBoundary: m,
      ...
      _[O] > I[O] && (H = Ot(H));
      var K = Ot(H), ne = [];
      if (a && ne.push(G[U] <= 0), l && ne.push(G[H] <= 0, G[K] <= 0), ne.every(function(j) {
        return j;
      })) {
        C = M, x = !1;
        break;
    ...
    if (x)
      for (var ie = A ? 3 : 1, Ee = function(q) {
        var pe = g.find(function(ge) {
          var ue = S.get(ge);
          if (ue)
            return ue.slice(0, q).every(function(Be) {
              return Be;
            });
        });
        if (pe)
...
function Hn(t) {
  return [ee, de, ce, te].some(function(e) {
    return t[e] >= 0;
  });
}
function ri(t) {
  var e = t.state, n = t.name, s = e.rects.reference, o = e.rects.popper, a = e.modifiersData.preventOverflow, i = pt(e, {
    elementContext: "reference"
  }), l = pt(e, {
    altBoundary: !0
  }), d = Gn(i, s), p = Gn(l, o, a), r = Hn(d), c = Hn(p);
  e.modifiersData[n] = {
...
function li(t, e, n) {
  var s = be(t), o = [te, ee].indexOf(s) >= 0 ? -1 : 1, a = typeof n == "function" ? n(Object.assign({}, e, {
    placement: t
  })) : n, i = a[0], l = a[1];
  return i = i || 0, l = (l || 0) * o, [te, de].indexOf(s) >= 0 ? {
    x: l,
    y: i
  } : {
    x: i,
...
function ci(t) {
  var e = t.state, n = t.options, s = t.name, o = n.offset, a = o === void 0 ? [0, 0] : o, i = cs.reduce(function(r, c) {
    return r[c] = li(c, e.rects, a), r;
  }, {}), l = i[e.placement], d = l.x, p = l.y;
  e.modifiersData.popperOffsets != null && (e.modifiersData.popperOffsets.x += d, e.modifiersData.popperOffsets.y += p), e.modifiersData[s] = i;
}
const di = {
  name: "offset",
  enabled: !0,
...
function hi(t) {
  var e = t.state, n = t.options, s = t.name, o = n.mainAxis, a = o === void 0 ? !0 : o, i = n.altAxis, l = i === void 0 ? !1 : i, d = n.boundary, p = n.rootBoundary, r = n.altBoundary, c = n.padding, m = n.tether, h = m === void 0 ? !0 : m, A = n.tetherOffset, b = A === void 0 ? 0 : A, y = pt(e, {
    boundary: d,
    rootBoundary: p,
    padding: c,
    altBoundary: r
  }), E = be(e.placement), v = Ye(e.placement), L = !v, g = mn(E), _ = fi(g), I = e.modifiersData.popperOffsets, S = e.rects.reference, x = e.rects.popper, C = typeof b == "function" ? b(Object.assign({}, e.rects, {
    placement: e.placement
  })) : b, k = typeof C == "number" ? {
    mainAxis: C,
    altAxis: C
  ...
  if (I) {
    if (a) {
      var $, w = g === "y" ? ee : te, O = g === "y" ? ce : de, G = g === "y" ? "height" : "width", H = I[g], K = H + y[w], ne = H - y[O], ie = h ? -x[G] / 2 : 0, Ee = v === ze ? S[G] : x[G], se = v === ze ? -x[G] : -S[G], V = e.elements.arrow, j = h && V ? gn(V) : {
        width: 0,
        height: 0
      }, q = e.modifiersData["arrow#persistent"] ? e.modifiersData["arrow#persistent"].padding : fs(), pe = q[w], ge = q[O], ue = it(0, S[G], j[G]), Be = L ? S[G] / 2 - ie - ue - pe - k.mainAxis : Ee - ue - pe - k.mainAxis, _e = L ? -S[G] / 2 + ie + ue + ge + k.mainAxis : se + ue + ge + k.mainAxis, Ge = e.elements.arrow && bt(e.elements.arrow), yt = Ge ? g === "y" ? Ge.clientTop || 0 : Ge.clientLeft || 0 : 0, Xe = ($ = M?.[g]) != null ? $ : 0, Et = H + Be - Xe - yt, vt = H + _e - Xe, Je = it(h ? Mt(K, Et) : K, H, h ? Re(ne, vt) : ne);
      I[g] = Je, U[g] = Je - H;
    }
    if (l) {
      var Ze, Lt = g === "x" ? ee : te, Tt = g === "x" ? ce : de, ve = I[_], xe = _ === "y" ? "height" : "width", et = ve + y[Lt], Ce = ve - y[Tt], tt = [ee, te].indexOf(E) !== -1, It = (Ze = M?.[_]) != null ? Ze : 0, _t = tt ? et : ve - S[xe] - x[xe] - It + k.altAxis, xt = tt ? ve + S[xe] + x[xe] - It - k.altAxis : Ce, Nt = h && tt ? Ga(_t, ve, xt) : it(h ? _t : et, ve, h ? xt : Ce);
      I[_] = Nt, U[_] = Nt - ve;
    }
    e.modifiersData[s] = U;
  }
...
function yi(t, e, n) {
  n === void 0 && (n = !1);
  var s = le(e), o = le(e) && bi(e), a = we(e), i = Qe(t, o, n), l = {
    scrollLeft: 0,
    scrollTop: 0
  }, d = {
    x: 0,
  ...
  return (s || !s && !n) && ((ye(e) !== "body" || // https://github.com/popperjs/popper-core/issues/1078
  yn(a)) && (l = Ai(e)), le(e) ? (d = Qe(e, !0), d.x += e.clientLeft, d.y += e.clientTop) : a && (d.x = bn(a))), {
...
function Ii(t) {
  t === void 0 && (t = {});
  var e = t, n = e.defaultModifiers, s = n === void 0 ? [] : n, o = e.defaultOptions, a = o === void 0 ? Un : o;
  return function(l, d, p) {
    ...
    var r = {
      ...
      setOptions: function(E) {
        var v = typeof E == "function" ? E(r.options) : E;
        b(), r.options = Object.assign({}, a, r.options, v), r.scrollParents = {
          reference: Me(l) ? rt(l) : l.contextElement ? rt(l.contextElement) : [],
          popper: rt(d)
        };
        var L = vi(Ti([].concat(s, r.options.modifiers)));
        return r.orderedModifiers = L.filter(function(g) {
          return g.enabled;
        }), A(), h.update();
      },
      ...
      forceUpdate: function() {
        if (!m) {
          var E = r.elements, v = E.reference, L = E.popper;
          if ($n(v, L)) {
            r.rects = {
              reference: yi(v, bt(L), r.options.strategy === "fixed"),
              popper: gn(L)
            }, r.reset = !1, r.placement = r.options.placement, r.orderedModifiers.forEach(function(k) {
              return r.modifiersData[k.name] = Object.assign({}, k.data);
            });
            for (var g = 0; g < r.orderedModifiers.length; g++) {
              ...
              var _ = r.orderedModifiers[g], I = _.fn, S = _.options, x = S === void 0 ? {} : S, C = _.name;
    ...
    function A() {
      r.orderedModifiers.forEach(function(y) {
        var E = y.name, v = y.options, L = v === void 0 ? {} : v, g = y.effect;
...
var _i = [Ya, ui, Va, ds, di, ii, gi, Fa, oi], xi = /* @__PURE__ */ Ii({
  defaultModifiers: _i
}), Ni = "tippy-box", bs = "tippy-content", Si = "tippy-backdrop", ys = "tippy-arrow", Es = "tippy-svg-arrow", Oe = {
  passive: !0,
  capture: !0
}, vs = function() {
...
function Di(t) {
  return Ut(t) ? [t] : Pi(t) ? Bt(t) : Array.isArray(t) ? t : Bt(document.querySelectorAll(t));
}
function Xt(t, e) {
...
function Bi(t, e) {
  ...
  return t.every(function(o) {
    var a = o.popperRect, i = o.popperState, l = o.props, d = l.interactiveBorder, p = Ci(i.placement), r = i.modifiersData.offset;
    if (!r)
      return !0;
    var c = p === "bottom" ? r.top.y : 0, m = p === "top" ? r.bottom.y : 0, h = p === "right" ? r.left.x : 0, A = p === "left" ? r.right.x : 0, b = a.top - s + c > d, y = s - a.bottom - m > d, E = a.left - n + h > d, v = n - a.right - A > d;
    return b || y || E || v;
  });
}
function Jt(t, e, n) {
...
function Gi() {
  me.isTouch || (me.isTouch = !0, window.performance && document.addEventListener("mousemove", Ts));
}
function Ts() {
  var t = performance.now();
  t - Qn < 20 && (me.isTouch = !1, document.removeEventListener("mousemove", Ts)), Qn = t;
}
function Hi() {
...
function Ui() {
  document.addEventListener("touchstart", Gi, Oe), window.addEventListener("blur", Hi);
}
var $i = typeof window < "u" && typeof document < "u", Fi = $i ? (
  ...
  triggerTarget: null
}, Ki, Wi), zi = Object.keys(he), Vi = function(e) {
  var n = Object.keys(e);
...
function Is(t) {
  var e = t.plugins || [], n = e.reduce(function(s, o) {
    var a = o.name, i = o.defaultValue;
    if (a) {
      var l;
      s[a] = t[a] !== void 0 ? t[a] : (l = he[a]) != null ? l : i;
    }
    return s;
  }, {});
...
function Qi(t, e) {
  var n = e ? Object.keys(Is(Object.assign({}, he, {
    plugins: e
  }))) : zi, s = n.reduce(function(o, a) {
...
function Yn(t, e) {
  var n = Object.assign({}, e, {
    content: Ls(e.content, [t])
  }, e.ignoreAttributes ? {} : Qi(t, e.plugins));
  return n.aria = Object.assign({}, he.aria, n.aria), n.aria = {
...
function jn(t) {
  var e = ot();
  return t === !0 ? e.className = ys : (e.className = Es, Ut(t) ? e.appendChild(t) : on(e, t)), e;
}
function qn(t, e) {
...
function ln(t) {
  ...
  return {
    ...
    content: n.find(function(s) {
      return s.classList.contains(bs);
    }),
    arrow: n.find(function(s) {
      return s.classList.contains(ys) || s.classList.contains(Es);
    }),
...
function _s(t) {
  ...
  var s = ot();
  s.className = bs, s.setAttribute("data-state", "hidden"), qn(s, t.props), e.appendChild(n), n.appendChild(s), o(t.props, t.props);
  function o(a, i) {
    var l = ln(e), d = l.box, p = l.content, r = l.arrow;
    i.theme ? d.setAttribute("data-theme", i.theme) : d.removeAttribute("data-theme"), typeof i.animation == "string" ? d.setAttribute("data-animation", i.animation) : d.removeAttribute("data-animation"), i.inertia ? d.setAttribute("data-inertia", "") : d.removeAttribute("data-inertia"), d.style.maxWidth = typeof i.maxWidth == "number" ? i.maxWidth + "px" : i.maxWidth, i.role ? d.setAttribute("role", i.role) : d.removeAttribute("role"), (a.content !== i.content || a.allowHTML !== i.allowHTML) && qn(p, t.props), i.arrow ? r ? a.arrow !== i.arrow && (d.removeChild(r), d.appendChild(jn(i.arrow))) : d.appendChild(jn(i.arrow)) : r && d.removeChild(r);
  }
  return {
    popper: e,
    onUpdate: o
...
var ji = 1, Ct = [], Zt = [];
function qi(t, e) {
  var n = Yn(t, Object.assign({}, he, Is(Wn(e)))), s, o, a, i = !1, l = !1, d = !1, p = !1, r, c, m, h = [], A = Fn(Et, n.interactiveDebounce), b, y = ji++, E = null, v = wi(n.plugins), L = {
  ...
  return Ge(), ie(), H(), K("onCreate", [g]), n.showOnCreate && et(), I.addEventListener("mouseenter", function() {
  ...
  function G(T) {
    return g.state.isMounted && !g.state.isVisible || me.isTouch || r && r.type === "focus" ? 0 : qt(g.props.delay, T ? 0 : 1, he.delay);
  }
  function H(T) {
    T === void 0 && (T = !1), I.style.pointerEvents = g.props.interactive && !T ? "" : "none", I.style.zIndex = "" + g.props.zIndex;
  }
  function K(T, P, B) {
    if (B === void 0 && (B = !0), x.forEach(function(F) {
      F[T] && F[T].apply(F, P);
    }), B) {
      var W;
      (W = g.props)[T].apply(W, P);
    }
  }
  function ne() {
    var T = g.props.aria;
    if (T.content) {
      var P = "aria-" + T.content, B = I.id, W = Fe(g.props.triggerTarget || t);
      W.forEach(function(F) {
        var X = F.getAttribute(P);
        if (g.state.isVisible)
          F.setAttribute(P, X ? X + " " + B : B);
        else {
          var re = X && X.replace(B, "").trim();
          re ? F.setAttribute(P, re) : F.removeAttribute(P);
        }
      });
    }
  }
  function ie() {
    if (!(C || !g.props.aria.expanded)) {
      var T = Fe(g.props.triggerTarget || t);
      T.forEach(function(P) {
        g.props.interactive ? P.setAttribute("aria-expanded", g.state.isVisible && P === $() ? "true" : "false") : P.removeAttribute("aria-expanded");
      });
    }
  }
  function Ee() {
  ...
  function se(T) {
    if (!(me.isTouch && (d || T.type === "mousedown"))) {
      var P = T.composedPath && T.composedPath()[0] || T.target;
      if (!(g.props.interactive && Vn(I, P))) {
        if (Fe(g.props.triggerTarget || t).some(function(B) {
          return Vn(B, P);
        })) {
          if (me.isTouch || g.state.isVisible && g.props.trigger.indexOf("click") >= 0)
            return;
        } else
          K("onClickOutside", [g, T]);
        g.props.hideOnClick === !0 && (g.clearDelayTimeouts(), g.hide(), l = !0, setTimeout(function() {
          l = !1;
        }), g.state.isMounted || pe());
      }
    }
  }
  function V() {
  ...
  function q() {
    var T = w();
    T.addEventListener("mousedown", se, !0), T.addEventListener("touchend", se, Oe), T.addEventListener("touchstart", j, Oe), T.addEventListener("touchmove", V, Oe);
  }
  function pe() {
    var T = w();
    T.removeEventListener("mousedown", se, !0), T.removeEventListener("touchend", se, Oe), T.removeEventListener("touchstart", j, Oe), T.removeEventListener("touchmove", V, Oe);
  }
  function ge(T, P) {
    Be(T, function() {
      !g.state.isVisible && I.parentNode && I.parentNode.contains(I) && P();
    });
  }
  function ue(T, P) {
    Be(T, P);
  ...
  function Be(T, P) {
    var B = O().box;
    function W(F) {
      F.target === B && (Jt(B, "remove", W), P());
    }
    if (T === 0)
      return P();
    Jt(B, "remove", c), Jt(B, "add", W), c = W;
  }
  function _e(T, P, B) {
    B === void 0 && (B = !1);
    var W = Fe(g.props.triggerTarget || t);
    W.forEach(function(F) {
      F.addEventListener(T, P, B), h.push({
        node: F,
        eventType: T,
        handler: P,
        options: B
  ...
  function yt() {
    h.forEach(function(T) {
      var P = T.node, B = T.eventType, W = T.handler, F = T.options;
  ...
  function Xe(T) {
    ...
    if (!(!g.state.isEnabled || Ze(T) || l)) {
      var W = ((P = r) == null ? void 0 : P.type) === "focus";
      r = T, b = T.currentTarget, ie(), !g.state.isVisible && Oi(T) && Ct.forEach(function(F) {
        return F(T);
      }), T.type === "click" && (g.props.trigger.indexOf("mouseenter") < 0 || i) && g.props.hideOnClick !== !1 && g.state.isVisible ? B = !0 : et(T), T.type === "click" && (i = !B), B && !W && Ce(T);
    }
  }
  function Et(T) {
    var P = T.target, B = $().contains(P) || I.contains(P);
    if (!(T.type === "mousemove" && B)) {
      var W = xe().concat(I).map(function(F) {
        var X, re = F._tippy, He = (X = re.popperInstance) == null ? void 0 : X.state;
        return He ? {
  ...
  function vt(T) {
    var P = Ze(T) || g.props.trigger.indexOf("click") >= 0 && i;
    if (!P) {
  ...
  function Je(T) {
    g.props.trigger.indexOf("focusin") < 0 && T.target !== $() || g.props.interactive && T.relatedTarget && I.contains(T.relatedTarget) || Ce(T);
  }
  function Ze(T) {
    return me.isTouch ? M() !== T.type.indexOf("touch") >= 0 : !1;
  }
  function Lt() {
    Tt();
    var T = g.props, P = T.popperOptions, B = T.placement, W = T.offset, F = T.getReferenceClientRect, X = T.moveTransition, re = U() ? ln(I).arrow : null, He = F ? {
      getBoundingClientRect: F,
      contextElement: F.contextElement || $()
    } : t, In = {
      name: "$$tippy",
      enabled: !0,
      phase: "beforeWrite",
      ...
      fn: function(St) {
        ...
        if (U()) {
          ...
          ["placement", "reference-hidden", "escaped"].forEach(function(kt) {
            kt === "placement" ? Kt.setAttribute("data-placement", Ue.placement) : Ue.attributes.popper["data-popper-" + kt] ? Kt.setAttribute("data-" + kt, "") : Kt.removeAttribute("data-" + kt);
    ...
    }), Pe.push.apply(Pe, P?.modifiers || []), g.popperInstance = xi(He, I, Object.assign({}, P, {
      placement: B,
      onFirstUpdate: m,
      modifiers: Pe
    }));
  ...
  function ve() {
    var T = g.props.appendTo, P, B = $();
    g.props.interactive && T === vs || T === "parent" ? P = B.parentNode : P = Ls(T, [B]), P.contains(I) || P.appendChild(I), g.state.isMounted = !0, Lt();
  }
  function xe() {
    return Bt(I.querySelectorAll("[data-tippy-root]"));
  }
  function et(T) {
    g.clearDelayTimeouts(), T && K("onTrigger", [g, T]), q();
    var P = G(!0), B = k(), W = B[0], F = B[1];
    me.isTouch && W === "hold" && F && (P = F), P ? s = setTimeout(function() {
      g.show();
    }, P) : g.show();
  }
  function Ce(T) {
    if (g.clearDelayTimeouts(), K("onUntrigger", [g, T]), !g.state.isVisible) {
      pe();
      return;
    }
  ...
  function _t() {
    clearTimeout(s), clearTimeout(o), cancelAnimationFrame(a);
  }
  function xt(T) {
    if (!g.state.isDestroyed) {
      K("onBeforeUpdate", [g, T]), yt();
      var P = g.props, B = Yn(t, Object.assign({}, P, Wn(T), {
        ignoreAttributes: !0
      }));
      g.props = B, Ge(), P.interactiveDebounce !== B.interactiveDebounce && (Ee(), A = Fn(Et, B.interactiveDebounce)), P.triggerTarget && !B.triggerTarget ? Fe(P.triggerTarget).forEach(function(W) {
        W.removeAttribute("aria-expanded");
      }) : B.triggerTarget && t.removeAttribute("aria-expanded"), ie(), H(), S && S(P, B), g.popperInstance && (Lt(), xe().forEach(function(W) {
  ...
  function Rs() {
    var T = g.state.isVisible, P = g.state.isDestroyed, B = !g.state.isEnabled, W = me.isTouch && !g.props.touch, F = qt(g.props.duration, 0, he.duration);
    if (!(T || P || B || W) && !$().hasAttribute("disabled") && (K("onShow", [g], !1), g.props.onShow(g) !== !1)) {
      if (g.state.isVisible = !0, U() && (I.style.visibility = "visible"), H(), q(), g.state.isMounted || (I.style.transition = "none"), U()) {
        var X = O(), re = X.box, He = X.content;
        Xt([re, He], 0);
      }
      m = function() {
        var Pe;
        if (!(!g.state.isVisible || p)) {
          if (p = !0, I.offsetHeight, I.style.transition = g.props.moveTransition, U() && g.props.animation) {
            var Ft = O(), St = Ft.box, Ue = Ft.content;
            Xt([St, Ue], F), zn([St, Ue], "visible");
          }
          ne(), ie(), Kn(Zt, g), (Pe = g.popperInstance) == null || Pe.forceUpdate(), K("onMount", [g]), g.props.animation && U() && ue(F, function() {
            g.state.isShown = !0, K("onShown", [g]);
          });
        }
      }, ve();
    }
  ...
  function Ds() {
    var T = !g.state.isVisible, P = g.state.isDestroyed, B = !g.state.isEnabled, W = qt(g.props.duration, 1, he.duration);
    if (!(T || P || B) && (K("onHide", [g], !1), g.props.onHide(g) !== !1)) {
      if (g.state.isVisible = !1, g.state.isShown = !1, p = !1, i = !1, U() && (I.style.visibility = "hidden"), Ee(), pe(), H(!0), U()) {
        var F = O(), X = F.box, re = F.content;
        g.props.animation && (Xt([X, re], W), zn([X, re], "hidden"));
      }
      ne(), ie(), g.props.animation ? U() && ge(W, g.unmount) : g.unmount();
    }
  }
  function Ms(T) {
  ...
  function Bs() {
    g.state.isVisible && g.hide(), g.state.isMounted && (Tt(), xe().forEach(function(T) {
      ...
      return T !== g;
    }), g.state.isMounted = !1, K("onHidden", [g]));
  }
  function Gs() {
    g.state.isDestroyed || (g.clearDelayTimeouts(), g.unmount(), yt(), delete t._tippy, g.state.isDestroyed = !0, K("onDestroy", [g]));
  }
}
function Le(t, e) {
  e === void 0 && (e = {});
  var n = he.plugins.concat(e.plugins || []);
  Ui();
  var s = Object.assign({}, e, {
    plugins: n
  }), o = Di(t), a = o.reduce(function(i, l) {
    var d = l && qi(l, s);
    return d && i.push(d), i;
  }, []);
  return Ut(t) ? a[0] : a;
...
function Ns(t, e, n = -1) {
  ...
  if (isNaN(t) || isNaN(e) || Math.min(t, e) < s[0] || Math.max(t, e) > s[1])
    return 0;
  let o = 0, a = 0, i = "BoW";
  return t = t > z.blkThrs ? t : t + Math.pow(z.blkThrs - t, z.blkClmp), e = e > z.blkThrs ? e : e + Math.pow(z.blkThrs - e, z.blkClmp), Math.abs(e - t) < z.deltaYmin ? 0 : (e > t ? (o = (Math.pow(e, z.normBG) - Math.pow(t, z.normTXT)) * z.scaleBoW, a = o < z.loClip ? 0 : o - z.loBoWoffset) : (i = "WoB", o = (Math.pow(e, z.revBG) - Math.pow(t, z.revTXT)) * z.scaleWoB, a = o > -0.1 ? 0 : o + z.loWoBoffset), n < 0 ? a * 100 : n == 0 ? Math.round(Math.abs(a) * 100) + "<sub>" + i + "</sub>" : Number.isInteger(n) ? (a * 100).toFixed(n) : 0);
}
function $t(t, e = 2) {
  const n = [
  ...
  let r = 0, c = (t - n[p][r]) * d;
  for (r++; r < a; r++)
    l = n[p][r], l > 400 ? i[r] = l : t < 14.5 ? i[r] = 999 : t < 29.5 ? i[r] = 777 : l > 24 ? i[r] = Math.round(l - s[p][r] * c) : i[r] = l - (2 * s[p][r] * c | 0) * 0.5;
  return i;
}
function ut(t = [0, 0, 0]) {
  function e(n) {
...
function vn(t = [0, 0, 0, 1], e = [0, 0, 0], n = !0) {
  ...
  let s = 1 - t[3], o = [0, 0, 0, 1, !0];
  for (let a = 0; a < 3; a++)
    o[a] = e[a] * s + t[a] * t[3], n && (o[a] = Math.min(Math.round(o[a]), 255));
  return o;
}
function Xn(t) {
...
function fe(t, e) {
  ...
  if (n.startsWith("rgb")) {
    ...
    d.fillStyle = n, d.fillRect(0, 0, 1, 1), [s, o, a, i] = d.getImageData(0, 0, 1, 1).data, i = (i / 255).toFixed(2);
  }
  return e && e < 1 && (i = (i * e).toFixed(2)), [s, o, a, Number(i)];
}
function Rt(t, e) {
  let n = t;
  for (; n && n.nodeType === 1; ) {
    ...
    const s = getComputedStyle(n), o = s.backgroundImage;
    if (o !== "none")
      return { type: "image", value: o };
    const a = fe(s.backgroundColor);
    if (a[3] !== 0 && a !== "transparent") {
      if (a[3] < 1) {
        ...
        const d = fe(l || "rgba(255, 255, 255, 1)");
        return vn(a, d);
      }
      return a;
    }
...
function Gt(t, e) {
  const n = vn(t, e).slice(0, 4), s = ut(n), o = ut(e);
  return { ratio: Ns(s, o), blendedColor: n };
}
function at(t, e) {
...
function J(t) {
  const [e, n, s] = t.map((l) => Math.min(255, Math.max(0, l))), o = e.toString(16).padStart(2, "0"), a = n.toString(16).padStart(2, "0"), i = s.toString(16).padStart(2, "0");
  return `#${o}${a}${i}`;
}
function Ln(t) {
...
function ft(t, e, n) {
  ...
  return { ratio: s, blendedColor: o };
}
function Ji(t, e, n, s) {
  let o;
  s === "AAA" ? o = n ? 4.5 : 7 : o = n ? 3 : 4.5;
  const a = lt(t), i = lt(e), l = a > i ? at(1, i) > o : at(0, i) < o, d = (L, g, _) => _ ? Ss(L, g) : ks(L, g);
  let p = t, r = p, c = at(a, i), m = c, h = t, A = 0.16;
  const b = 0.5, y = 0.01;
  let E = 0;
  const v = 100;
  for (; A >= y; ) {
    ...
    p = d(p, A, l);
    const L = lt(p);
    c = at(L, i), c >= o && (r = c <= m ? p : r, m = c, p = h, A *= b), h = p;
  }
  return { color: J(r) };
}
const Zi = (t, e) => {
  const n = Gt(t, [0, 0, 0, 1]), s = Gt(t, [255, 255, 255, 1]), o = Math.abs(n.ratio) > Math.abs(s.ratio), a = o ? [0, 0, 0, 1] : [255, 255, 255, 1], i = o ? n.ratio : s.ratio, l = $t(i).slice(1), d = Math.ceil(l[Math.floor(e / 100) - 1]);
  return { suggestedColor: a, size: d };
};
function er(t, e, n, s) {
  const a = n == null || s == null, i = ut(e), l = (C, k) => i <= 0.179 ? Ss(C, k) : ks(C, k);
  let d = t, p = Gt(d, e), { ratio: r } = p, c = null, m = r, h = null, A, b, y;
  const E = () => (A = $t(r).slice(1), b = Math.min(
    Math.max(Math.floor(n / 100) - 1, 0),
    A.length - 1
  ), y = A[b], y <= s && y !== 999 && y !== 777), v = () => Math.abs(r) >= 45;
  if (a) {
    if (v())
      return { color: J(t), size: null };
  } else {
    if (c = Zi(e, n), c.size > s)
      return {
        color: J(c.suggestedColor),
        size: c.size
      };
    if (E())
      return { color: J(t), size: null };
  ...
  const x = 50;
  for (; g >= I && S < x; )
    S += 1, d = l(d, g), p = Gt(d, e), r = p.ratio, (a ? v() : E()) && ((Math.abs(r) <= Math.abs(m) || !h) && (h = d, m = r), d = L, g *= _), L = d;
  return h ? { color: J(h), size: null } : !a && c ? {
    color: J(c.suggestedColor),
    size: c.size
  } : { color: J(t), size: null };
}
function tr(t, e, n, s, o, a, i) {
  const { ratio: l, blendedColor: d } = ft(e, n), p = s >= 24 || s >= 18.67 && o >= 700;
  let r;
  if (i === "AAA")
    r = p ? l < 4.5 : l < 7;
  else {
...
function nr(t, e, n, s, o, a, i) {
  const { ratio: l, blendedColor: d } = ft(e, n, i), p = $t(l).slice(1), r = Math.floor(o / 100) - 1, c = p[r];
  return s < c ? {
    $el: t,
    ratio: Ln(l),
    color: d,
...
function Jn(t, e, n, s, o, a, i) {
  return (i === "APCA" ? nr : tr)(t, e, n, s, o, a, i);
}
function sr(t) {
  const { sanitizedText: e, color: n, background: s, fontWeight: o, fontSize: a, ratio: i, textUnderline: l } = t, d = s && s.type !== "image", p = d ? J(s) : "#000000", r = n ? J(n) : "#000000", c = o ? `font-weight:${o};` : "", m = a ? `font-size:${a}px;` : "", h = l ? `text-decoration:${l};` : "", A = n ? "" : 'class="unknown"', b = s && s.type !== "image" ? "" : 'class="unknown"', y = n ? "" : `<span id="fg-input-unknown" class="visually-hidden">(${f._("UNKNOWN")})</span>`, E = s ? "" : `<span id="bg-input-unknown" class="visually-hidden">(${f._("UNKNOWN")})</span>`;
  let v;
...
function ar(t, e) {
  ...
  const { fontSize: s, fontWeight: o, type: a, isLargeText: i } = e, l = t.querySelector("#contrast"), d = t.querySelector("#contrast-preview"), p = t.querySelector("#fg-input"), r = t.querySelector("#bg-input"), c = t.querySelector("#value"), m = t.querySelector("#good"), h = (y, E) => {
    y.forEach((v) => {
    ...
    const y = getComputedStyle(d).fontSize;
    if (y) {
      const E = y.match(/([\d.]+)/);
      if (E) return parseFloat(E[1]);
    }
    ...
    const E = p.value, v = r.value, L = A();
    if (setTimeout(() => {
      const S = p.classList.contains("unknown"), x = r.classList.contains("unknown");
      d.style.color = S ? "" : E, d.style.backgroundColor = x ? "" : v, d.style.backgroundImage = x ? "" : "none";
    }, 0), y?.target && (y.target.classList.remove("unknown"), y.target.parentElement.classList.remove("unknown"), t.querySelector(`#${y.target.id}-unknown`)?.remove()), p.classList.contains("unknown") || r.classList.contains("unknown")) return;
    ...
    if (g === "APCA") {
      ...
      switch (a) {
        ...
        default: {
          const M = x[Math.floor(o / 100) - 1];
          k = L >= M, h(I, k), m.hidden = !k;
          break;
        }
      }
    } else {
      const S = _.ratio;
      c.textContent = Tn(S);
      const x = g === "AAA", C = 3, k = x ? 7 : 4.5, M = x ? 4.5 : 3, U = S >= C, $ = L >= 24 || L >= 18.66 && o >= 700 || i, w = S >= k, O = S >= M;
      switch (a) {
        case "svg-error":
        case "svg-text":
        case "svg-warning": {
        ...
        default: {
          $ ? (h([c, l], O), m.hidden = !O) : (h([c, l], w), m.hidden = !w);
          break;
        }
      }
    }
  };
  p.addEventListener("input", b), r.addEventListener("input", b), setTimeout(() => {
    const y = (E, v) => {
      const L = t.querySelector(E);
      L && L.addEventListener("click", () => {
        v(L.textContent), b(), navigator.clipboard.writeText(L.textContent).catch(() => {
        });
      });
    };
    y("#suggest", (E) => {
...
function ir(t) {
  let e;
  const { color: n, background: s, fontWeight: o, fontSize: a, isLargeText: i, type: l } = t;
  if (n && s && s.type !== "image" && (l === "text" || l === "svg-error" || l === "input")) {
    const d = u.Global.contrastAlgorithm === "APCA" ? er(n, s, o, a) : Ji(
      n,
      s,
      i,
      u.Global.contrastAlgorithm
    ...
    let p;
    const r = '<hr aria-hidden="true">', c = J(t.background), h = `<button id="suggest" class="badge" style="${`color:${d.color};background-color:${c};`}">${d.color}</button>`, A = `<button id="suggest-size" class="normal-badge">${d.size}px</button>`;
    u.Global.contrastAlgorithm === "AA" || u.Global.contrastAlgorithm === "AAA" ? d.color === null ? p = `${r} ${f._("NO_SUGGESTION")}` : p = `${r} ${f._("CONTRAST_COLOR")} ${h}` : d.color && d.size ? p = `${r} ${f._("CONTRAST_APCA")} ${h} ${A}` : d.color ? p = `${r} ${f._("CONTRAST_COLOR")} ${h}` : d.size && (p = `${r} ${f._("CONTRAST_SIZE")} ${A}`), e = document.createElement("div"), e.id = "advice";
    const b = t.opacity < 1 ? `<hr aria-hidden="true"> ${f.sprintf("CONTRAST_OPACITY")}` : p;
    e.innerHTML = b;
  }
  return e;
}
...
function lr(t, e) {
  ...
  if (n) {
    ...
    b.innerHTML = `<h3>${h[s]}</h3> ${o}${A}`, u.Panel.pageIssuesList.insertAdjacentElement("afterbegin", b), u.Panel.pageIssues.classList.add("active"), u.Panel.panel.classList.add("has-page-issues");
  }
}
  ...
  connectedCallback() {
    ...
    const s = Le(ws, {
      ...
      onShow(o) {
        ...
        const a = o.reference.getRootNode().host;
        a.setAttribute("data-sa11y-opened", "");
        const i = o.popper.querySelector(".close-btn"), l = () => {
          o.hide(), o.reference.focus();
        };
        i.addEventListener("click", l);
        const d = (m) => {
          m.key === "Escape" && (o.hide(), o.reference.focus());
        };
        if (o.popper.addEventListener("keydown", d), !o.popper.hasAttribute("contrast-tools-initialized")) {
          const m = parseInt(a.getAttribute("data-sa11y-annotation"), 10), h = window.sa11yCheckComplete.results.find(
            (b) => b.id === m
          ), { contrastDetails: A } = h || {};
          if (A) {
            const b = o.popper.querySelector("[data-sa11y-contrast-details]"), y = sr(A);
            b.appendChild(y), ar(o.popper, A);
            const E = ir(A);
            E && b.appendChild(E), o.popper.setAttribute("contrast-tools-initialized", !0);
          }
        }
        let p = !0;
        function r(m) {
          p && m.target.matches('input[type="color"]') && (o.reference.click(), p = !1, o.popper.removeEventListener("mousedown", r));
        }
        o.popper.addEventListener("mousedown", r);
        const c = () => {
          i.removeEventListener("click", l), o.popper.removeEventListener("keydown", d), o.popper.removeEventListener("hidden", c);
        };
        o.popper.addEventListener("hidden", c);
      },
      onTrigger(o, a) {
        a.type === "click" && setTimeout(() => {
          o.popper.querySelector(".close-btn").focus(), Ys(o.popper);
        }, 0);
      },
      onHide(o) {
        o.popper.querySelector(".close-btn").removeEventListener("click", () => {
          o.hide();
        }), o.reference.getRootNode().host.removeAttribute("data-sa11y-opened");
      }
    });
  }
}
class es extends HTMLElement {
  connectedCallback() {
    ...
    s.innerHTML = xs + mt, n.appendChild(s);
    const o = navigator.userAgent.indexOf("Mac") !== -1 ? '<span class="kbd">Option</span> + <span class="kbd">S</span>' : '<span class="kbd">Alt</span> + <span class="kbd">S</span>';
    if (Le(u.Panel.skipButton, {
      ...e(n),
      offset: [0, 8],
      maxWidth: 200,
      content: `${f._("SKIP_TO_ISSUE")} &raquo; <br> ${o}`
...
const pr = () => {
  ...
  if (t.offsetTop === 0) {
    const s = t.getRootNode().host, o = ht(s, "display", "none"), a = parseInt(s.getAttribute("data-sa11y-annotation"), 10), i = e.find((p) => p.id === a), l = as(i);
    ur(t);
    const d = t.getAttribute("data-tippy-content");
    if (je(`${f._("NOT_VISIBLE")}`, d, l), pr(), o) {
      const p = o.previousElementSibling, { parentNode: r } = o;
      return p ? Vt(p).top - 150 : Vt(r).top - 150;
    }
  } else
...
const Ps = () => {
  ...
  t !== null && (oe = parseInt(t, 10) - 1), R.removeItem("sa11y-latest-dismissed");
  const e = Z("[data-sa11y-opened]", "root");
  e[0] && (oe = parseInt(e[0].getAttribute("data-sa11y-position"), 10));
}, Os = (t) => {
  Ps();
  const e = D.Annotations.Array;
  oe >= e.length - 1 && (oe = -1);
  const s = e[oe + 1].shadowRoot.querySelector("button"), o = Cs(s, t);
  window.scrollTo({
    top: o,
    behavior: `${u.Global.scrollBehaviour}`
  ...
  const e = D.Annotations.Array;
  oe <= 0 && (oe = e.length);
  const n = D.Annotations.Array[oe - 1].shadowRoot.querySelector("button"), s = Cs(n, t);
  window.scrollTo({
    top: s,
    behavior: `${u.Global.scrollBehaviour}`
  }), n.offsetTop !== 0 && (n.focus(), n.click()), oe -= 1, oe === -1 && (oe = D.Annotations.Array.length - 1);
};
function hr(t, e) {
  D.Annotations.Array.length && !u.Panel.skipButton.hasAttribute("disabled") && (t.altKey && (t.code === "KeyS" || t.code === "Period") ? (t.preventDefault(), Os(e)) : t.altKey && (t.code === "KeyW" || t.code === "Comma") && (t.preventDefault(), fr(e)));
}
let cn, dn;
function gr(t) {
  cn = (e) => {
...
function br(t, e) {
  const n = e.susAltStopWords ? e.susAltStopWords.split(",").map((l) => l.trim().toLowerCase()).filter(Boolean) : f._("SUS_ALT_STOPWORDS"), s = new Set(f._("PLACEHOLDER_ALT_STOPWORDS")), o = Ke(e.altPlaceholder, !0), a = e.extraPlaceholderStopWords.split(",").map((l) => l.trim().toLowerCase()).filter(Boolean), i = (l) => {
    const d = l.toLowerCase(), p = d.replace(/\d+/g, "").trim(), r = [null, null, null];
    for (const c of Ar)
      if (d.includes(c)) {
        r[0] = c;
        break;
    ...
    if ((s.has(d) || s.has(p)) && (r[2] = l), a.length)
      for (const c of a) {
        const m = d.indexOf(c);
        if (m > -1 && m < 6) {
          r[2] = c;
  ...
  return D.Found.Images.forEach((l) => {
    const d = ke(l) === "noAria" ? l.getAttribute("alt") : ke(l);
    if (l.height < 2 && l.width < 2 && (De(l) || d === ""))
      return;
    const p = l.closest(
      e.imageWithinLightbox ? `a[href]:not(${e.imageWithinLightbox})` : "a[href]"
    ), r = l.getAttribute("src") ? l.getAttribute("src") : l.getAttribute("srcset"), c = p ? qe(p, u.Exclusions.LinkSpan).textContent : "", m = Array.isArray(e.linkIgnoreStrings) ? e.linkIgnoreStrings.reduce(
      (w, O) => w.replace(O, ""),
      c
    ) : c;
    if (p && p.getAttribute("aria-hidden") === "true") {
      ...
      e.checks.HIDDEN_FOCUSABLE && !w && t.push({
        ...
        dismiss: N(`IMGHIDDENFOCUSABLE${r}`),
        dismissAll: e.checks.HIDDEN_FOCUSABLE.dismissAll ? "LINK_HIDDEN_FOCUSABLE" : !1,
        developer: e.checks.HIDDEN_FOCUSABLE.developer || !0
      });
      return;
    }
    const h = p ? Ae(m).length : 0;
    if (d === null) {
      if (p) {
        const w = h === 0 ? e.checks.MISSING_ALT_LINK : e.checks.MISSING_ALT_LINK_HAS_TEXT, O = h === 0 ? "MISSING_ALT_LINK" : "MISSING_ALT_LINK_HAS_TEXT";
        w && t.push({
          test: O,
          element: l,
          type: w.type || "error",
          content: f.sprintf(w.content || O),
          dismiss: N(`${O + r + h}`),
        ...
        dismiss: N(`IMGNOALT${r}`),
        dismissAll: e.checks.MISSING_ALT.dismissAll ? "MISSING_ALT" : !1,
        developer: e.checks.MISSING_ALT.developer || !1
      });
      return;
    }
    const A = Y(d), b = Ae(A), y = l.getAttribute("aria-label") || l.getAttribute("aria-labelledby");
    if (e.checks.MISSING_ALT && y && b === "") {
      t.push({
        test: "MISSING_ALT",
        element: l,
        type: e.checks.MISSING_ALT.type || "error",
        content: f.sprintf(e.checks.MISSING_ALT.content || "MISSING_ALT"),
        dismiss: N(`IMGNOALTARIA${r}`),
        dismissAll: e.checks.MISSING_ALT.dismissAll ? "MISSING_ALT" : !1,
        developer: e.checks.MISSING_ALT.developer || !1
      });
      return;
    }
    const E = d === "", v = l.closest("figure"), L = v?.querySelector("figcaption"), g = L ? Q(L) : "", _ = e.checks.LINK_IMAGE_LONG_ALT.maxLength || 250, I = e.checks.IMAGE_ALT_TOO_LONG.maxLength || 250, S = d.match(o)?.[0];
    if (E || S) {
      const w = e.checks.IMAGE_DECORATIVE_CAROUSEL.sources, O = w ? l.closest(w) : "";
      if (O) {
        const G = O.querySelectorAll("img"), H = G.length === 1 ? e.checks.IMAGE_DECORATIVE : e.checks.IMAGE_DECORATIVE_CAROUSEL, K = G.length === 1 ? "IMAGE_DECORATIVE" : "IMAGE_DECORATIVE_CAROUSEL";
        H && t.push({
          test: K,
          element: l,
          type: H.type || "warning",
        ...
        const G = h === 0 ? e.checks.LINK_IMAGE_NO_ALT_TEXT : e.checks.LINK_IMAGE_TEXT, H = h === 0 ? "LINK_IMAGE_NO_ALT_TEXT" : "LINK_IMAGE_TEXT";
        G && t.push({
          test: H,
          element: l,
          type: G.type || (h === 0 ? "error" : "good"),
          content: f.sprintf(G.content || H),
          dismiss: N(`${H + r + h}`),
          dismissAll: G.dismissAll ? H : !1,
          developer: G.developer || !1
        });
      } else if (v) {
        const G = L && g.length ? e.checks.IMAGE_FIGURE_DECORATIVE : e.checks.IMAGE_DECORATIVE, H = L && g.length ? "IMAGE_FIGURE_DECORATIVE" : "IMAGE_DECORATIVE";
        G && t.push({
          test: H,
          element: l,
          type: G.type || "warning",
        ...
        dismiss: N(`DECIMAGE${r}`),
        dismissAll: e.checks.IMAGE_DECORATIVE.dismissAll ? "IMAGE_DECORATIVE" : !1,
        developer: e.checks.IMAGE_DECORATIVE.developer || !1
      });
      return;
    }
    const x = p ? e.checks.LINK_ALT_UNPRONOUNCEABLE : e.checks.ALT_UNPRONOUNCEABLE;
    if (x && d.replace(/"|'|\?|\.|-|\s+/g, "") === "" && h === 0) {
      const w = p ? "LINK_ALT_UNPRONOUNCEABLE" : "ALT_UNPRONOUNCEABLE";
      t.push({
        test: w,
        element: l,
        type: x.type || "error",
        content: f.sprintf(x.content || w, b),
        dismiss: N(`UNPRONOUNCEABLE${r}`),
        dismissAll: x.dismissAll ? "ALT_UNPRONOUNCEABLE" : !1,
        developer: x.developer || !1
      });
      return;
    }
    const C = i(b), k = p ? e.checks.LINK_ALT_MAYBE_BAD : e.checks.ALT_MAYBE_BAD, M = new RegExp(`^\\S{${k.minLength || 15},}$`), U = /[^\p{L}\-,.!?]/u.test(d);
    if (C[0] !== null) {
      const w = p ? e.checks.LINK_ALT_FILE_EXT : e.checks.ALT_FILE_EXT, O = p ? "LINK_ALT_FILE_EXT" : "ALT_FILE_EXT";
      w && t.push({
        test: O,
        element: l,
        type: w.type || "error",
        content: f.sprintf(w.content || O, C[0], b),
        dismiss: N(`${O + r + b}`),
        dismissAll: w.dismissAll ? O : !1,
        developer: w.developer || !1
      });
    } else if (C[2] !== null) {
      const w = p ? e.checks.LINK_PLACEHOLDER_ALT : e.checks.ALT_PLACEHOLDER, O = p ? "LINK_PLACEHOLDER_ALT" : "ALT_PLACEHOLDER";
      w && t.push({
        test: O,
        element: l,
        type: w.type || "error",
        content: f.sprintf(w.content || O, b),
        dismiss: N(`${O + r + b}`),
        dismissAll: w.dismissAll ? O : !1,
        developer: w.developer || !1
      });
    } else if (C[1] !== null) {
      const w = p ? e.checks.LINK_SUS_ALT : e.checks.SUS_ALT, O = p ? "LINK_SUS_ALT" : "SUS_ALT";
      w && t.push({
        test: O,
        element: l,
        type: w.type || "warning",
        content: f.sprintf(w.content || O, C[1], b),
        dismiss: N(`${O + r + b}`),
        dismissAll: w.dismissAll ? O : !1,
        developer: w.developer || !1
      });
    } else if (k && M.test(d) && U) {
    ...
    } else if (p ? d.length > _ : d.length > I) {
      const w = p ? e.checks.LINK_IMAGE_LONG_ALT : e.checks.IMAGE_ALT_TOO_LONG, O = p ? "LINK_IMAGE_LONG_ALT" : "IMAGE_ALT_TOO_LONG", G = Se(b, 600);
      w && t.push({
        test: O,
        element: l,
        type: w.type || "warning",
        content: f.sprintf(w.content || O, d.length, G),
        dismiss: N(`${O + r + b}`),
        dismissAll: w.dismissAll ? O : !1,
        developer: w.developer || !1
      });
    } else if (p) {
      const w = h === 0 ? e.checks.LINK_IMAGE_ALT : e.checks.LINK_IMAGE_ALT_AND_TEXT, O = h === 0 ? "LINK_IMAGE_ALT" : "LINK_IMAGE_ALT_AND_TEXT";
      if (w) {
        const G = Te(p), H = Ae(G), K = Y(H), ne = h === 0 ? f.sprintf("LINK_IMAGE_ALT", b) : `${f.sprintf("LINK_IMAGE_ALT_AND_TEXT", b, K)} ${f.sprintf("ACC_NAME_TIP")}`;
        t.push({
          test: O,
          element: l,
          type: w.type || "warning",
          content: w.content ? f.sprintf(w.content, b, K) : ne,
          dismiss: N(`${O + r + b}`),
          dismissAll: w.dismissAll ? O : !1,
          developer: w.developer || !1
        });
      }
    } else v ? !!L && g.toLowerCase() === b.toLowerCase() ? e.checks.IMAGE_FIGURE_DUPLICATE_ALT && t.push({
      ...
      dismiss: N(`FIGIMGPASS${r + b}`),
      dismissAll: e.checks.IMAGE_PASS.dismissAll ? "IMAGE_PASS" : !1,
      developer: e.checks.IMAGE_PASS.developer || !1
    }) : e.checks.IMAGE_PASS && (l.closest('button, [role="button"]') || t.push({
      test: "IMAGE_PASS",
      element: l,
      type: e.checks.IMAGE_PASS.type || "good",
      content: f.sprintf(e.checks.IMAGE_PASS.content || "IMAGE_PASS", b),
      dismiss: N(`IMAGEPASS${r + b}`),
      dismissAll: e.checks.IMAGE_PASS.dismissAll ? "IMAGE_PASS" : !1,
      developer: e.checks.IMAGE_PASS.developer || !1
    }));
    l.getAttribute("title")?.toLowerCase() === d.toLowerCase() && e.checks.DUPLICATE_TITLE && t.push({
...
function yr(t, e, n) {
  let s, o = "";
  const a = Ke(e.headerIgnoreStrings);
  return D.Found.Headings.forEach((i, l) => {
    const p = Te(i, u.Exclusions.HeaderSpan).replace(a, ""), r = Ae(p), c = Y(r), m = u.Root.areaToCheck.some((C) => C.contains(i)), h = u.Root.areaToCheck.some(
      (C) => C.contains(i.getRootNode().host)
    ), A = m || h, b = D.Found.HeadingOverrideStart.get(i);
    b && (s = b);
    const y = parseInt(i.getAttribute("aria-level") || i.tagName.slice(1), 10), E = r.length, v = e.checks.HEADING_LONG.maxLength || 160;
    ...
    _ && g && t.push({
      ...
      developer: I,
      margin: x
    }), s = y, o = c, D.Found.OutlineIgnore.includes(i) || n.push({
      element: i,
      headingLevel: y,
      text: c,
      type: g,
...
const Er = [
  ...
  "avi",
  "zip"
], vr = 'a[href$=".pdf"], a[href$=".doc"], a[href$=".docx"], a[href$=".zip"], a[href$=".mp3"], a[href$=".txt"], a[href$=".exe"], a[href$=".dmg"], a[href$=".rtf"], a[href$=".pptx"], a[href$=".ppt"], a[href$=".xls"], a[href$=".xlsx"], a[href$=".csv"], a[href$=".mp4"], a[href$=".mov"], a[href$=".avi"]', Lr = /(doi\.org\/|dl\.acm\.org\/|link\.springer\.com\/|pubmed\.ncbi\.nlm\.nih\.gov\/|scholar\.google\.com\/|ieeexplore\.ieee\.org\/|researchgate\.net\/publication\/|sciencedirect\.com\/science\/article\/)[a-z0-9/.-]+/i, Tr = /\b(?:\.edu\/|\.gob\/|\.gov\/|\.app\/|\.com\/|\.net\/|\.org\/|\.us\/|\.ca\/|\.de\/|\.icu\/|\.uk\/|\.ru\/|\.info\/|\.top\/|\.xyz\/|\.tk\/|\.cn\/|\.ga\/|\.cf\/|\.nl\/|\.io\/|\.fr\/|\.pe\/|\.nz\/|\.pt\/|\.es\/|\.pl\/|\.ua\/)\b/i, Ir = /[^a-zA-Z0-9]/g, _r = /([<>↣↳←→↓«»↴]+)/, en = (t, e) => e.has(t) ? t : null;
function xr(t, e) {
  const n = e.linkStopWords ? e.linkStopWords.split(",").map((r) => r.toLowerCase().trim()) : [], s = /* @__PURE__ */ new Set([...f._("PARTIAL_ALT_STOPWORDS"), ...n]), o = new Set(e.linkIgnoreStrings.map((r) => r.toLowerCase())), a = Ke(f._("CLICK")), i = Ke(f._("NEW_WINDOW_PHRASES")), l = Ke(Er), d = Ke(e.linkIgnoreStrings), p = {};
  return D.Found.Links.forEach((r) => {
    const c = qs(r), m = r.getAttribute("title"), h = r.getAttribute("aria-hidden") === "true", A = r.getAttribute("tabindex") === "-1", b = r.getAttribute("target")?.toLowerCase() === "_blank", y = r.getAttribute("aria-label"), E = r.getAttribute("aria-labelledby"), v = E ? null : r.querySelector("[aria-labelledby]"), L = E || v, g = L || y || r.querySelector("[aria-label]"), _ = Ae(
      Te(r, u.Exclusions.LinkSpan)
    ), I = _.replace(d, ""), S = I.toLowerCase(), x = Wt(S), C = Q(r).toLowerCase(), k = S.match(i)?.[0] || C.match(i)?.[0], M = S.match(l)?.[0] || C.match(l)?.[0], U = r.matches(vr);
    if (!r.querySelector("img")) {
      ...
      if (g && I.length !== 0) {
        const V = Y(I), j = qe(r, u.Exclusions.LinkSpan), q = Q(j).replace(d, ""), pe = Wt(q), ge = en(pe, s), ue = is(
          r,
          _,
        ...
        e.checks.LINK_STOPWORD_ARIA && ge !== null ? t.push({
          ...
          content: e.checks.LINK_STOPWORD_ARIA.content ? f.sprintf(e.checks.LINK_STOPWORD_ARIA.content, ge, V) : f.sprintf("LINK_STOPWORD_ARIA", ge, V) + f.sprintf("LINK_TIP"),
          inline: !0,
          dismiss: N(`LINKSTOPWORDARIA${c + x}`),
          dismissAll: e.checks.LINK_STOPWORD_ARIA.dismissAll ? " LINK_STOPWORD_ARIA" : !1,
          developer: e.checks.LINK_STOPWORD_ARIA.developer || !0
        }) : e.checks.LABEL_IN_NAME && ue && C.length !== 0 ? t.push({
          test: "LABEL_IN_NAME",
          element: r,
          ...
          content: e.checks.LINK_LABEL.content ? f.sprintf(e.checks.LINK_LABEL.content, V) : `${f.sprintf("ACC_NAME", V)} ${f.sprintf("ACC_NAME_TIP")}`,
          inline: !0,
          position: "afterend",
          dismiss: N(`LINKGOOD${c + x}`),
      ...
      let $;
      const w = (V, j) => {
        e.checks.LINK_STOPWORD && !$ && ($ = !0, t.push({
          test: "LINK_STOPWORD",
          element: V,
          type: e.checks.LINK_STOPWORD.type || "error",
          content: e.checks.LINK_STOPWORD.content ? f.sprintf(e.checks.LINK_STOPWORD.content, j) : f.sprintf("LINK_STOPWORD", j) + f.sprintf("LINK_TIP"),
          inline: !0,
          position: "afterend",
          dismiss: N(`LINKSTOPWORD${c + x}`),
      ...
      if (I.length === 0) {
        if (L)
          ...
          let V = !1;
          if (e.linkIgnoreSpan) {
            const j = r.querySelector(e.linkIgnoreSpan);
            if (j) {
              const q = Wt(j.textContent).trim().toLowerCase();
              q === C && (w(r, q), V = !0);
            }
          }
          !V && e.checks.LINK_EMPTY_NO_LABEL && t.push({
        ...
        } else !O && e.checks.LINK_EMPTY && t.push({
          test: "LINK_EMPTY",
          element: r,
      ...
      const G = en(x, s), H = x.match(a)?.[0] || C.match(a)?.[0], K = S.match(Lr)?.[0], ie = S.startsWith("www.") || S.startsWith("http") ? "URL Prefix" : S.match(Tr)?.[0], Ee = I.length === 1 && Ir.test(I), se = S.match(_r)?.[0];
      if (G)
        w(r, G);
      else if (K)
      ...
      H && e.checks.LINK_CLICK_HERE && t.push({
        ...
        content: f.sprintf(e.checks.DUPLICATE_TITLE.content || "DUPLICATE_TITLE"),
        inline: !0,
        dismiss: N(`LINKDUPLICATETITLE${c + x}`),
        dismissAll: e.checks.DUPLICATE_TITLE.dismissAll ? "DUPLICATE_TITLE" : !1,
        developer: e.checks.DUPLICATE_TITLE.developer || !1
      });
    }
    if (x.length !== 0) {
      if (p[x] && !p[c]) {
        const $ = r.ariaHidden === "true" && r.getAttribute("tabindex") === "-1", w = r.hasAttribute("role") || r.hasAttribute("disabled");
        if (e.checks.LINK_IDENTICAL_NAME && !w && !$) {
          const O = Y(I);
          t.push({
            test: "LINK_IDENTICAL_NAME",
            element: r,
            type: e.checks.LINK_IDENTICAL_NAME.type || "warning",
            content: e.checks.LINK_IDENTICAL_NAME.content ? f.sprintf(e.checks.LINK_IDENTICAL_NAME.content, O) : `${f.sprintf("LINK_IDENTICAL_NAME", O)} ${f.sprintf("ACC_NAME_TIP")}`,
            inline: !0,
            dismiss: N(`LINKSEEN${c + x}`),
...
function Nr(t, e) {
  const n = [];
  for (let a = 0; a < D.Found.Contrast.length; a++) {
    const i = D.Found.Contrast[a], l = getComputedStyle(i), d = parseFloat(l.opacity), p = fe(l.color, d), r = parseFloat(l.fontSize), c = l.fontWeight, m = Xn(c), h = Rt(i, u.Global.shadowDetection), A = Ks(i), y = De(i) || A || d === 0 || r === 0, v = Array.from(i.childNodes).filter((g) => g.nodeType === 3).map((g) => g.textContent).join("").trim(), L = ["SELECT", "INPUT", "TEXTAREA"].includes(i.tagName);
    if (v.length !== 0 || L) {
      const g = r >= 24 || r >= 18.67 && m >= 700;
  ...
  return D.Found.Svg.forEach((a) => {
    const i = { $el: a, type: "svg-warning" }, l = Rt(a, u.Global.shadowDetection), d = l !== "unsupported" && l.type !== "image", p = a.querySelectorAll("path, rect, circle, ellipse, polygon, text, use"), r = a.querySelectorAll(
      "*:not(path):not(rect):not(circle):not(ellipse):not(polygon):not(text):not(use):not(title)"
    );
    let c = !1;
    if (p.length) {
      const m = getComputedStyle(p[0]);
      c = Array.from(p).every((h) => {
    ...
    if ((p.length === 1 || c) && r.length === 0) {
      const m = getComputedStyle(p[0]), { fill: h, stroke: A, strokeWidth: b, opacity: y } = m;
      let E = 0;
      const { width: v, height: L } = a.getBBox();
      A && A !== "none" && (b.endsWith("%") ? E = parseFloat(b) / 100 * Math.min(v, L) : E = ["inherit", "initial", "unset"].includes(b) ? 1 : parseFloat(b));
      const g = Math.min(v, L) < 50 ? 1 : 3, _ = A && E >= g && A !== "none", I = h && h !== "none" && !h.startsWith("url("), S = fe(h === "currentColor" ? getComputedStyle(p[0]).color : h, y), x = fe(A === "currentColor" ? getComputedStyle(p[0]).color : A, y);
      if (![S, x].includes("unsupported") && d) {
        let k, M = !1, U = !1;
        if (I && (k = ft(
          S,
          l,
          e.contrastAlgorithm
        ), M = e.contrastAlgorithm === "APCA" ? k.ratio >= 45 : k.ratio >= 3), _ && (k = ft(
          x,
          l,
          e.contrastAlgorithm
        ), U = e.contrastAlgorithm === "APCA" ? k.ratio >= 45 : k.ratio >= 3), I && _ && !M && !U || I && !_ && !M || !I && _ && !U) {
          const G = J(l), H = J(S), K = J(x);
          if (H === G && !_ || K === G && !I)
            return;
          n.push({
            $el: a,
        ...
        I && S !== "unsupported" ? i.color = S : _ && x !== "unsupported" && (i.color = x), d && (i.background = l), n.push(i);
    } else
      d && (i.background = l), n.push(i);
  }), D.Found.Inputs.forEach((a) => {
    if (a.placeholder && a.placeholder.length !== 0) {
      const i = getComputedStyle(a, "::placeholder"), l = fe(i.getPropertyValue("color")), d = parseFloat(i.fontSize), p = Xn(i.fontWeight), r = Rt(a, u.Global.shadowDetection), c = parseFloat(i.opacity);
      if (l === "unsupported")
    ...
    if (e.contrastAlgorithm === "APCA")
      ...
      const p = i.reduce((r, c) => {
        const m = r, h = JSON.stringify({
          background: c.background.value,
          color: c.color,
          isLargeText: c.isLargeText
        });
        return m[h] || (m[h] = []), m[h].push(c), m;
      }, {});
      d = Object.values(p).map((r) => ({
        ...r[0]
      }));
    }
    return [...d, ...l];
  })(n).forEach((a) => {
    const { $el: i, ratio: l } = a, d = a, p = i.tagName === "OPTION" ? i.closest("datalist, select, optgroup") : i, r = qe(p, ["option:not(option:first-child)"]), c = Q(r), m = Se(c, 80), h = Y(m);
    let A;
    a.type === "placeholder" || a.type === "placeholder-unsupported" ? A = Y(i.placeholder) : a.type === "svg-error" || a.type === "svg-warning" ? A = "" : A = h, d.sanitizedText = A;
    let b = "";
    if (e.contrastAlgorithm === "AA" || e.contrastAlgorithm === "AAA") {
      const E = e.contrastAlgorithm === "AAA" ? "7:1" : "4.5:1", v = e.contrastAlgorithm === "AAA" ? "4.5:1" : "3:1", L = a.isLargeText ? v : E, g = a.isLargeText ? "CONTRAST_LARGE" : "CONTRAST_NORMAL";
      b = ` ${f.sprintf(g, L)}`;
    }
    ...
    switch (a.type) {
      case "text":
        e.checks.CONTRAST_ERROR && t.push({
          ...
          content: e.checks.CONTRAST_ERROR.content ? f.sprintf(e.checks.CONTRAST_ERROR.content) : f.sprintf("CONTRAST_ERROR") + b,
      ...
      case "input":
        if (e.checks.CONTRAST_INPUT) {
          const E = Ne(i.outerHTML);
          t.push({
            test: "CONTRAST_INPUT",
            element: p,
            type: e.checks.CONTRAST_INPUT.type || "error",
            content: e.checks.CONTRAST_INPUT.content ? f.sprintf(e.checks.CONTRAST_INPUT.content) : f.sprintf("CONTRAST_INPUT", l) + b,
      ...
      case "placeholder":
        if (e.checks.CONTRAST_PLACEHOLDER) {
          ...
          t.push({
            ...
            content: e.checks.CONTRAST_PLACEHOLDER.content ? f.sprintf(e.checks.CONTRAST_PLACEHOLDER.content) : f.sprintf("CONTRAST_PLACEHOLDER") + b,
            position: "afterend",
      ...
      case "placeholder-unsupported":
        if (e.checks.CONTRAST_PLACEHOLDER_UNSUPPORTED) {
          ...
          t.push({
            ...
            content: e.checks.CONTRAST_PLACEHOLDER_UNSUPPORTED.content ? f.sprintf(e.checks.CONTRAST_PLACEHOLDER_UNSUPPORTED.content) : f.sprintf("CONTRAST_PLACEHOLDER_UNSUPPORTED") + b,
            position: "afterend",
      ...
      case "background-image":
        e.checks.CONTRAST_WARNING && t.push({
          ...
          content: e.checks.CONTRAST_WARNING.content ? f.sprintf(e.checks.CONTRAST_WARNING.content) : f.sprintf("CONTRAST_WARNING") + b,
          dismiss: N(`CONTRAST${h}`),
      ...
      case "unsupported":
        e.checks.CONTRAST_UNSUPPORTED && t.push({
          ...
          content: e.checks.CONTRAST_UNSUPPORTED.content ? f.sprintf(e.checks.CONTRAST_UNSUPPORTED.content) : f.sprintf("CONTRAST_WARNING") + b,
          dismiss: N(`CONTRAST${h}`),
...
function Sr(t, e) {
  return e.formLabelsPlugin && D.Found.Inputs.forEach((n) => {
    const s = n.getAttribute("aria-hidden") === "true", o = n.getAttribute("tabindex") === "-1";
    if (De(n) || s && o)
      return;
    const i = Te(n), l = Ae(i), d = n.getAttribute("alt"), p = n.getAttribute("type"), r = n.getAttribute("title"), c = n.placeholder && n.placeholder !== 0, m = n.getAttribute("aria-label") || n.getAttribute("aria-labelledby");
    if (p === "submit" || p === "button" || p === "hidden")
      return;
    if (p === "image") {
      e.checks.LABELS_MISSING_IMAGE_INPUT && (!d || d.trim() === "") && !m && !r && t.push({
    ...
    const h = n.closest("label"), A = h ? Ae(Te(h)) : "";
    if (h && A.length)
      return;
    const b = n.getAttribute("id");
    b ? D.Found.Labels.some((y) => y.getAttribute("for") === b) || e.checks.LABELS_NO_FOR_ATTRIBUTE && t.push({
...
`, ""), L.length <= 3)
    ...
    let h = !1;
    e === "en" ? h = 206.835 - 1.015 * (l / p) - 84.6 * (r / l) : e === "fr" ? h = 207 - 1.015 * (l / p) - 73.6 * (r / l) : e === "es" ? h = 206.84 - 1.02 * (l / p) - 0.6 * (100 * (r / l)) : e === "de" ? h = 180 - l / p - 58.5 * (r / l) : e === "nl" ? h = 206.84 - 0.77 * (100 * (r / l)) - 0.93 * (l / p) : e === "it" ? h = 217 - 1.3 * (l / p) - 0.6 * (100 * (r / l)) : e === "pt" && (h = 248.835 - 1.015 * (l / p) - 84.6 * (r / l)), h > 100 ? h = 100 : h < 0 && (h = 0);
    const A = Number(h.toFixed(1)), b = Number((l / p).toFixed(1)), y = Math.round(100 * ((l - (c + m)) / l));
    let E;
  ...
  if (["sv", "fi", "da", "no", "nb", "nn"].includes(e)) {
    const a = () => o.replace(/[-'.]/gi, "").split(/[^a-zA-ZöäåÖÄÅÆæØø0-9]/g).filter(Boolean), i = () => {
      const b = /\?|!|\.|\n/g;
      return o.split(b).filter(Boolean);
    }, l = a(), d = l.length, p = l.filter((b) => b.length > 6).length, r = i().length || 1, c = Math.round(d / r + p * 100 / d), m = Number((d / r).toFixed(1)), h = Math.round(100 * (p / d));
    let A;
...
function Cr(t, e) {
  ...
  return e.checks.EMBED_AUDIO && D.Found.Audio.forEach((s) => {
    ...
    const o = s.querySelector("track"), a = o?.getAttribute("src");
    (o === null || a === null || a.trim().length === 0) && t.push({
      test: "EMBED_VIDEO",
      element: s,
    ...
    const o = ["presentation", "none"].includes(s.getAttribute("role")), a = De(s), i = s.tagName === "VIDEO" || s.tagName === "AUDIO", l = s.getAttribute("aria-hidden") === "true", d = s.getAttribute("tabindex") === "-1";
    if (!(a || i || l && d || o)) {
      if (d) {
      ...
      if (e.checks.EMBED_MISSING_TITLE) {
        const p = ke(s), r = p === "noAria" ? s.getAttribute("title") || "" : p;
        Ae(r).length === 0 && t.push({
    ...
    const o = ["presentation", "none"].includes(s.getAttribute("role")), a = s.getAttribute("aria-hidden") === "true", i = s.getAttribute("tabindex") === "-1";
    De(s) || a && i || o || s.tagName === "VIDEO" || s.tagName === "AUDIO" || t.push({
      test: "EMBED_GENERAL",
      element: s,
...
function Pr(t, e) {
  if (e.checks.QA_BAD_LINK && D.Found.CustomErrorLinks.forEach((i) => {
    t.push({
      test: "QA_BAD_LINK",
      element: i,
  ...
  }), e.checks.QA_STRONG_ITALICS && D.Found.StrongItalics.forEach((i) => {
    const l = Q(i);
    ...
    if (i.hasAttribute("href")) {
      const l = i.getAttribute("href"), d = i.matches(u.Global.documentSources), p = i.matches('a[href$=".pdf"], a[href*=".pdf?"]');
      if (e.checks.QA_IN_PAGE_LINK) {
        const r = Q(i).length !== 0, c = i.ariaHidden === "true" && i.getAttribute("tabindex") === "-1", m = i.hasAttribute("role") || i.hasAttribute("aria-haspopup") || i.hasAttribute("aria-expanded") || i.hasAttribute("onclick") || i.hasAttribute("disabled") || i.closest('nav, [role="navigation"]');
        if ((l.startsWith("#") || l === "") && r && !c && !m) {
  ...
  }), e.checks.QA_BLOCKQUOTE && D.Found.Blockquotes.forEach((i) => {
    const l = Q(i);
    ...
    if (De(i) === !1) {
      const l = i.querySelectorAll("th"), d = i.querySelectorAll("h1, h2, h3, h4, h5, h6"), p = N(`TABLE${i.textContent}`);
      ...
      }), e.checks.TABLES_SEMANTIC_HEADING && d.length > 0 && d.forEach((r) => {
        t.push({
    ...
    const i = (c, m) => {
      ...
      const m = getComputedStyle(c).fontSize.replace("px", ""), h = Q(c), A = h.match(/[.;?!"]/) === null, b = h.length >= 4 && h.length <= 120;
      if (m >= 24 && !c.closest(d) && b && A && !l(c)) {
        const y = Y(h);
        i(c, y);
      }
    }, r = (c) => {
      if (/^<\s*(strong|b)(\s+[^>]*)?>/i.test(c.innerHTML.trim()) && !c.closest(d)) {
        const h = c.querySelector("strong, b"), A = Q(h), b = A.match(/[.:;?!"']/) === null;
        if (A.length >= 3 && A.length <= 120 && b) {
          const E = qe(c, ["strong", "b"]).textContent.trim().length;
          if (E !== 0 && E <= 250)
            return;
          const v = Y(A);
          i(h, v);
  ...
  if (e.checks.QA_FAKE_LIST) {
    const i = new RegExp(/(([023456789][\d\s])|(1\d))/, ""), l = new RegExp(/(^[aA1αаΑ]|[^p{Alphabetic}\s])[-\s.)]/, "u"), d = new RegExp(/\p{Extended_Pictographic}/, "u"), p = ["a", "A", "α", "Α", "а", "А", "1"], r = /[([{#]/, c = {
      2: "1",
      b: "a",
      B: "A",
      β: "α",
      ...
      б: "а",
      Б: "А"
    }, m = (y) => y.replace(/^b|^B|^б|^Б|^β|^В|^2/, (E) => c[E]);
    let h = "", A = "", b = !1;
    D.Found.Paragraphs.forEach((y, E) => {
      let v = !1, L = !1;
      A = A || Q(y).replace("(", "");
      const g = A.substring(0, 2), _ = g.match(l), I = g.match(i), S = g.match(d), x = r.test(g.charAt(0));
      if (g.length > 0 && g !== h && !I && (_ || S || x)) {
        const C = D.Found.Paragraphs[E + 1];
        if (C) {
          if (v = Q(C).replace("(", "").substring(0, 2), p.includes(v?.toLowerCase().trim()))
            return;
          const k = m(v);
          _ ? g !== "A " && g === k && (L = !0) : S && !b && k.match(d) && (L = !0, b = !0);
        }
        if (!L) {
          let k = y?.querySelector("br")?.nextSibling?.nodeValue;
          k && (k = k.replace(/<\/?[^>]+(>|$)/g, "").trim().substring(0, 2), (r.test(k.charAt(0)) || g === m(k) || !b && k.match(d)) && (L = !0));
        }
        L ? (t.push({
          test: "QA_FAKE_LIST",
          element: y,
  ...
  if (e.checks.QA_UPPERCASE) {
    const i = (l) => {
      ...
      const p = /([A-Z]{2,}[ ])([A-Z]{2,}[ ])([A-Z]{2,}[ ])([A-Z]{2,})/g, r = d.match(p);
  ...
  const n = (i) => {
    ...
    const l = getComputedStyle(i), { textDecorationLine: d, textAlign: p, fontSize: r } = l, c = 'a[href], button, abbr, [role="link"], [role="button"], [tabindex="0"], [onclick]';
    e.checks.QA_UNDERLINE && (i.closest("u") || d === "underline") && !i.closest(c) && !i.matches(c) && n(i);
    const m = e.checks.QA_SMALL_TEXT.fontSize || 10, h = parseFloat(r), y = !((i.parentElement ? parseFloat(getComputedStyle(i.parentElement).fontSize) : null) === h) && h > 1 && h <= m;
    e.checks.QA_SMALL_TEXT && y && o(i);
  ...
  if (e.checks.QA_UNDERLINE || e.checks.QA_JUSTIFY || e.checks.QA_SMALL_TEXT)
    for (let i = 0; i < D.Found.Everything.length; i++) {
      const l = D.Found.Everything[i];
      Array.from(l.childNodes).filter((r) => r.nodeType === 3).map((r) => r.textContent).join("").trim().length !== 0 && a(l);
    }
  return e.checks.QA_SUBSCRIPT && D.Found.Subscripts.forEach((i) => {
    const l = Q(i);
    l.length >= 80 && t.push({
      test: "QA_SUBSCRIPT",
      element: i,
  ...
  }), e.checks.QA_NESTED_COMPONENTS && D.Found.NestedComponents.forEach((i) => {
...
function Or(t, e) {
  ...
  if (e.checks.META_SCALABLE || e.checks.META_MAX) {
    ...
    if (n) {
      ...
      if (s) {
        const o = s.split(",").reduce((i, l) => {
          const [d, p] = l.split("=").map((r) => r.trim());
          return i[d] = p, i;
        }, {});
  ...
  return e.checks.META_REFRESH && document.querySelector('meta[http-equiv="refresh"]') && t.push({
    ...
    const o = /* @__PURE__ */ new Set(), a = (l, d) => {
      l.forEach((p) => {
        const { id: r } = p;
        typeof r != "string" || r.trim().length === 0 || (r && !o.has(r) ? o.add(r) : Array.from(
          d.querySelectorAll(`
                a[href*="${r}"],
                label[for*="${r}"],
                [aria-labelledby*="${r}"],
          ...
          content: f.sprintf(e.checks.DUPLICATE_ID.content || "DUPLICATE_ID", r),
          dismiss: N(`DUPLICATEID${r}${p.textContent}`),
...
class Rr {
  constructor(e) {
    ...
    this.initialize = () => {
      (() => {
        const { doNotRun: o } = n;
        return o.trim().length > 0 ? document.querySelector(o) : !1;
      })() || (customElements.define("sa11y-heading-label", dr), customElements.define("sa11y-heading-anchor", cr), customElements.define("sa11y-annotation", or), customElements.define("sa11y-tooltips", Zn), customElements.define("sa11y-panel-tooltips", es), customElements.define("sa11y-control-panel", Sn), customElements.define("sa11y-console-error", Nn), u.initializeGlobal(n), u.initializeReadability(n), u.initializeExclusions(n), n.developerChecksOnByDefault && (R.getItem("sa11y-developer") === null || n.checkAllHideToggles) && R.setItem("sa11y-developer", "On"), Fs(() => {
        if (n.headless)
          this.checkAll(), R.removeItem("sa11y-dismissed");
        else {
          const o = R.getItem("sa11y-position"), { panelPosition: a } = n;
          n.showMovePanelToggle && (!o || !o.includes("top") != !a.includes("top")) && R.setItem("sa11y-position", a);
          const i = new Sn();
          document.body.appendChild(i), ha(this.checkAll, this.resetAll), ga(), aa(), Js(n.detectSPArouting, this.checkAll, this.resetAll), this.panelTooltips = new es(), document.body.appendChild(this.panelTooltips), u.Panel.toggle.disabled = !1, setTimeout(() => {
            this.resetAll(), this.checkAll();
          }, n.delayCheck), n.delayCheck >= 700 && (u.Panel.toggle.disabled = !0), ua(this.checkAll, this.resetAll);
        }
      }));
    }, this.checkAll = async (s = n.checkRoot, o = n.readabilityRoot, a = n.fixedRoots) => {
      try {
        this.results = [], this.headingOutline = [], this.errorCount = 0, this.warningCount = 0, this.customChecksRunning = !1, u.initializeRoot(s, o, a), $s(n), D.initializeElements(n), yr(this.results, n, this.headingOutline), xr(this.results, n), br(this.results, n), Sr(this.results, n), Pr(this.results, n), Or(this.results, n), n.embeddedContentPlugin && Cr(this.results, n), n.contrastPlugin && Nr(this.results, n), n.readabilityPlugin && R.getItem("sa11y-readability") === "On" && wr(this.results), this.imageResults = D.Found.Images.map((i) => {
          const l = this.results.find((d) => d.element === i);
          return l && {
            element: i,
            type: l.type,
            dismiss: l.dismiss,
      ...
      const s = R.getItem("sa11y-developer") === "Off" || R.getItem("sa11y-developer") === null;
      if ((s || n.externalDeveloperChecks === !0) && (this.results = this.results.filter((a) => a.developer !== !0)), s && (this.results = this.results.filter((a) => a.external !== !0)), this.results.forEach((a, i) => {
        const l = n.selectorPath ? Qs(a.element) : "", d = a.element?.outerHTML.replace(/\s{2,}/g, " ").trim() || "";
        Object.assign(a, { htmlPath: d, cssPath: l, id: i });
      }), n.headless === !1) {
        const a = Zs(
          this.results,
          this.panelTooltips,
        ...
        if (Ea(i.error, i.warning), R.getItem("sa11y-panel") === "Opened") {
          ...
          this.results.forEach((p) => {
            ...
            if (p.element && !p.margin) {
              ...
              r = { ...p, margin: `${c * 20 + m}px` };
            }
            lr(r, n);
          }), D.initializeAnnotations();
          const d = new Zn();
          document.body.appendChild(d), na(this.results, this.dismissed, this.checkAll, this.resetAll), ma(this.dismissedPageResults, this.headingOutline, n), n.showImageOutline && ba(this.dismissedPageResults, this.imageResults, n), ya(a.dismissCount, i.error, i.warning), gr(this.results), n.exportResultsPlugin && da(this.results, a.dismissedResults), gt(u.Panel.pageIssuesList, u.Panel.pageIssuesContent);
        }
        u.Panel.toggle.disabled = !1;
      }
      window.sa11yCheckComplete = null;
      const o = new CustomEvent("sa11y-check-complete", {
        detail: {
          results: this.results,
          page: window.location.pathname
        }
      });
      window.sa11yCheckComplete = o.detail, document.dispatchEvent(o);
    }, this.resetAll = (s = !0) => {
      for (u.Global.html.removeAttribute("data-sa11y-active"), ns(
        ...
        "document"
      ), os() && Z("[data-sa11y-error], [data-sa11y-warning], [data-sa11y-good]", "document").forEach(
        (o) => {
          const a = o, i = (a.style.anchorName || "").split(",").map((l) => l.trim()).filter((l) => l && !l.startsWith("--sa11y-anchor"));
        ...
        "document"
      ), u.Panel.outlineList.innerHTML = "", n.showImageOutline && (u.Panel.imagesList.innerHTML = ""), u.Panel.pageIssuesList.innerHTML = "", u.Panel.readabilityInfo.innerHTML = "", u.Panel.readabilityDetails.innerHTML = "", u.Panel.panel.classList.remove("has-page-issues"), u.Panel.pageIssues.classList.remove("active"), u.Panel.settingsContent.classList.remove("hide-settings-border"), u.Panel.panel.querySelector("#readability-alert")?.remove(), We(), mr(), pa(), sa(), ia(); u.Panel.status.firstChild; )
        u.Panel.status.removeChild(u.Panel.status.firstChild);
      document.querySelectorAll("[data-sa11y-has-shadow-root]").forEach((o) => {
        o.shadowRoot.querySelectorAll("style.sa11y-css-utilities").forEach((a) => {
          a.remove();
        }), o.removeAttribute("data-sa11y-has-shadow-root");
      }), s && u.Panel.panel.classList.remove("active");
    }, this.disabled = () => {
      setTimeout(() => {
        R.getItem("sa11y-panel") === "Opened" && u.Panel.toggle.click(), u.Panel.toggle.disabled = !0;
      }, n.delayCheck + 10);
    }, this.enabled = () => {
      u.Panel.toggle && (u.Panel.toggle.disabled = !1);
    }, this.find = (s, o, a) => Z(s, o, a), this.prepareDismissal = (s) => N(s), this.sanitizeHTML = (s) => Y(s), this.truncateString = (s, o) => Se(s, o), this.initialize();
  }
}
export {
  f as Lang,
