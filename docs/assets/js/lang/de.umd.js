...
(function (global, factory) {
  ...
  var de = {
    ...
    strings: {
      ...
      // Headings
      HEADING_SKIPPED_LEVEL: 'Nicht-konsekutive Überschriftenebene verwendet. Überschriften sollten niemals Ebenen überspringen oder von <strong>Überschrift %(prevLevel)</strong> zu <strong {R}>Überschrift %(level)</strong> gehen.',
      ...
      LINK_ALT_FILE_EXT: 'Dateierweiterung im Alt-Text gefunden. Achte darauf, dass der Alt-Text das Ziel des Links beschreibt und nicht eine wörtliche Beschreibung des Bildes ist. Entferne: <strong {R}>%(ERROR)</strong> <hr> {ALT} {L} <strong {R}>%(ALT_TEXT)</strong>',
      LINK_PLACEHOLDER_ALT: 'Nicht beschreibender oder Platzhalter-Alt-Text innerhalb eines verlinkten Bildes gefunden. Achte darauf, dass der Alt-Text das Ziel des Links beschreibt und nicht eine wörtliche Beschreibung des Bildes ist. Ersetzen Sie den folgenden Alt-Text. <hr> {ALT} {L} <strong {R}>%(ALT_TEXT)</strong>',
      LINK_SUS_ALT: 'Assistive Technologien zeigen bereits an, dass es sich um ein Bild handelt, so dass &quot;<strong {R}>%(ERROR)</strong>&quot; möglicherweise überflüssig ist. Achte darauf, dass der Alt-Text das Ziel des Links beschreibt und nicht eine wörtliche Beschreibung des Bildes ist. <hr> {ALT} {L} <strong {W}>%(ALT_TEXT)</strong>',
      ALT_FILE_EXT: 'Dateierweiterung im Alt-Text gefunden. Wenn das Bild eine Geschichte, eine Stimmung oder eine wichtige Information vermittelt - beschreibe das Bild unbedingt. Entferne: <strong {R}>%(ERROR)</strong> <hr> {ALT} <strong {R}>%(ALT_TEXT)</strong>',
      ALT_PLACEHOLDER: 'Nicht-beschreibender oder Platzhalter-Alt-Text gefunden. Ersetze den folgenden Alt-Text durch einen aussagekräftigeren Text. <hr> {ALT} <strong {R}>%(ALT_TEXT)</strong>',
      SUS_ALT: 'Assistive Technologien zeigen bereits an, dass es sich um ein Bild handelt, so dass &quot;<strong {R}>%(ERROR)</strong>&quot; möglicherweise überflüssig ist. <hr> {ALT} <strong {W}>%(ALT_TEXT)</strong>',
      ...
      LINK_IMAGE_ALT: 'Der Bildlink enthält einen Alt-Text. <strong>Beschreibt der Alt-Text, wohin der Link Sie führt?</strong> Erwägen Sie, den Titel der Seite, zu der der Link führt, als Alt-Text zu verwenden. <hr> {ALT} {L} <strong {W}>%(ALT_TEXT)</strong>',
      ...
      LABELS_ARIA_LABEL_INPUT: 'Die Eingabe hat einen zugänglichen Namen, stelle bitte trotzdem sicher, dass es auch ein sichtbares Label gibt. <hr> <strong {B}>Eingabelabel</strong> <strong {W}>%(TEXT)</strong>',
      ...
      QA_PAGE_LANG: 'Seitensprache nicht angegeben! Bitte <a href="https://www.w3.org/International/questions/qa-html-language-declarations">deklariere die Sprache im HTML-Tag.</a>',
      ...
      QA_BLOCKQUOTE: 'Ist das eine Überschrift? <strong {W}>%(TEXT)</strong> <hr> Blockquotes sollten nur für Zitate verwendet werden. Wenn dies eine Überschrift sein soll, ändere dieses Blockquote in eine semantische Überschrift (z. B. Überschrift 2 oder Überschrift 3).',
