...
(function (global, factory) {
  ...
  var sk = {
    ...
    strings: {
      ...
      HEADING_SKIPPED_LEVEL: 'Použitá nepokračujúca úroveň záhlavia. Nadpisy by nikdy nemali preskakovať úrovne alebo prechádzať z <strong>Nadpisu %(prevLevel)</strong> na <strong {R}>Nadpis %(level)</strong>.',
      ...
      LINK_ALT_FILE_EXT: 'Nájdená prípona súboru v texte alt. Uistite sa, že text alt opisuje cieľ odkazu, nie doslovný opis obrázka. Odstráňte: <strong {R}>%(ERROR)</strong> <hr> {ALT} {L} <strong {R}>%(ALT_TEXT)</strong>',
      LINK_PLACEHOLDER_ALT: 'Nájdený text alt bez popisu alebo zástupný text v rámci prepojeného obrázka. Uistite sa, že text alt opisuje cieľ odkazu, nie doslovný opis obrázka. Nahraďte nasledujúci text alt. <hr> {ALT} {L} <strong {R}>%(ALT_TEXT)</strong>',
      LINK_SUS_ALT: 'Asistenčné technológie už indikujú, že ide o obrázok, takže &quot;<strong {W}>%(ERROR)</strong>&quot; môže byť zbytočné. Uistite sa, že alt text opisuje cieľ odkazu, nie doslovný opis obrázka. <hr> {ALT} {L} <strong {W}>%(ALT_TEXT)</strong>',
      ALT_FILE_EXT: 'Nájdená prípona súboru v texte alt. Ak obrázok vyjadruje príbeh, náladu alebo dôležitú informáciu, nezabudnite ho opísať. Odstráňte: <strong {R}>%(ERROR)</strong> <hr> {ALT} <strong {R}>%(ALT_TEXT)</strong>',
      ALT_PLACEHOLDER: 'Nájdený text alt bez popisu alebo zástupného textu. Nahraďte nasledujúci alt text niečím zmysluplnejším. <hr> {ALT} <strong {R}>%(ALT_TEXT)</strong>',
      SUS_ALT: 'Asistenčné technológie už indikujú, že ide o obrázok, takže &quot;<strong {W}>%(ERROR)</strong>&quot; môže byť zbytočné. <hr> {ALT} <strong {W}>%(ALT_TEXT)</strong>',
      ...
      LINK_IMAGE_ALT: 'Odkaz na obrázok obsahuje text alt. Opisuje text alt, kam vás odkaz zavedie? <strong>Zvážte použitie názvu stránky, na ktorú odkazuje, ako alt textu.</strong> <hr> {ALT} {L} <strong {W}>%(ALT_TEXT)</strong>',
      ...
      LABELS_ARIA_LABEL_INPUT: 'Vstup má prístupný názov, ale dbajte na to, aby bol viditeľný aj štítok. <hr> <strong {B}>Štítok vstupu</strong> <strong {W}>%(TEXT)</strong>',
      ...
      QA_PAGE_LANG: 'Jazyk stránky nie je deklarovaný! Prosím <a href="https://www.w3.org/International/questions/qa-html-language-declarations">deklarujte jazyk v značke HTML.</a>',
      ...
      QA_BLOCKQUOTE: 'Je to nadpis? <strong {W}>%(TEXT)</strong> <hr> Blokové úvodzovky by sa mali používať len pre citácie. Ak to má byť nadpis, zmeňte tento blokový citát na sémantický nadpis (napr. Nadpis 2 alebo Nadpis 3).',
