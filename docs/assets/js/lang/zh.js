...
var zh = {
  ...
  strings: {
    ...
    HEADING_SKIPPED_LEVEL: '使用了非连续的标题级别。标题不应跳级, 或从<strong>标题 %(prevLevel)</strong>到<strong {R}>标题%(level)</strong>。',
    ...
    LINK_ALT_FILE_EXT: '在alt文本中发现文件扩展名。确保alt文本描述链接的目的地, 而不是图片的字面描述。删除: <strong {R}>%(ERROR)</strong>. <hr> {ALT} {L} <strong {R}>%(ALT_TEXT)</strong>',
    LINK_PLACEHOLDER_ALT: '发现链接图片中的非描述性或占位符的alt文本。确保alt文本描述了链接的目的地, 而不是图像的字面描述。替换以下alt文本. <hr> {ALT} {L} <strong {R}>%(ALT_TEXT)</strong>',
    LINK_SUS_ALT: '辅助技术已经表明这是一张图片, 所以&quot;<strong {W}>%(ERROR)</strong>&quot; 可能是多余的。确保alt文本描述了链接的目的地, 而不是图像的字面描述。<hr> {ALT} {L} <strong {W}>%(ALT_TEXT)</strong>',
    ALT_FILE_EXT: '在alt文本内找到文件扩展名。如果图片传达了一个故事、情绪或重要信息--一定要描述图片。删除: <strong {R}>%(ERROR)</strong> <hr> {ALT} <strong {R}>%(ALT_TEXT)</strong>',
    ALT_PLACEHOLDER: '发现非描述性或占位符的alt文本。用更有意义的内容替换下面的alt文本. <hr> {ALT} <strong {R}>%(ALT_TEXT)</strong>。',
    SUS_ALT: '辅助技术已经表明这是一张图片, 所以&quot;<strong {W}>%(ERROR)</strong>&quot; 可能是多余的。<hr> {ALT} <strong {W}>%(ALT_TEXT)</strong>',
    ...
    LINK_IMAGE_ALT: '图片链接包含alt文本。符号文本是否描述了该链接的位置？可以考虑使用它所链接的页面的标题作为alt文本。 <hr> {ALT} {L} <strong {W}>%(ALT_TEXT)</strong>',
    LINK_IMAGE_ALT_AND_TEXT: '图片链接包含<strong>alt文本和周围的链接文本。</strong>如果该图片是装饰性的, 并被用作另一个页面的功能链接, 请考虑将该图片标记为装饰性或无效--周围的链接文本应该足够了。<hr> {ALT} <strong {W}>%(ALT_TEXT)</strong> <hr> <strong>链接标签:</strong> {L} <strong {W}>%(TEXT)</strong>',
    ...
    LABELS_ARIA_LABEL_INPUT: '输入有一个无障碍名称, 但请确保也有一个可见的标签。<hr> <strong {B}>输入标签</strong> <strong {W}>%(TEXT)</strong>',
    ...
    QA_PAGE_LANG: '页面语言未声明!请<a href="https://www.w3.org/International/questions/qa-html-language-declarations">在HTML标签上声明语言。</a>',
    QA_PAGE_TITLE: '缺少页面标题!请提供一个<a href="https://developer.mozilla.org/zh/docs/Web/HTML/Element/title">页面标题。</a>',
    QA_BLOCKQUOTE: '这是一个标题吗？<strong {W}>%(TEXT)</strong> <hr> 方块引号应该只用于引号。如果这是一个标题, 请将这个区块引号改为语义标题 (例如标题2或标题3) 。',
    QA_FAKE_HEADING: '这是一个标题吗？<strong {W}>%(TEXT)</strong> <hr> 一行粗体或大字体可能看起来像一个标题, 但使用屏幕阅读器的人无法看出它的重要性或跳到它的内容。粗体或大字体永远不应取代语义标题 (标题2至标题6) 。',
    QA_FAKE_LIST: '您是否试图创建一个列表？找到了可能的列表项: <strong {W}>%(firstPrefix)</strong> <hr> 请确保使用语义列表, 用子弹或数字格式按钮代替。当使用语义列表时, 辅助技术能够传达信息, 如项目的总数和每个项目在列表中的相对位置。了解更多关于<a href="https://www.w3.org/WAI/tutorials/page-structure/content/#lists">语义列表的信息。</a>',
