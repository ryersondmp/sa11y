...
var it = {
  ...
  strings: {
    ...
    HEADING_SKIPPED_LEVEL: 'È stato utilizzato un livello di intestazione non consecutivo. I titoli non devono mai saltare livelli o passare da <strong>Heading %(prevLevel)</strong> a <strong {R}>Heading %(level)</strong>.',
    ...
    LINK_ALT_FILE_EXT: "Estensione del file nel testo alt trovata. Assicurarsi che il testo alt descriva la destinazione del link e non una descrizione letterale dell'immagine. Rimuovere: <strong {R}>%(ERROR)</strong> <hr> {ALT} {L} <strong {R}>%(ALT_TEXT)</strong>",
    LINK_PLACEHOLDER_ALT: "Trovato testo alt non descrittivo o segnaposto in un'immagine collegata. Assicurarsi che il testo alt descriva la destinazione del link e non una descrizione letterale dell'immagine. Sostituire il seguente testo alt. <hr> {ALT} {L} <strong {R}>%(ALT_TEXT)</strong>",
    LINK_SUS_ALT: "Le tecnologie assistive indicano già che si tratta di un'immagine, quindi &quot;<strong {W}>%(ERROR)</strong>&quot; potrebbe essere ridondante. Assicuratevi che il testo alt descriva la destinazione del link e non una descrizione letterale dell'immagine. <hr> {ALT} {L} <strong {W}>%(ALT_TEXT)</strong>",
    ALT_FILE_EXT: "Estensione del file nel testo alt trovato. Se l'immagine trasmette una storia, uno stato d'animo o un'informazione importante, assicuratevi di descrivere l'immagine. Rimuovere: <strong {R}>%(ERROR)</strong> <hr> {ALT} <strong {R}>%(ALT_TEXT)</strong>",
    ALT_PLACEHOLDER: 'Trovato testo alt non descrittivo o segnaposto. Sostituire il seguente testo alt con qualcosa di più significativo. <hr> {ALT} <strong {R}>%(ALT_TEXT)</strong>',
    SUS_ALT: "Le tecnologie assistive indicano già che si tratta di un'immagine, quindi &quot;<strong {W}>%(ERROR)</strong>&quot; potrebbe essere ridondante. <hr> {ALT} <strong {W}>%(ALT_TEXT)</strong>",
    ...
    LINK_IMAGE_ALT: "Il link all'immagine contiene un testo alt. <strong>Il testo alt descrive dove porta il link?</strong> Considerate la possibilità di utilizzare come testo alt il titolo della pagina a cui rimanda. <hr> {ALT} {L} <strong {W}>%(ALT_TEXT)</strong>",
    ...
    LABELS_ARIA_LABEL_INPUT: "L'input ha un nome accessibile, anche se bisogna assicurarsi che ci sia anche un'etichetta visibile. <hr> <strong {B}>Etichetta input</strong> <strong {W}>%(TEXT)</strong>",
    ...
    QA_PAGE_LANG: 'Lingua della pagina non dichiarata! Si prega di <a href="https://www.w3.org/International/questions/qa-html-language-declarations">dichiarare la lingua nel tag HTML.</a>',
    ...
    QA_BLOCKQUOTE: "È un titolo? <strong {W}>%(TEXT)</strong> <hr> Le virgolette dovrebbero essere usate solo per le citazioni. Se si tratta di un'intestazione, cambiare questo blockquote in un'intestazione semantica (per esempio, Intestazione 2 o Intestazione 3).",
