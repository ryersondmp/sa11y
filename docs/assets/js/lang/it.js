...
var it = {
  ...
  strings: {
    ...
    LINK_IDENTICAL_NAME: 'Il link ha lo stesso testo di un altro link, anche se punta a una pagina diversa. Più link con lo stesso testo possono creare confusione per le persone che utilizzano gli screen reader. <hr> Considerate di rendere il seguente link più descrittivo per aiutarlo a distinguersi dagli altri link: <strong {C}>%(TEXT)</strong>',
    ...
    LABELS_ARIA_LABEL_INPUT: "L'input ha un nome accessibile, anche se bisogna assicurarsi che ci sia anche un'etichetta visibile. <hr> <strong {B}>Etichetta input</strong> <strong {C}>%(TEXT)</strong>",
    ...
    QA_DUPLICATE_ID: 'Trovato <strong>ID duplicato</strong>. Gli errori di ID duplicato sono noti per causare problemi alle tecnologie assistive quando cercano di interagire con i contenuti. <hr> Si prega di rimuovere o modificare il seguente ID: <strong {C}>%(id)</strong>',
    ...
    QA_UNCONTAINED_LI: 'Tutti gli elementi di elenco <code>&lt;li&gt;</code> devono essere inseriti all\'interno di elementi <code>&lt;ul&gt;</code> non ordinati o <code>&lt;ol&gt;</code> ordinati. Questa struttura aiuta i lettori di schermo ad annunciare l\'elenco e i suoi elementi con precisione.',
    QA_META_SCALABLE: 'Rimuovi il parametro <code>user-scalable="no"</code> nell\'elemento <code>&lt;meta name="viewport"&gt;</code> per consentire lo zoom.',
    QA_META_MAX: 'Assicurati che il parametro <code>maximum-scale</code> nell\'elemento <code>&lt;meta name="viewport"&gt;</code> non sia inferiore a 2.',
    QA_JUSTIFY: 'Evita di utilizzare il testo giustificato, che si allinea sia ai margini sinistro che destro. Questo può essere difficile da leggere per alcune persone a causa degli spazi irregolari tra le parole. Usa il testo allineato a sinistra per una migliore leggibilità.',
