...
(function (global, factory) {
  ...
  var tr = {
    ...
    strings: {
      ...
      HEADING_SKIPPED_LEVEL: "Ardışık olmayan başlık seviyesi kullanıldı. Başlıklar asla seviye atlamamalı veya <strong>Başlık %(prevLevel)</strong>'den <strong {R}>Başlık %(level)</strong>'e geçmemelidir.",
      ...
      LINK_ALT_FILE_EXT: 'Alt metin içinde dosya uzantısı bulundu. Alt metnin, görüntünün gerçek bir tanımını değil, bağlantının hedefini açıkladığından emin olun. Kaldırın: <strong {R}>%(ERROR)</strong> <hr> {ALT} {L} <strong {R}>%(ALT_TEXT)</strong>',
      LINK_PLACEHOLDER_ALT: 'Bağlantılı bir resim içinde tanımlayıcı olmayan veya yer tutucu alt metin bulundu. Alt metnin, görüntünün gerçek bir tanımını değil, bağlantının hedefini açıkladığından emin olun. Aşağıdaki alt metni değiştirin. <hr> {ALT} {L} <strong {R}>%(ALT_TEXT)</strong>',
      LINK_SUS_ALT: 'Yardımcı teknolojiler zaten bunun bir resim olduğunu gösterir, bu nedenle &quot;<strong {R}>%(ERROR)</strong>&quot; gereksiz olabilir. Alt metnin, resmin gerçek bir tanımını değil, bağlantının hedefini açıkladığından emin olun. <hr> {ALT} {L} <strong {W}>%(ALT_TEXT)</strong>',
      ALT_FILE_EXT: 'Bulunan alt metin içinde dosya uzantısı. Görsel bir hikaye, ruh hali veya önemli bir bilgi aktarıyorsa, görseli tanımladığınızdan emin olun. Kaldırın: <strong {R}>%(ERROR)</strong> <hr> {ALT} <strong {R}>%(ALT_TEXT)</strong>',
      ALT_PLACEHOLDER: 'Tanımlayıcı olmayan veya yer tutucu alt metin bulundu. Aşağıdaki alt metni daha anlamlı bir metinle değiştirin. <hr> {ALT} <strong {R}>%(ALT_TEXT)</strong>',
      SUS_ALT: 'Yardımcı teknolojiler zaten bunun bir resim olduğunu belirtmektedir, bu nedenle &quot;<strong {W}>%(ERROR)</strong>&quot; gereksiz olabilir. <hr> {ALT} <strong {W}>%(ALT_TEXT)</strong>',
      ...
      LINK_IMAGE_ALT: 'Resim bağlantısı alt metin içeriyor. <strong>Alt metin bağlantının sizi nereye götürdüğünü açıklıyor mu?</strong> Bağlantı verilen sayfanın başlığını alt metin olarak kullanmayı düşünün. <hr> {ALT} {L} <strong {W}>%(ALT_TEXT)</strong>',
      ...
      LABELS_ARIA_LABEL_INPUT: 'Girdinin erişilebilir bir adı vardır, ancak lütfen görünür bir etiket olduğundan da emin olun. <hr> <strong {B}>Giriş etiketi</strong> <strong {W}>%(TEXT)</strong>',
      ...
      QA_PAGE_LANG: 'Sayfa dili bildirilmedi! Lütfen <a href="https://www.w3.org/International/questions/qa-html-language-declarations">declare language on HTML tag.</a>',
      ...
      QA_BLOCKQUOTE: 'Bu bir başlık mı? <strong {W}>%(TEXT)</strong> <hr> Blok tırnaklar yalnızca alıntılar için kullanılmalıdır. Bunun bir başlık olması amaçlanıyorsa, bu blok alıntıyı anlamsal bir başlığa (örneğin Başlık 2 veya Başlık 3) değiştirin.',
      ...
      CONTRAST_INPUT: 'Bu girişteki metin arka planla yeterli kontrasta sahip değil. Kontrast oranı normal metin için en az 4,5:1 ve büyük metin için 3:1 olmalıdır. <hr> <strong {B}>Kontrast Oranı</strong> <strong {B}>%(RATIO)</strong> <strong {R}>%(TEXT)</strong>',
    },
  };
