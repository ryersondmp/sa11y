...
(function (global, factory) {
  ...
  var ptPT = {
    ...
    strings: {
      ...
      HEADING_SKIPPED_LEVEL: 'Nível de cabeçalho não consecutivo utilizado. Os títulos nunca devem saltar níveis, ou ir de <strong>Título %(prevLevel)</strong> para <strong {R}>Título %(level)</strong>.',
      ...
      LINK_ALT_FILE_EXT: 'Extensão de ficheiro no texto alternativo encontrada. Certifique-se de que o texto alternativo descreve o destino da hiperligação e não uma descrição literal da imagem. Remover: <strong {R}>%(ERROR)</strong> <hr> {ALT} {L} <strong {R}>%(ALT_TEXT)</strong>',
      LINK_PLACEHOLDER_ALT: 'Encontrado texto alternativo não descritivo ou de espaço reservado numa imagem ligada. Certifique-se de que o texto alternativo descreve o destino da hiperligação e não uma descrição literal da imagem. Substitua o seguinte texto alternativo. <hr> {ALT} {L} <strong {R}>%(ALT_TEXT)</strong>',
      LINK_SUS_ALT: 'As tecnologias de assistência já indicam que se trata de uma imagem, pelo que &quot;<strong {W}>%(ERROR)</strong>&quot; pode ser redundante. Certifique-se de que o texto alternativo descreve o destino da hiperligação e não uma descrição literal da imagem. <hr> {ALT} {L} <strong {W}>%(ALT_TEXT)</strong>',
      ALT_FILE_EXT: 'Extensão do ficheiro no texto alternativo encontrado. Se a imagem transmitir uma história, um estado de espírito ou uma informação importante, não se esqueça de a descrever. Remover: <strong {R}>%(ERROR)</strong> <hr> {ALT} <strong {R}>%(ALT_TEXT)</strong>',
      ALT_PLACEHOLDER: 'Encontrado texto alternativo não descritivo ou de espaço reservado. Substitua o seguinte texto alternativo por algo mais significativo. <hr> {ALT} <strong {R}>%(ALT_TEXT)</strong>',
      SUS_ALT: 'As tecnologias de assistência já indicam que se trata de uma imagem, pelo que &quot;<strong {W}>%(ERROR)</strong>&quot; pode ser redundante. <hr> {ALT} <strong {W}>%(ALT_TEXT)</strong>',
      ...
      LINK_IMAGE_ALT: 'A hiperligação da imagem contém texto alternativo. <strong>O texto alternativo descreve para onde o link o leva?</strong> Considere a possibilidade de utilizar o título da página para a qual a ligação remete como texto alternativo. <hr> {ALT} {L} <strong {W}>%(ALT_TEXT)</strong>',
      ...
      LABELS_ARIA_LABEL_INPUT: 'A entrada tem um nome acessível, mas certifique-se de que também existe uma etiqueta visível. <hr> <strong {B}>Rótulo</strong> <strong {W}>%(TEXT)</strong>',
      ...
      QA_PAGE_LANG: 'Idioma da página não declarado! Por favor <a href="https://www.w3.org/International/questions/qa-html-language-declarations">declare o idioma na etiqueta HTML.</a>',
      ...
      QA_BLOCKQUOTE: 'Isto é um cabeçalho? <strong {W}>%(TEXT)</strong> <hr> As aspas de bloco devem ser usadas apenas para citações. Se se pretende que isto seja um título, altere esta citação de bloco para um título semântico (por exemplo, Título 2 ou Título 3).',
