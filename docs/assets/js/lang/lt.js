...
var lt = {
  ...
  strings: {
    ...
    LINK_IDENTICAL_NAME: 'Nuoroda turi tokį patį tekstą kaip ir kita nuoroda, nors nukreipia į kitą puslapį. Kelios nuorodos su tuo pačiu tekstu gali klaidinti ekrano skaitytuvus naudojančius žmones. <hr> Pagalvokite, ar nevertėtų šios nuorodos aprašyti labiau, kad ją būtų lengviau atskirti nuo kitų nuorodų: <strong {C}>%(TEXT)</strong>',
    ...
    LABELS_ARIA_LABEL_INPUT: 'Įvestis turi prieinamą pavadinimą, tačiau užtikrinkite, kad būtų matoma ir etiketė. <hr> <strong {B}>Įvesties žymė</strong> <strong {C}>%(TEXT)</strong>',
    ...
    QA_DUPLICATE_ID: 'Rastas <strong>dvigubas ID</strong>. Yra žinoma, kad pasikartojančio ID klaidos sukelia problemų pagalbinėms technologijoms, kai jos bando sąveikauti su turiniu. <hr> Prašome pašalinti arba pakeisti šį ID: <strong {C}>%(id)</strong>',
    ...
    QA_UNCONTAINED_LI: 'Visi <code>&lt;li&gt;</code> sąrašo elementai turi būti įdėti į <code>&lt;ul&gt;</code> nesurūšiuotus arba <code>&lt;ol&gt;</code> surūšiuotus elementus. Ši struktūra padeda ekranų skaitytuvams tiksliai paskelbti sąrašą ir jo elementus.',
    QA_META_SCALABLE: 'Pašalinkite <code>user-scalable="no"</code> parametrą <code>&lt;meta name="viewport"&gt;</code> elemente, kad būtų leidžiamas priartinimas.',
    QA_META_MAX: 'Įsitikinkite, kad <code>maximum-scale</code> parametras <code>&lt;meta name="viewport"&gt;</code> elemente nėra mažesnis nei 2.',
    QA_JUSTIFY: 'Venkite naudoti išlygintą tekstą, kuris išlyginamas tiek kairiuoju, tiek dešiniuoju kraštu. Tai gali būti sunku skaityti kai kuriems žmonėms dėl netolygiai paskirstytų tarpo tarp žodžių. Naudokite kairėje išlygintą tekstą geresniam skaitymui.',
