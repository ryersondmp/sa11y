...
var ptBR = {
  ...
  strings: {
    ...
    LABELS_ARIA_LABEL_INPUT: 'A entrada tem um nome acessível, mas certifique-se de que também haja um rótulo visível. <hr> <strong {B}>Rótulo</strong> <strong {C}>%(TEXT)</strong>',
    ...
    QA_DUPLICATE_ID: 'Encontrada <strong>identificação duplicada</strong>. Erros de ID duplicado são conhecidos por causar problemas para tecnologias assistivas quando elas estão tentando interagir com o conteúdo. <hr> Remova ou altere o seguinte ID: <strong {C}>%(id)</strong>',
    ...
    QA_UNCONTAINED_LI: 'Todos os itens de lista <code>&lt;li&gt;</code> devem ser colocados dentro dos elementos <code>&lt;ul&gt;</code> não ordenados ou <code>&lt;ol&gt;</code> ordenados. Essa estrutura ajuda leitores de tela a anunciar a lista e seus itens com precisão.',
    QA_META_SCALABLE: 'Remova o parâmetro <code>user-scalable="no"</code> no elemento <code>&lt;meta name="viewport"&gt;</code> para permitir o zoom.',
    QA_META_MAX: 'Certifique-se de que o parâmetro <code>maximum-scale</code> no elemento <code>&lt;meta name="viewport"&gt;</code> não seja inferior a 2.',
    QA_JUSTIFY: 'Evite usar texto justificado, que se alinha tanto às margens esquerda quanto direita. Isso pode ser difícil de ler para algumas pessoas devido aos espaços desiguais entre as palavras. Use texto alinhado à esquerda para uma melhor legibilidade.',
