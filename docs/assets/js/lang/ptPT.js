...
var ptPT = {
  ...
  strings: {
    ...
    HEADING_LONG: 'O título é longo! Os títulos devem ser utilizados para organizar o conteúdo e transmitir estrutura. Devem ser breves, informativos e únicos. Os títulos devem ter menos de 160 caracteres (não mais do que uma frase). <hr> Contagem de caracteres: <strong {r}>%(headingLength)</strong>',
    ...
    LINK_IMAGE_BAD_ALT_MESSAGE: 'Extensão de ficheiro no texto alternativo encontrada. Certifique-se de que o texto alternativo descreve o destino da hiperligação e não uma descrição literal da imagem. Remover: <strong {r}>%(error)</strong>.<hr><strong>Texto alternativo:</strong> %(altText)',
    LINK_IMAGE_PLACEHOLDER_ALT_MESSAGE: 'Encontrado texto alternativo não descritivo ou de espaço reservado numa imagem ligada. Certifique-se de que o texto alternativo descreve o destino da hiperligação e não uma descrição literal da imagem. Substitua o seguinte texto alternativo: <strong {r}>%(altText)</strong>',
    LINK_IMAGE_SUS_ALT_MESSAGE: 'As tecnologias de assistência já indicam que se trata de uma imagem, pelo que &quot;<strong {r}>%(error)</strong>&quot; pode ser redundante. Certifique-se de que o texto alternativo descreve o destino da hiperligação e não uma descrição literal da imagem. <hr> <strong>Texto alternativo:</strong> %(altText)',
    LINK_ALT_HAS_BAD_WORD_MESSAGE: 'Extensão do ficheiro no texto alternativo encontrado. Se a imagem transmitir uma história, um estado de espírito ou uma informação importante, não se esqueça de a descrever. Remover: <strong {r}>%(erro)</strong>.<hr><strong>Texto alternativo:</strong> %(altText)',
    ALT_PLACEHOLDER_MESSAGE: 'Encontrado texto alternativo não descritivo ou de espaço reservado. Substitua o seguinte texto alternativo por algo mais significativo: <strong {r}>%(altText)</strong>',
    ALT_HAS_SUS_WORD: 'As tecnologias de assistência já indicam que se trata de uma imagem, pelo que &quot;<strong {r}>%(error)</strong>&quot; pode ser redundante. <hr> <strong>Texto alternativo:</strong> %(altText)',
    LINK_IMAGE_ARIA_HIDDEN: 'O link em torno da imagem tem <code>aria-hidden=&quot;true&quot;</code>, mas ainda pode ser focalizado pelo teclado. Se a intenção é esconder um link redundante ou duplicado, adicione <code>tabindex=&quot;-1&quot;</code> também.',
    ...
    LINK_IMAGE_LONG_ALT: 'A descrição do texto alternativo numa imagem ligada é <strong>muito longa</strong>. O texto alternativo em imagens vinculadas deve descrever para onde o link o leva, não uma descrição literal da imagem. <strong>Considere usar o título da página para a qual o link leva como o texto alternativo.</strong> <hr> <strong>Texto alternativo (<span {r}>%(altLength)</span> caracteres):</strong> %(altText)',
    LINK_IMAGE_ALT_WARNING: 'A hiperligação da imagem contém texto alternativo. <strong>O texto alternativo descreve para onde o link o leva?</strong> Considere a possibilidade de utilizar o título da página para a qual a ligação remete como texto alternativo. <hr> <strong>Texto alternativo:</strong> %(altText)',
    LINK_IMAGE_ALT_AND_TEXT_WARNING: 'Se esta imagem for decorativa e estiver a ser utilizada como uma ligação funcional a outra página, considere marcar a imagem como decorativa ou nula - o texto da ligação circundante deve ser suficiente. <hr> <strong>Texto alternativo:</strong> %(altText)',
    ...
    LABELS_ARIA_LABEL_INPUT_MESSAGE: 'A entrada tem um nome acessível, mas certifique-se de que também existe uma etiqueta visível. <hr> O nome acessível para esta entrada é: <strong>%(sanitizedText)</strong>',
    ...
    EMBED_MISSING_TITLE: 'O conteúdo incorporado requer um nome acessível que descreva o seu conteúdo. Forneça um atributo <code>title</code> ou <code>aria-label</code> exclusivo no elemento <code>iframe</code>. Saiba mais sobre <a href="https://dequeuniversity.com/tips/provide-iframe-titles">iFrames.</a>',
    ...
    QA_BAD_LINK: 'Encontrada uma ligação incorrecta. O link parece apontar para um ambiente de desenvolvimento. <hr> Este link aponta para: <br> <strong {r}>%(el)</strong>',
    ...
    CONTRAST_ERROR: 'Este texto não tem contraste suficiente com o fundo. A relação de contraste deve ser de pelo menos 4,5:1 para texto normal e 3:1 para texto grande. <hr> A taxa de contraste é <strong {r}>%(cratio)</strong> para o seguinte texto: <strong {r}>%(sanitizedText)</strong>',
    CONTRAST_WARNING: 'O contraste deste texto é desconhecido e tem de ser revisto manualmente. Certifique-se de que o texto e o fundo têm cores fortes e contrastantes. A relação de contraste deve ser de pelo menos 4,5:1 para texto normal e 3:1 para texto grande. <hr> <strong>Por favor, reveja:</strong> %(sanitizedText)',
    CONTRAST_INPUT_ERROR: 'O texto nesta entrada não tem contraste suficiente com o fundo. O rácio de contraste deve ser de, pelo menos, 4,5:1 para texto normal e 3:1 para texto grande. <hr> Relação de contraste: <strong {r}>%(cratio)</strong>',
  },
};
