...
(function (global, factory) {
  ...
  var lt = {
    ...
    strings: {
      ...
      LINK_URL: 'Ilgesnius, mažiau suprantamus URL adresus, naudojamus kaip nuorodos tekstą, gali būti sunku išklausyti naudojant pagalbines technologijas. Daugeliu atvejų vietoj URL adreso geriau naudoti žmogui suprantamą tekstą. Trumpi URL adresai (pavyzdžiui, svetainės pagrindinis puslapis) yra tinkami.<hr><strong>Tipas!</strong> Nuorodos tekstas visada turi būti aiškus, unikalus ir prasmingas, kad jį būtų galima suprasti iš konteksto.',
      ...
      LINK_IMAGE_BAD_ALT_MESSAGE: 'Rastas failo plėtinys alt tekste. Užtikrinkite, kad alt tekstas apibūdintų nuorodos paskirties vietą, o ne pažodinį paveikslėlio aprašymą. Pašalinti: <strong {r}>%(error)</strong>.<hr><strong>Alt tekstas:</strong> %(altText)',
      ...
      LINK_ALT_HAS_BAD_WORD_MESSAGE: 'Rastas failo plėtinys alt tekste. Jei vaizdas perteikia istoriją, nuotaiką ar svarbią informaciją, būtinai aprašykite vaizdą. Pašalinti: <strong {r}>%(error)</strong>.<hr><strong>Alt tekstas:</strong> %(altText)',
      ...
      LINK_IMAGE_ARIA_HIDDEN: 'Nuoroda aplink paveikslėlį turi <code>aria-hidden=&quot;true&quot;</code>, bet ją vis tiek galima fokusuoti klaviatūra. Jei ketinate paslėpti nereikalingą arba besidubliuojančią nuorodą, taip pat pridėkite <code>tabindex=&quot;-1&quot;</code>.',
      ...
      LABELS_ARIA_LABEL_INPUT_MESSAGE: 'Įvestis turi prieinamą pavadinimą, tačiau užtikrinkite, kad būtų matoma ir etiketė. <hr> Šio įvesties įvesties prieinamas pavadinimas yra: <strong>%(sanitizedText)</strong>',
      ...
      EMBED_MISSING_TITLE: 'Įterptajam turiniui reikalingas prieinamas pavadinimas, apibūdinantis jo turinį. Elemente <code>iframe</code> nurodykite unikalų <code>title</code> arba <code>aria-label</code> atributą. Sužinokite daugiau apie <a href="https://dequeuniversity.com/tips/provide-iframe-titles">iRėmus.</a>.',
