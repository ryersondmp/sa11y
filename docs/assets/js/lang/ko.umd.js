...
(function (global, factory) {
  ...
  var ko = {
    ...
    strings: {
      ...
      ACC_NAME: '<strong {B}>링크 레이블</strong> %(TEXT)',
      ...
      LINK_IDENTICAL_NAME: '다른 페이지를 가리키지만 링크의 텍스트가 다른 링크와 동일합니다. 동일한 텍스트의 링크가 여러 개 있으면 화면 리더를 사용하는 사용자에게 혼란을 줄 수 있습니다.<hr>다음 링크를 다른 링크와 구분할 수 있도록 더 설명적으로 만드는 것을 고려하세요: <strong {C}>%(TEXT)</strong>',
      BTN_EMPTY: '버튼에 그 목적을 설명하는 접근 가능한 이름이 없습니다. 접근 가능한 <a href="https://www.sarasoueidan.com/blog/accessible-icon-buttons/">버튼을 만드는 방법</a>을 배우십시오.',
      BTN_EMPTY_LABELLEDBY: '버튼의 <code>aria-labelledby</code> 값이 비어 있거나 페이지의 다른 요소의 <code>id</code> 값과 일치하지 않습니다.',
      ...
      LINK_IMAGE_ALT_AND_TEXT: '이미지 링크에 <strong>대체 텍스트와 주변 링크 텍스트가 모두 포함됨</strong> 이 이미지가 장식용이고 다른 페이지에 대한 기능적 링크로 사용되는 경우 이미지를 장식용으로 표시하거나 주변 링크 텍스트로 충분할 수 있도록 null로 표시하는 것을 고려하세요. <hr> {ALT} <strong {C}>%(ALT_TEXT)</strong> <hr> <strong {B}>링크 레이블</strong> {L} <strong {C}>%(TEXT)</strong>',
      ...
      LABELS_ARIA_LABEL_INPUT: '입력에 접근 가능한 이름이 있지만 레이블도 표시되어 있는지 확인하세요. <hr> <strong {B}>입력 레이블</strong> <strong {C}>%(TEXT)</strong>',
      ...
      PAGE_LANG: '페이지 언어가 선언되지 않았습니다! <a href="https://www.w3.org/International/questions/qa-html-language-declarations">HTML 태그에 언어를 선언하세요.</a>',
