...
(function (global, factory) {
  ...
  var ja = {
    ...
    strings: {
      ...
      HEADING_LONG: '見出しが長すぎます！見出しはコンテンツを整理し、構造を伝えるために使用されます。簡潔で情報量があり、他と区別される必要があります。見出しは160文字未満に保つようにしてください（1文を超えないでください）。<hr> <strong {wb}>%(HEADING_LENGTH) 文字</strong>',
      ...
      LINK_ALT_HAS_FILE_EXTENSION: 'altテキスト内にファイルの拡張子が見つかりました。altテキストが画像のリンク先を説明しているかどうか、画像の文字通りの説明ではないことを確認してください。<strong {eb}>ALT</strong> <strong {r}>%(ALT_TEXT)</strong>',
      LINK_IMAGE_PLACEHOLDER_ALT_MESSAGE: 'リンクされた画像内の記述のないまたはプレースホルダーのaltテキストが見つかりました。altテキストが画像のリンク先を説明していることを確認してください。<strong {r}>%(ALT_TEXT)</strong>',
      LINK_IMAGE_SUS_ALT_MESSAGE: '支援技術はすでにこれが画像であることを示しているため、「<strong {w}>%(ERROR)</strong>」は冗長かもしれません。altテキストが画像のリンク先を説明していることを確認してください。<hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong>',
      ALT_HAS_FILE_EXTENSION: 'altテキスト内にファイルの拡張子が見つかりました。画像が物語やムード、重要な情報を伝えている場合は、画像を説明してください。<strong {eb}>ALT</strong> <strong {r}>%(ALT_TEXT)</strong>',
      ALT_PLACEHOLDER_MESSAGE: '記述のないまたはプレースホルダーのaltテキストが見つかりました。次のaltテキストをより意味のあるものに置き換えてください：<strong {r}>%(ALT_TEXT)</strong>',
      ALT_HAS_SUS_WORD: '支援技術はすでにこれが画像であることを示しているため、「<strong {w}>%(ERROR)</strong>」は冗長かもしれません。<hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong>',
      ...
      LINK_IMAGE_LONG_ALT: 'リンクされた画像のaltテキストの説明が<strong>長すぎます</strong>。リンクされた画像のaltテキストは、画像の文字通りの説明ではなく、リンクの目的を説明する必要があります。<strong>リンク先のページのタイトルをaltテキストとして使用することを検討してください。</strong> <hr> <strong {wb}>ALT</strong> <strong {wb}>%(altLength) 文字</strong> <strong {w}>%(ALT_TEXT)</strong>',
      LINK_IMAGE_ALT_WARNING: '画像リンクにはaltテキストが含まれています。altテキストがリンク先を説明していますか？<strong>リンク先のページのタイトルをaltテキストとして使用することを検討してください。</strong> <hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong>',
      LINK_IMAGE_ALT_AND_TEXT_WARNING: '画像リンクには<strong>altテキストと周囲のリンクテキストの両方が含まれています。</strong> この画像が装飾用であり、他のページへの機能的なリンクとして使用されている場合は、画像を装飾用としてマークすることを検討してください。周囲のリンクテキストで十分です。<hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong> <hr> <strong {wb}>ラベル</strong> <strong {w}>%(TEXT)</strong>',
      ...
      IMAGE_FIGURE_DUPLICATE_ALT: 'altとキャプションテキストにはまったく同じ単語を使用しないでください。スクリーンリーダーは情報を二重に発表します。<ul><li>altテキストは画像の内容を簡潔に説明する必要があります。</li><li>キャプションは通常、画像を周囲のコンテンツに関連付けるための文脈を提供したり、特定の情報に注意を払ったりするために使用されます。</li></ul>詳細はこちら：<a href="https://thoughtbot.com/blog/alt-vs-figcaption#the-figcaption-element">altとfigcaptionの比較</a> <hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong>',
      IMAGE_DECORATIVE: '画像が<strong>装飾用</strong>としてマークされ、支援技術によって無視されます。画像が物語やムード、重要な情報を伝えている場合は、altテキストを追加してください。',
      IMAGE_ALT_TOO_LONG: 'altテキストの説明が<strong>長すぎます</strong>。altテキストは簡潔でありながら意味のあるものである必要があります（ツイートのように、約100文字）。これが複雑な画像やグラフの場合は、画像の長い説明を以下のテキストまたは折りたたみコンポーネントに入れることを検討してください。<hr> <strong {wb}>ALT</strong> <strong {wb}>%(altLength) 文字</strong> <strong {w}>%(ALT_TEXT)</strong>',
      ...
      LABELS_ARIA_LABEL_INPUT_MESSAGE: '入力フィールドにはアクセス可能な名前がありますが、見えるラベルもあることを確認してください。<hr> <strong {wb}>入力ラベル</strong> <strong {w}>%(TEXT)</strong>',
      ...
      // Contrast
      CONTRAST_ERROR: 'このテキストは背景との対比が不足しています。通常のテキストには対比比率が少なくとも4.5:1、大きなテキストには3:1以上である必要があります。<hr> <strong {eb}>コントラスト比</strong> <strong {eb}>%(RATIO)</strong> <strong {r}>%(TEXT)</strong>',
      ...
      CONTRAST_INPUT_ERROR: 'この入力内のテキストは背景との対比が不足しています。通常のテキストには対比比率が少なくとも4.5:1、大きなテキストには3:1以上である必要があります。<hr> <strong {eb}>コントラスト比</strong> <strong {eb}>%(RATIO)</strong>',
    },
  };
