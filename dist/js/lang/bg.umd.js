...
(function (global, factory) {
  ...
  var bg = {
    ...
    strings: {
      ...
      LINK_IMAGE_BAD_ALT_MESSAGE: 'Намерено е разширение на файла в алтертекста. Уверете се, че текстът alt описва дестинацията на връзката, а не буквално описание на изображението. Премахване: <strong {r}>%(грешка)</strong>.<hr><strong>Алттекст:</strong> %(altText)',
      ...
      LINK_ALT_HAS_BAD_WORD_MESSAGE: 'Намерено е разширение на файла в алтертекста. Ако изображението предава история, настроение или важна информация - не забравяйте да опишете изображението. Премахване: <strong {r}>%(грешка)</strong>.<hr><strong>Алттекст:</strong> %(altText)',
      ...
      LINK_IMAGE_ARIA_HIDDEN: 'Връзката около изображението има <code>aria-hidden=&quot;true&quot;</code>, но все още може да се фокусира от клавиатурата. Ако възнамерявате да скриете излишна или дублираща се връзка, добавете и <code>tabindex=&quot;-1&quot;</code>.',
      ...
      LABELS_ARIA_LABEL_INPUT_MESSAGE: 'Входът има достъпно име, но се уверете, че има и видим етикет. <hr> Достъпното име за този вход е: <strong>%(sanitizedText)</strong>',
      ...
      EMBED_MISSING_TITLE: 'Вграденото съдържание изисква достъпно име, което описва съдържанието му. Моля, посочете уникален атрибут <code>title</code> или <code>aria-label</code> на елемента <code>iframe</code>. Научете повече за <a href="https://dequeuniversity.com/tips/provide-iframe-titles">iFrames.</a>.',
