...
(function (global, factory) {
  ...
  var bg = {
    ...
    strings: {
      ...
      HEADING_LONG: 'Курсът е дълъг! Заглавията трябва да се използват за организиране на съдържанието и предаване на структурата. Те трябва да са кратки, информативни и уникални. Моля, запазете заглавията по-малко от 160 знака (не повече от едно изречение). <hr> <strong {wb}>%(HEADING_LENGTH) Символа</strong>',
      ...
      LINK_ALT_HAS_FILE_EXTENSION: 'Намерено е разширение на файла в алтертекста. Уверете се, че текстът alt описва дестинацията на връзката, а не буквално описание на изображението. Премахване: <strong {r}>%(грешка)</strong>.<hr><strong {eb}>АЛТ</strong> <strong {r}>%(ALT_TEXT)</strong>',
      LINK_IMAGE_PLACEHOLDER_ALT_MESSAGE: 'Намерен е неописан или заместващ alt текст в свързано изображение. Уверете се, че текстът alt описва дестинацията на връзката, а не буквално описание на изображението. Заменете следния alt текст: <strong {r}>%(ALT_TEXT)</strong>',
      LINK_IMAGE_SUS_ALT_MESSAGE: 'Подпомагащите технологии вече показват, че това е изображение, така че &quot;<strong {r}>%(ERROR)</strong>&quot; може да е излишно. Уверете се, че текстът alt описва местоназначението на връзката, а не буквално описание на изображението. <hr> <strong {b}>АЛТ</strong> %(ALT_TEXT)',
      ALT_HAS_FILE_EXTENSION: 'Намерено е разширение на файла в алтертекста. Ако изображението предава история, настроение или важна информация - не забравяйте да опишете изображението. Премахване: <strong {r}>%(грешка)</strong>.<hr> <strong {eb}>АЛТ</strong> <strong {r}>%(ALT_TEXT)</strong>',
      ALT_PLACEHOLDER_MESSAGE: 'Открит е текст без описание или заместител. Заменете следния alt текст с нещо по-смислено: <strong {r}>%(ALT_TEXT)</strong>',
      ALT_HAS_SUS_WORD: 'Подпомагащите технологии вече показват, че това е изображение, така че &quot;<strong {r}>%(ERROR)</strong>&quot; може да е излишно. <hr> <strong {wb}>АЛТ</strong> <strong {w}>%(ALT_TEXT)</strong>',
      ...
      LINK_IMAGE_LONG_ALT: 'Описанието на Alt текста на свързано изображение е <strong>прекалено дълго</strong>. Alt текстът на свързаните изображения трябва да описва мястото, до което води връзката, а не буквално описание на изображението. <strong>Помислете за използване на заглавието на страницата, към която води връзката, като алт текст.</strong> <hr> <strong {wb}>АЛТ</strong> <strong {wb}>%(altLength) Символа</strong> <strong {w}>%(ALT_TEXT)</strong>',
      LINK_IMAGE_ALT_WARNING: 'Връзката на изображението съдържа текст alt. Описва ли алтертекстът мястото, до което ви води връзката? <strong>Помислете дали да не използвате заглавието на страницата, към която води връзката, като алт текст.</strong> <hr> <strong {b}>АЛТ</strong> <strong {w}>%(ALT_TEXT)</strong>',
      LINK_IMAGE_ALT_AND_TEXT_WARNING: 'Връзката към изображението съдържа <strong>както alt текст, така и заобикалящ текст на връзката.</strong> Ако това изображение е декоративно и се използва като функционална връзка към друга страница, помислете дали да не маркирате изображението като декоративно или нулево - заобикалящият текст на връзката трябва да е достатъчен. <hr> <strong {wb}>АЛТ</strong> <strong {w}>%(ALT_TEXT)</strong> <hr> <strong {wb}>Етикет на връзката:</strong> <strong {w}>%(TEXT)</strong>',
      ...
      IMAGE_FIGURE_DUPLICATE_ALT: 'Не използвайте едни и същи думи за текста на алтер и надписа. Екранните четци ще обявят информацията два пъти.<ul><li>The alt text should provide a concise description of what is in the image.</li><li>The caption should usually provide context to relate the image back to the surrounding content, or give attention to a particular piece of information.</li></ul> Learn more: <a href="https://thoughtbot.com/blog/alt-vs-figcaption#the-figcaption-element">alt срещу figcaption.</a> <hr> <strong {wb}>АЛТ</strong> <strong {w}>%(ALT_TEXT)</strong>',
      ...
      IMAGE_ALT_TOO_LONG: 'Описанието на Alt текста е <strong>прекалено дълго</strong>. Alt текстът трябва да бъде кратък, но съдържателен като <em>tweet</em> (около 100 знака). Ако става въпрос за сложно изображение или графика, помислете дали да не поставите дългото описание на изображението в текста под него или в компонент на хармоника. <hr> <strong {wb}>АЛТ</strong> <strong {wb}>%(altLength) Символа</strong> <strong {w}>%(ALT_TEXT)</strong>',
      IMAGE_PASS: '<strong {b}>АЛТ</strong> %(ALT_TEXT)',
      ...
      LABELS_ARIA_LABEL_INPUT_MESSAGE: 'Входът има достъпно име, но се уверете, че има и видим етикет. <hr> <strong {wb}>Етикет за вход</strong> <strong {w}>%(TEXT)</strong>',
      ...
      CONTRAST_ERROR: 'Този текст не контрастира достатъчно с фона. Контрастното съотношение трябва да бъде поне 4,5:1 за нормален текст и 3:1 за голям текст. <hr> <strong {eb}>Съотношение на контраста</strong> <strong {eb}>%(RATIO)</strong> <strong {r}>%(TEXT)</strong>',
      ...
      CONTRAST_INPUT_ERROR: 'Текстът в този вход не контрастира достатъчно с фона. Съотношението на контраста трябва да е поне 4,5:1 за нормален текст и 3:1 за голям текст. <hr>  <strong {eb}>Съотношение на контраста</strong> <strong {eb}>%(RATIO)</strong>',
    },
  };
