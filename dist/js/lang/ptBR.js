...
var ptBR = {
  ...
  strings: {
    ...
    HEADING_LONG: 'O título é longo! Os títulos devem ser usados para organizar o conteúdo e transmitir estrutura. Eles devem ser breves, informativos e exclusivos. Mantenha os títulos com menos de 160 caracteres (não mais do que uma frase). <hr> Contagem de caracteres: <strong {r}>%(headingLength)</strong>',
    ...
    LINK_URL: 'URLs mais longos e menos inteligíveis usados como texto de link podem ser difíceis de ouvir com a tecnologia assistiva. Na maioria dos casos, é melhor usar texto legível por humanos em vez do URL. URLs curtos (como a página inicial de um site) são aceitáveis.<hr><strong>Dica!</strong> O texto do link deve ser sempre claro, exclusivo e significativo para que possa ser entendido fora do contexto.',
    ...
    LINK_IMAGE_BAD_ALT_MESSAGE: 'Extensão de arquivo encontrada no texto alternativo. Certifique-se de que o texto alternativo descreva o destino do link, e não uma descrição literal da imagem. Remover: <strong {r}>%(error)</strong>.<hr><strong>Texto alternativo:</strong> %(altText)',
    LINK_IMAGE_PLACEHOLDER_ALT_MESSAGE: 'Encontrado texto alternativo não descritivo ou de espaço reservado em uma imagem vinculada. Certifique-se de que o texto alternativo descreva o destino do link e não uma descrição literal da imagem. Substitua o seguinte texto alternativo: <strong {r}>%(altText)</strong>',
    LINK_IMAGE_SUS_ALT_MESSAGE: 'As tecnologias assistivas já indicam que se trata de uma imagem, portanto, &quot;<strong {r}>%(error)</strong>&quot; pode ser redundante. Certifique-se de que o texto alternativo descreva o destino do link, e não uma descrição literal da imagem. <hr> <strong>Texto alternativo:</strong> %(altText)',
    LINK_ALT_HAS_BAD_WORD_MESSAGE: 'Extensão do arquivo no texto alternativo encontrado. Se a imagem transmitir uma história, um clima ou informações importantes, não deixe de descrevê-la. Remover: <strong {r}>%(erro)</strong>.<hr><strong>Texto alternativo:</strong> %(altText)',
    ALT_PLACEHOLDER_MESSAGE: 'Texto alternativo não descritivo ou de espaço reservado encontrado. Substitua o texto alternativo a seguir por algo mais significativo: <strong {r}>%(altText)</strong>',
    ALT_HAS_SUS_WORD: 'As tecnologias assistivas já indicam que se trata de uma imagem, portanto, &quot;<strong {r}>%(error)</strong>&quot; pode ser redundante. <hr> <strong>Texto alternativo:</strong> %(altText)',
    LINK_IMAGE_ARIA_HIDDEN: 'O link ao redor da imagem tem <code>aria-hidden=&quot;true&quot;</code>, mas ainda pode ser focalizado pelo teclado. Se estiver pretendendo ocultar um link redundante ou duplicado, adicione <code>tabindex=&quot;-1&quot;</code> também.',
    ...
    LABELS_ARIA_LABEL_INPUT_MESSAGE: 'A entrada tem um nome acessível, mas certifique-se de que também haja um rótulo visível. <hr> O nome acessível para esse input é: <strong>%(sanitizedText)</strong>',
    ...
    EMBED_MISSING_TITLE: 'O conteúdo incorporado requer um nome acessível que descreva seu conteúdo. Forneça um atributo <code>title</code> ou <code>aria-label</code> exclusivo no elemento <code>iframe</code>. Saiba mais sobre <a href="https://dequeuniversity.com/tips/provide-iframe-titles">iFrames.</a>',
    ...
    QA_BLOCKQUOTE_MESSAGE: 'Isso é um título? <strong {r}>%(sanitizedText)</strong> <hr> As aspas de bloco devem ser usadas somente para citações. Se a intenção é que isso seja um título, altere essa citação de bloco para um título semântico (por exemplo, Título 2 ou Título 3).',
    QA_FAKE_HEADING: 'Isso é um título? <strong {r}>%(boldtext)</strong> <hr> Uma linha de texto em negrito ou grande pode parecer um título, mas alguém que usa um leitor de tela não pode dizer que ela é importante ou pular para o seu conteúdo. O texto em negrito ou grande nunca deve substituir os títulos semânticos (Título 2 a Título 6).',
    ...
    CONTRAST_ERROR: 'Esse texto não tem contraste suficiente com o plano de fundo. A taxa de contraste deve ser de pelo menos 4,5:1 para texto normal e 3:1 para texto grande. <hr> A taxa de contraste é <strong {r}>%(cratio)</strong> para o texto a seguir: <strong {r}>%(sanitizedText)</strong>',
    CONTRAST_WARNING: 'O contraste desse texto é desconhecido e precisa ser revisado manualmente. Certifique-se de que o texto e o plano de fundo tenham cores fortes e contrastantes. A taxa de contraste deve ser de pelo menos 4,5:1 para texto normal e 3:1 para texto grande. <hr> <strong>Por favor, revise:</strong> %(sanitizedText)',
    CONTRAST_INPUT_ERROR: 'O texto nessa entrada não tem contraste suficiente com o plano de fundo. A taxa de contraste deve ser de pelo menos 4,5:1 para texto normal e 3:1 para texto grande. <hr> Taxa de contraste: <strong {r}>%(cratio)</strong>',
  },
};
