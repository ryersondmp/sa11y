...
var nl = {
  ...
  strings: {
    ...
    LINK_IDENTICAL_NAME: 'Link heeft dezelfde tekst als een andere link, maar verwijst naar een andere pagina. Meerdere links met dezelfde tekst kunnen verwarring veroorzaken voor mensen die schermlezers gebruiken. <hr> Overweeg om de volgende link beschrijvender te maken om hem te helpen onderscheiden van andere links: <strong {C}>%(TEXT)</strong>',
    ...
    LINK_IMAGE_ALT_AND_TEXT: 'Als deze afbeelding decoratief is en wordt gebruikt als een functionele link naar een andere pagina, overweeg dan om de afbeelding als decoratief of null te markeren - de omringende linktekst zou moeten volstaan. <hr> {ALT} <strong {C}>%(ALT_TEXT)</strong> <hr> <strong {B}>Linklabel</strong> {L} <strong {C}>%(TEXT)</strong>',
    ...
    LABELS_ARIA_LABEL_INPUT: 'De invoer heeft een toegankelijke naam, maar zorg ervoor dat er ook een zichtbaar label is. <hr> <strong {B}>Invoerlabel</strong> <strong {C}>%(TEXT)</strong>',
    ...
    QA_DUPLICATE_ID: '<strong>Duplicate ID</strong> gevonden. Het is bekend dat dubbele ID-fouten problemen veroorzaken voor hulptechnologieën wanneer deze proberen te communiceren met inhoud. <hr> Verwijder of wijzig de volgende ID: <strong {C}>%(id)</strong>',
    ...
    QA_UNCONTAINED_LI: 'Alle <code>&lt;li&gt;</code> lijstitems moeten worden geplaatst binnen <code>&lt;ul&gt;</code> ongeordende of <code>&lt;ol&gt;</code> geordende elementen. Deze structuur helpt schermlezers om de lijst en de items ervan nauwkeurig aan te kondigen.',
    QA_META_SCALABLE: 'Verwijder de parameter <code>user-scalable="no"</code> in het <code>&lt;meta name="viewport"&gt;</code> element om zoomen mogelijk te maken.',
    QA_META_MAX: 'Zorg ervoor dat de parameter <code>maximum-scale</code> in het <code>&lt;meta name="viewport"&gt;</code> element niet minder is dan 2.',
    QA_JUSTIFY: 'Vermijd het gebruik van uitgelijnde tekst, die zowel aan de linker- als rechterkant is uitgelijnd. Dit kan moeilijk leesbaar zijn voor sommige mensen vanwege de ongelijke ruimtes tussen woorden. Gebruik links uitgelijnde tekst voor een betere leesbaarheid.',
