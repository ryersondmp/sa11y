...
var ko = {
  ...
  strings: {
    ...
    // Headings
    HEADING_SKIPPED_LEVEL: '비연속적인 머리글 레벨이 사용되었습니다. 제목은 레벨을 건너뛰거나 <strong>헤딩 %(level)</strong>에서 <strong {C}>헤딩 %(level)</strong>로 이동하지 않아야 합니다.',
    ...
    LINK_STOPWORD: "링크 텍스트가 문맥에 맞지 않게 충분히 설명적이지 않을 수 있습니다: <strong {C}>%(ERROR)</strong> <hr> <strong>Tip!</strong> 링크 텍스트는 항상 명확하고 고유하며 의미 있는 것이어야 합니다. '여기를 클릭하세요' 또는 '자세히 알아보기'와 같은 일반적인 단어는 피하세요",
    LINK_BEST_PRACTICES: '맥락 없이 링크 텍스트가 충분히 설명적이지 않을 수 있습니다: <strong {C}>%(ERROR)</strong> <hr> <strong>팁!</strong> 링크 텍스트는 항상 명확하고 유일하며 의미 있어야 합니다. &quot;여기를 클릭하세요&quot;나 &quot;자세히 알아보기&quot;와 같은 흔한 단어를 피하세요&quot;',
    ...
    CONTRAST_WARNING: '이 텍스트의 대비를 알 수 없으므로 수동으로 검토해야 합니다. 텍스트와 배경의 색상이 강하게 대비되는지 확인합니다. 대비 비율은 일반 텍스트의 경우 4.5:1 이상, 큰 텍스트의 경우 3:1 이상이어야 합니다. <hr> 검토해 주세요: <strong {C}>%(TEXT)</strong>',
