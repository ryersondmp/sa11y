...
(function (global, factory) {
  ...
  var tr = {
    ...
    strings: {
      ...
      HEADING_LONG: 'Başlık uzun! Başlıklar içeriği düzenlemek ve yapıyı aktarmak için kullanılmalıdır. Kısa, bilgilendirici ve benzersiz olmalıdırlar. Lütfen başlıkları 160 karakterden az tutun (bir cümleden fazla olmamalıdır). <hr> <strong {wb}>%(HEADING_LENGTH) Karakter</strong>',
      ...
      LINK_ALT_HAS_FILE_EXTENSION: 'Alt metin içinde dosya uzantısı bulundu. Alt metnin, görüntünün gerçek bir tanımını değil, bağlantının hedefini açıkladığından emin olun. Kaldırın: <strong {r}>%(ERROR)</strong>. <hr> <strong {eb}>ALT</strong> <strong {r}>%(ALT_TEXT)</strong>',
      LINK_IMAGE_PLACEHOLDER_ALT_MESSAGE: 'Bağlantılı bir resim içinde tanımlayıcı olmayan veya yer tutucu alt metin bulundu. Alt metnin, görüntünün gerçek bir tanımını değil, bağlantının hedefini açıkladığından emin olun. Aşağıdaki alt metni değiştirin: <strong {r}>%(ALT_TEXT)</strong>',
      LINK_IMAGE_SUS_ALT_MESSAGE: 'Yardımcı teknolojiler zaten bunun bir resim olduğunu gösterir, bu nedenle &quot;<strong {r}>%(ERROR)</strong>&quot; gereksiz olabilir. Alt metnin, resmin gerçek bir tanımını değil, bağlantının hedefini açıkladığından emin olun. <hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong>',
      ALT_HAS_FILE_EXTENSION: 'Bulunan alt metin içinde dosya uzantısı. Görsel bir hikaye, ruh hali veya önemli bir bilgi aktarıyorsa, görseli tanımladığınızdan emin olun. Kaldırın: <strong {r}>%(ERROR)</strong>. <hr> <strong {eb}>ALT</strong> <strong {r}>%(ALT_TEXT)</strong>',
      ALT_PLACEHOLDER_MESSAGE: 'Tanımlayıcı olmayan veya yer tutucu alt metin bulundu. Aşağıdaki alt metni daha anlamlı bir metinle değiştirin: <strong {r}>%(ALT_TEXT)</strong>',
      ALT_HAS_SUS_WORD: 'Yardımcı teknolojiler zaten bunun bir resim olduğunu belirtmektedir, bu nedenle &quot;<strong {w}>%(ERROR)</strong>&quot; gereksiz olabilir. <hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong>',
      ...
      LINK_IMAGE_LONG_ALT: 'Bağlantılı bir görseldeki alt metin açıklaması <strong>çok uzun</strong>. Bağlantılı görsellerdeki alt metin, görselin birebir açıklamasını değil, bağlantının sizi nereye götürdüğünü açıklamalıdır. <strong>Alt metin olarak bağlantı verilen sayfanın başlığını kullanmayı düşünün.</strong> <hr> <strong {wb}>ALT</strong> <strong {wb}>%(altLength) Karakter</strong> <strong {w}>%(ALT_TEXT)</strong>',
      LINK_IMAGE_ALT_WARNING: 'Resim bağlantısı alt metin içeriyor. <strong>Alt metin bağlantının sizi nereye götürdüğünü açıklıyor mu?</strong> Bağlantı verilen sayfanın başlığını alt metin olarak kullanmayı düşünün. <hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong>',
      LINK_IMAGE_ALT_AND_TEXT_WARNING: 'Resim bağlantısı <strong>hem alt metin hem de çevresindeki bağlantı metnini içerir.</strong> Bu resim dekoratifse ve başka bir sayfaya işlevsel bir bağlantı olarak kullanılıyorsa, resmi dekoratif veya boş olarak işaretlemeyi düşünün - çevresindeki bağlantı metni yeterli olmalıdır. <hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong> <hr> <strong {wb}>Etiketi</strong> <strong {w}>%(TEXT)</strong>',
      ...
      LABELS_ARIA_LABEL_INPUT_MESSAGE: 'Girdinin erişilebilir bir adı vardır, ancak lütfen görünür bir etiket olduğundan da emin olun. <hr> <strong {wb}>Giriş etiketi</strong> <strong {w}>%(TEXT)</strong>',
      ...
      CONTRAST_ERROR: 'Bu metin arka planla yeterli kontrasta sahip değil. Kontrast oranı normal metin için en az 4,5:1 ve büyük metin için 3:1 olmalıdır. <hr> <strong {eb}>Kontrast Oranı</strong> <strong {eb}>%(RATIO)</strong> <strong {r}>%(TEXT)</strong>',
      ...
      CONTRAST_INPUT_ERROR: 'Bu girişteki metin arka planla yeterli kontrasta sahip değil. Kontrast oranı normal metin için en az 4,5:1 ve büyük metin için 3:1 olmalıdır. <hr> <strong {eb}>Kontrast Oranı</strong> <strong {eb}>%(RATIO)</strong> <strong {r}>%(TEXT)</strong>',
    },
  };
