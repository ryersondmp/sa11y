...
var es = {
  ...
  strings: {
    ...
    ACC_NAME: '<strong {B}>Etiqueta de enlace</strong> %(TEXT)',
    LINK_STOPWORD: 'Es posible que el texto del enlace no sea lo suficientemente descriptivo fuera de contexto: <strong {C}>%(ERROR)</strong> <hr> <strong>¡Consejo!</strong> El texto del enlace siempre debe ser claro, único y significativo. Evite palabras comunes como &quot;haga clic aquí&quot; o &quot;más información&quot;',
    LINK_BEST_PRACTICES: 'Considere reemplazar el texto del enlace: <strong {C}>%(ERROR)</strong> <hr> <ul><li>&quot;Haga clic aquí&quot; Los lugares se enfocan en la mecánica del mouse, cuando muchas personas no usan un mouse o pueden estar viendo este sitio web en un dispositivo móvil. Considere usar un verbo diferente que se relacione con la tarea.</li><li>Evite usar símbolos HTML como llamadas a la acción, a menos que estén ocultos a las tecnologías de asistencia.</li></ul>',
    ...
    LINK_IDENTICAL_NAME: 'El enlace tiene el mismo texto que otro enlace, aunque apunta a una página diferente. Varios enlaces con el mismo texto pueden causar confusión a las personas que usan lectores de pantalla. <hr> Considere hacer el siguiente enlace más descriptivo para ayudar a distinguirlo de otros enlaces: <strong {C}>%(TEXT)</strong>',

    // Buttons
    BTN_EMPTY: 'El botón no tiene un nombre accesible que describa su propósito. Aprenda a crear un <a href="https://www.sarasoueidan.com/blog/accessible-icon-buttons/">botón accesible.</a>',
    BTN_EMPTY_LABELLEDBY: 'El botón tiene un valor <code>aria-labelledby</code> vacío o que no coincide con el valor <code>id</code> de otro elemento en la página.',
    ...
    LABELS_ARIA_LABEL_INPUT: 'La entrada tiene un nombre accesible, aunque asegúrese de que también haya una etiqueta visible. <hr> <strong {B}>Etiqueta de entrada</strong> <strong {C}>%(TEXT)</strong>',
    ...
    PAGE_LANG: '¡Idioma de la página no declarado! <a href="https://www.w3.org/International/questions/qa-html-language-declarations">declare el idioma en la etiqueta HTML.</a>',
    ...
    DUPLICATE_ID: '<strong>Identificación duplicada</strong> encontrada. Se sabe que los errores de ID duplicados causan problemas a las tecnologías de asistencia cuando intentan interactuar con el contenido. <hr> Elimine o cambie el siguiente ID: <strong {C}>%(id)</strong>',
