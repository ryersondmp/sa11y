...
(function (global, factory) {
  ...
  var zh = {
    ...
    strings: {
      ...
      MISSING_ALT_LINK_MESSAGE: '图片被用作链接, 但缺少alt文本!请确保alt文本描述了链接的位置。',
      MISSING_ALT_MESSAGE: '缺少alt文本!如果图片传达了一个故事、情绪或重要信息--一定要描述图片。',
      LINK_IMAGE_BAD_ALT_MESSAGE: '在alt文本中发现文件扩展名。确保alt文本描述链接的目的地, 而不是图片的字面描述。删除: <strong {r}>%(error)</strong>.<hr><strong>Alt text:</strong>%(altText)',
      ...
      LINK_ALT_HAS_BAD_WORD_MESSAGE: '在alt文本内找到文件扩展名。如果图片传达了一个故事、情绪或重要信息--一定要描述图片。删除: <strong {r}>%(error)</strong>.<hr><strong>Alt text:</strong>%(altText)',
      ALT_PLACEHOLDER_MESSAGE: '发现非描述性或占位符的alt文本。用更有意义的内容替换下面的alt文本: <strong {r}>%(altText)</strong>。',
      ALT_HAS_SUS_WORD: '辅助技术已经表明这是一张图片, 所以&quot;<strong {r}>%(error)</strong>&quot; 可能是多余的。<hr> <strong>Alt text:</strong> %(altText)',
      ...
      LINK_IMAGE_ALT_WARNING: '图片链接包含alt文本。符号文本是否描述了该链接的位置？可以考虑使用它所链接的页面的标题作为alt文本。 <hr> <strong>alt text:</strong> %(altText)',
      ...
      IMAGE_FIGURE_DUPLICATE_ALT: '不要在alt和标题文本中使用完全相同的词。屏幕阅读器会将信息公布两次。<ul><li>alt文本应提供对图片中内容的简明描述。</li><li>标题通常应提供背景, 将图片与周围的内容联系起来, 或对某一特定信息给予关注。</li></ul>了解更多: <a href="https://thoughtbot.com/blog/alt-vs-figcaption#the-figcaption-element">alt与figcaption.</a> <hr> <strong>Alt text:</strong> %(altText)',
