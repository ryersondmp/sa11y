...
var fr = {
  ...
  strings: {
    ...
    LINK_ALT_HAS_FILE_EXTENSION: 'L’extension du fichier trouvé à l’intérieur du texte de remplacement. Assurez-vous que le texte de remplacement décrit la destination du lien pas une description complète de l’image. Retirer: <strong {r}>%(error)</strong>.<hr><strong>Texte de remplacement: </strong> %(altText)',
    ...
    ALT_HAS_FILE_EXTENSION: 'L’extension du fichier trouvé à l’intérieur du texte de remplacement. Si l’image trans-met un message, une émotion ou une information importante - assurez-vous d’ajouter une description de l’image. Retirer: <strong {r}>%(error)</strong>.<hr><strong>Texte de remplacement:</strong> %(altText)',
    ...
    LABELS_ARIA_LABEL_INPUT_MESSAGE: 'L’entrée a un nom accessible, assurez-vouer qu’il y a aussi une étiquette visible. <hr> <strong>Étiquette d\'entrée :</strong> %(sanitizedText)',
    ...
    QA_BLOCKQUOTE_MESSAGE: 'Est-ce un en-tête?  <strong {r}>%(sanitizedText)</strong><hr>Les citations en bloc doivent être utilisées pour les citations uniquement. S’il s’agit d’un en-tête, changez cette citation en bloc pour un en-tête sémantique (ex. En-tête 2 ou En-tête 3).',
    QA_FAKE_HEADING: 'Est-ce un en-tête? <strong {r}>%(boldtext)</strong> <hr> Une ligne de texte en gras ou de grande taille peut ressembler à un titre, mais pour une personne utili-sant un lecteur d’écran, il est impossible de déterminer ce qui est important ou com-ment accéder au contenu. Un texte en gras ou de grande taille ne doit jamais remplacer un en-tête sémantique (En-tête 2 à En-tête 6).',
    ...
    // Contrast
    CONTRAST_ERROR: 'Ce texte n’est pas suffisamment contrasté avec l’arrière-plan. Le ratio du contraste devrait être au moins de 4.5:1 pour le texte normal et 3:1 pour les textes plus grands.<hr>Le ratio du contraste est de <strong {r}>%(cratio)</strong> pour le texte suivant: <strong {r}>%(sanitizedText)</strong>',
    CONTRAST_WARNING: 'Le contraste de ce texte est inconnu et doit être manuellement révisé. Assurez-vous que le texte et l’arrière-plan représentent des couleurs contrastantes. Le ratio du con-traste devrait être au moins de 4.5:1 pour les textes normaux et 3:1 pour les textes plus grands.<hr><strong>Veuillez réviser:</strong> %(sanitizedText)',
