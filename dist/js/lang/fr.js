...
var fr = {
  ...
  strings: {
    ...
    // Headings
    HEADING_SKIPPED_LEVEL: 'Utilisation d’un niveau d’en-tête non consécutif. Les en-têtes ne doivent jamais sauter de niveaux ou aller de <strong>L’en-tête %(prevLevel)</strong> à <strong {R}>L’en-tête %(level)</strong>.',
    ...
    LINK_ALT_FILE_EXT: 'L’extension du fichier trouvé à l’intérieur du texte de remplacement. Assurez-vous que le texte de remplacement décrit la destination du lien pas une description complète de l’image. Retirer: <strong {R}>%(ERROR)</strong> <hr> {ALT} {L} <strong {R}>%(ALT_TEXT)</strong>',
    LINK_PLACEHOLDER_ALT: 'Texte de remplacement non-descriptif ou séparateur trouvé dans une image. Assurez-vous que le texte de remplacement décrit la destination du lien, pas une description compète de l’image. Remplacer le texte de remplacement suivant. <hr> {ALT} {L} <strong {R}>%(ALT_TEXT)</strong>',
    LINK_SUS_ALT: 'La technologie d’assistance indiquer déjà que c’est une image, donc &quot;<strong {W}>%(ERROR)</strong>&quot; pourrait être redondant. Assurez-vous que le texte de remplacement décrit la destination du lien, pas la des-cription complète de l’image. <hr> {ALT} {L} <strong {W}>%(ALT_TEXT)</strong>',
    ALT_FILE_EXT: 'L’extension du fichier trouvé à l’intérieur du texte de remplacement. Si l’image trans-met un message, une émotion ou une information importante - assurez-vous d’ajouter une description de l’image. Retirer: <strong {R}>%(ERROR)</strong> <hr> {ALT} <strong {R}>%(ALT_TEXT)</strong>',
    ALT_PLACEHOLDER: 'Texte de remplacement non-descriptif ou séparateur trouvé dans une image. Modifier le texte de remplacement suivant pour le rendre plus significatif. <hr> {ALT} <strong {R}>%(ALT_TEXT)</strong>',
    SUS_ALT: 'Les technologies d’assistances indiquent déjà qu’il s’agit d’une image, donc &quot;<strong {W}>%(ERROR)</strong>&quot; peut être redondant. <hr> {ALT} <strong {W}>%(ALT_TEXT)</strong>',
    ...
    LINK_IMAGE_ALT: 'Le lien vers l\'image contient un texte de remplacement. <strong>Le texte de remplacement décrit- il la destination du lien?</strong> Pensez à utiliser le titre de la page à laquelle le lien renvoie comme texte de remplacement. <hr> {ALT} {L} <strong {W}>%(ALT_TEXT)</strong>',
    ...
    LABELS_ARIA_LABEL_INPUT: 'L’entrée a un nom accessible, assurez-vouer qu’il y a aussi une étiquette visible. <hr> <strong {B}>Étiquette d\'entrée</strong> <strong {W}>%(TEXT)</strong>',
    ...
    QA_PAGE_LANG: 'La langue de la page n’est pas indiquée! Veuillez <a href="https://www.w3.org/International/questions/qa-html-language-declarations.fr">indiquer la langue sur la balise HTML.</a>',
    ...
    QA_BLOCKQUOTE: 'Est-ce un en-tête? <strong {W}>%(TEXT)</strong> <hr> Les citations en bloc doivent être utilisées pour les citations uniquement. S’il s’agit d’un en-tête, changez cette citation en bloc pour un en-tête sémantique (ex. En-tête 2 ou En-tête 3).',
    ...
    CONTRAST_INPUT: 'Le texte à l’intérieur de cette entrée n’est pas suffisamment contrasté avec l’arrière-plan. Le ratio du contraste devraient être au moins de 4.5:1 pour le texte normal et 3:1 pour les textes plus grands. <hr> <strong {B}>Ratio de contraste</strong> <strong {B}>%(RATIO)</strong>',
  },
};
