...
(function (global, factory) {
  ...
  var ua = {
    ...
    strings: {
      ...
      DEVELOPER_DESC: 'Перевіряє проблеми, які можуть вимагати знання програмування для виправлення.',
      ...
      // Headings
      HEADING_SKIPPED_LEVEL: 'Використано непослідовні рівні заголовків. Заголовки ніколи не повинні пропускати рівні або переходити від <strong>заголовка %(prevLevel)</strong> до <strong {C}>заголовка %(level)</strong>.',
      ...
      LABELS_ARIA_LABEL_INPUT: 'Введення має доступне ім\'я, проте, будь ласка, переконайтеся, що у нього є видимий підпис. <hr> <strong {B}>Етикетка для введення</strong> <strong {C}>%(TEXT)</strong>',
      ...
      // Quality assurance
      QA_BAD_LINK: 'Знайдено неякісне посилання. Схоже, що посилання вказує на середовище розробки. <hr> Це посилання вказує на: <br> <strong {C}>%(LINK)</strong>',
      ...
      QA_DUPLICATE_ID: 'Виявлено <strong>дублікат ID</strong>. Відомо, що помилки дублювання ID можуть викликати проблеми для допоміжних технологій, коли вони намагаються взаємодіяти із вмістом. <hr> Будь ласка, видаліть або змініть наступний ідентифікатор: <strong {C}>%(id)</strong>',
      ...
      QA_UNCONTAINED_LI: 'Всі елементи списку <code>&lt;li&gt;</code> повинні бути розміщені всередині елементів <code>&lt;ul&gt;</code> неупорядкованих або <code>&lt;ol&gt;</code> упорядкованих. Ця структура допомагає читачам екрана точно оголошувати список та його елементи.',
      QA_META_SCALABLE: 'Видаліть параметр <code>user-scalable="no"</code> з елемента <code>&lt;meta name="viewport"&gt;</code>, щоб дозволити масштабування.',
      QA_META_MAX: 'Переконайтеся, що параметр <code>maximum-scale</code> в елементі <code>&lt;meta name="viewport"&gt;</code> не менше ніж 2.',
      QA_JUSTIFY: 'Уникайте використання вирівняного тексту, який вирівнюється як з лівим, так і з правим краєм. Це може бути важко для деяких людей через нерівномірні проміжки між словами. Використовуйте текст, вирівняний зліва, для кращої читабельності.',
      ...
      // Contrast
      CONTRAST_ERROR: 'Цей текст недостатньо контрастний щодо фону. Коефіцієнт контрастності має бути не менше 4,5:1 для звичайного тексту та 3:1 для великого тексту. <hr> <strong {B}>Контрастне відношення</strong> <strong {B}>%(RATIO)</strong> <strong {C}>%(TEXT)</strong>',
      CONTRAST_WARNING: 'Контрастність цього тексту не встановлена і потребує перевірки вручну. Переконайтеся, що текст і фон мають різко контрастні кольори. Коефіцієнт контрастності має бути не менше 4,5:1 для звичайного тексту та 3:1 для великого тексту. <hr> Будь ласка, перевірте: <strong {C}>%(TEXT)</strong>',
