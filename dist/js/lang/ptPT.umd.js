...
(function (global, factory) {
  ...
  var ptPT = {
    ...
    strings: {
      ...
      HEADING_LONG: 'O título é longo! Os títulos devem ser utilizados para organizar o conteúdo e transmitir estrutura. Devem ser breves, informativos e únicos. Os títulos devem ter menos de 160 caracteres (não mais do que uma frase). <hr> <strong {wb}>%(HEADING_LENGTH) Caracteres</strong>',
      ...
      LINK_ALT_HAS_FILE_EXTENSION: 'Extensão de ficheiro no texto alternativo encontrada. Certifique-se de que o texto alternativo descreve o destino da hiperligação e não uma descrição literal da imagem. Remover: <strong {r}>%(ERROR)</strong>. <hr> <strong {eb}>ALT</strong> <strong {r}>%(ALT_TEXT)</strong>',
      LINK_IMAGE_PLACEHOLDER_ALT_MESSAGE: 'Encontrado texto alternativo não descritivo ou de espaço reservado numa imagem ligada. Certifique-se de que o texto alternativo descreve o destino da hiperligação e não uma descrição literal da imagem. Substitua o seguinte texto alternativo: <strong {r}>%(ALT_TEXT)</strong>',
      LINK_IMAGE_SUS_ALT_MESSAGE: 'As tecnologias de assistência já indicam que se trata de uma imagem, pelo que &quot;<strong {w}>%(ERROR)</strong>&quot; pode ser redundante. Certifique-se de que o texto alternativo descreve o destino da hiperligação e não uma descrição literal da imagem. <hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong>',
      ALT_HAS_FILE_EXTENSION: 'Extensão do ficheiro no texto alternativo encontrado. Se a imagem transmitir uma história, um estado de espírito ou uma informação importante, não se esqueça de a descrever. Remover: <strong {r}>%(ERROR)</strong>. <hr> <strong {eb}>ALT</strong> <strong {r}>%(ALT_TEXT)</strong>',
      ALT_PLACEHOLDER_MESSAGE: 'Encontrado texto alternativo não descritivo ou de espaço reservado. Substitua o seguinte texto alternativo por algo mais significativo: <strong {r}>%(ALT_TEXT)</strong>',
      ALT_HAS_SUS_WORD: 'As tecnologias de assistência já indicam que se trata de uma imagem, pelo que &quot;<strong {w}>%(ERROR)</strong>&quot; pode ser redundante. <hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong>',
      ...
      LINK_IMAGE_LONG_ALT: 'A descrição do texto alternativo numa imagem ligada é <strong>muito longa</strong>. O texto alternativo em imagens vinculadas deve descrever para onde o link o leva, não uma descrição literal da imagem. <strong>Considere usar o título da página para a qual o link leva como o texto alternativo.</strong> <hr> <strong {wb}>ALT</strong> <strong {wb}>%(altLength) Caracteres</strong> <strong {w}>%(ALT_TEXT)</strong>',
      LINK_IMAGE_ALT_WARNING: 'A hiperligação da imagem contém texto alternativo. <strong>O texto alternativo descreve para onde o link o leva?</strong> Considere a possibilidade de utilizar o título da página para a qual a ligação remete como texto alternativo. <hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong>',
      LINK_IMAGE_ALT_AND_TEXT_WARNING: 'Se esta imagem for decorativa e estiver a ser utilizada como uma ligação funcional a outra página, considere marcar a imagem como decorativa ou nula - o texto da ligação circundante deve ser suficiente. <hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong> <hr> <strong {wb}>Rótulo</strong> <strong {w}>%(TEXT)</strong>',
      ...
      IMAGE_ALT_TOO_LONG: 'A descrição do texto alternativo é <strong>muito longa</strong>. O texto alternativo deve ser conciso, mas significativo, como um <em>tweet</em> (cerca de 100 caracteres). Se esta for uma imagem complexa ou um gráfico, considere colocar a descrição longa da imagem no texto abaixo ou num componente de acordeão. <hr> <strong {wb}>ALT</strong> <strong {wb}>%(altLength) Caracteres</strong> <strong {w}>%(ALT_TEXT)</strong>',
      ...
      LABELS_ARIA_LABEL_INPUT_MESSAGE: 'A entrada tem um nome acessível, mas certifique-se de que também existe uma etiqueta visível. <hr> <strong {wb}>Rótulo</strong> <strong {w}>%(TEXT)</strong>',
      ...
      CONTRAST_ERROR: 'Este texto não tem contraste suficiente com o fundo. A relação de contraste deve ser de pelo menos 4,5:1 para texto normal e 3:1 para texto grande. <hr> <strong {eb}>Razão de contraste</strong> <strong {eb}>%(RATIO)</strong> <strong {r}>%(TEXT)</strong>',
      ...
      CONTRAST_INPUT_ERROR: 'O texto nesta entrada não tem contraste suficiente com o fundo. O rácio de contraste deve ser de, pelo menos, 4,5:1 para texto normal e 3:1 para texto grande. <hr> <strong {eb}>Razão de contraste</strong> <strong {eb}>%(RATIO)</strong>',
    },
  };
