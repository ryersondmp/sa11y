...
(function (global, factory) {
  ...
  var hu = {
    ...
    strings: {
      ...
      LINK_IDENTICAL_NAME: 'A link szövege megegyezik egy másik link szövegével, bár egy másik oldalra mutat. A több azonos szövegű hivatkozás zavart okozhat a képernyőolvasót használó emberek számára.<hr>Figyeljen arra, hogy a következő hivatkozás leíróbb legyen, hogy jobban megkülönböztethető legyen a többi hivatkozástól: <strong {C}>%(TEXT)</strong>',
      ...
      LINK_IMAGE_ALT_AND_TEXT: 'A kép linkje <strong>mind az alt szöveget, mind a környező linkszöveget tartalmazza.</strong> Ha ez a kép dekoratív, és egy másik oldalra mutató funkcionális linkként használják, fontolja meg a kép dekoratív vagy nullás megjelölését - a környező linkszövegnek elegendőnek kell lennie. <hr> {ALT} <strong {C}>%(ALT_TEXT)</strong> <hr> <strong {B}>Link címke</strong> {L} <strong {C}>%(TEXT)</strong>',
      ...
      LABELS_ARIA_LABEL_INPUT: 'A bemenetnek elérhető neve van, de kérjük, gondoskodjon arról, hogy a címke is látható legyen. <hr> <strong {B}>Bemeneti címke</strong> <strong {C}>%(TEXT)</strong>',
      ...
      QA_DUPLICATE_ID: 'Találtunk <strong>duplikált azonosítót</strong>. A duplikált azonosító hibák köztudottan problémákat okoznak a segítő technológiák számára, amikor azok megpróbálnak interakcióba lépni a tartalommal. <hr> Kérjük, távolítsa el vagy változtassa meg a következő azonosítót: <strong {C}>%(id)</strong>',
      ...
      QA_UNCONTAINED_LI: 'Minden <code>&lt;li&gt;</code> listatételt a <code>&lt;ul&gt;</code> rendezetlen vagy <code>&lt;ol&gt;</code> rendezett elemek belsejébe kell elhelyezni. Ez a struktúra segít a képernyőolvasóknak pontosan bejelenteni a listát és annak elemeit.',
      QA_META_SCALABLE: 'Távolítsa el a <code>user-scalable="no"</code> paramétert a <code>&lt;meta name="viewport"&gt;</code> elemből a nagyítás engedélyezése érdekében.',
      QA_META_MAX: 'Győződjön meg arról, hogy a <code>maximum-scale</code> paraméter a <code>&lt;meta name="viewport"&gt;</code> elemben nem kisebb, mint 2.',
      QA_JUSTIFY: 'Kerülje az igazított szöveg használatát, amely mind a bal, mind a jobb margóra igazodik. Ez nehezen olvasható lehet egyes emberek számára a szavak közötti egyenetlen térközök miatt. Használjon balra igazított szöveget a jobb olvashatóság érdekében.',
