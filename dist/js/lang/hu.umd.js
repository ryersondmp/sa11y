...
(function (global, factory) {
  ...
  var hu = {
    ...
    strings: {
      ...
      HEADING_SKIPPED_LEVEL: 'Nem egymást követő címsorszintet használtak. A fejlécek soha nem ugorhatnak szinteket, vagy nem mehetnek <strong>Felirat %(prevLevel)</strong>-ről <strong {R}>Felirat %(level)</strong>-ra.',
      ...
      LINK_ALT_FILE_EXT: 'Fájlkiterjesztés az alt szövegben található. Győződjön meg róla, hogy az alt szöveg a link célját írja le, nem pedig a kép szó szerinti leírását. Távolítsa el: <strong {R}>%(ERROR)</strong> <hr> {ALT} {L} <strong {R}>%(ALT_TEXT)</strong>',
      LINK_PLACEHOLDER_ALT: 'Nem leíró vagy helyőrző alt szöveg a linkelt képen belül. Győződjön meg róla, hogy az alt szöveg a link célját írja le, nem pedig a kép szó szerinti leírását. Cserélje ki a következő alt szöveget. <hr> {ALT} {L} <strong {R}>%(ALT_TEXT)</strong>',
      LINK_SUS_ALT: 'A segítő technológiák már jelzik, hogy ez egy kép, így a &quot;<strong {W}>%(ERROR)</strong>&quot; felesleges lehet. Győződjön meg róla, hogy az alt szöveg a link célját írja le, nem pedig a kép szó szerinti leírását. <hr> {ALT} {L} <strong {W}>%(ALT_TEXT)</strong>',
      ALT_FILE_EXT: 'Fájlkiterjesztés az alt szövegben található. Ha a kép történetet, hangulatot vagy fontos információt közvetít - mindenképpen írja le a képet. Távolítsa el: <strong {R}>%(ERROR)</strong> <hr> {ALT} <strong {R}>%(ALT_TEXT)</strong>',
      ALT_PLACEHOLDER: 'Nem leíró vagy helyőrző alt szöveg található. Cserélje ki a következő alt szöveget valami értelmesebbre. <hr> {ALT} <strong {R}>%(ALT_TEXT)</strong>',
      SUS_ALT: 'A segítő technológiák már jelzik, hogy ez egy kép, így a &quot;<strong {W}>%(ERROR)</strong>&quot; felesleges lehet. <hr> {ALT} <strong {W}>%(ALT_TEXT)</strong>',
      ...
      LINK_IMAGE_ALT: 'A kép linkje alt szöveget tartalmaz. Az alt szöveg leírja, hogy hová vezet a link? <strong>Figyeljen arra, hogy alt szövegként annak az oldalnak a címét használja, amelyre a link mutat.</strong> <hr> {ALT} {L} <strong {W}>%(ALT_TEXT)</strong>',
      LINK_IMAGE_ALT_AND_TEXT: 'A kép linkje <strong>mind az alt szöveget, mind a környező linkszöveget tartalmazza.</strong> Ha ez a kép dekoratív, és egy másik oldalra mutató funkcionális linkként használják, fontolja meg a kép dekoratív vagy nullás megjelölését - a környező linkszövegnek elegendőnek kell lennie. <hr> {ALT} <strong {W}>%(ALT_TEXT)</strong> <hr> <strong {B}>Link címke</strong> {L} <strong {W}>%(TEXT)</strong>',
      ...
      LABELS_ARIA_LABEL_INPUT: 'A bemenetnek elérhető neve van, de kérjük, gondoskodjon arról, hogy a címke is látható legyen. <hr> <strong {B}>Bemeneti címke</strong> <strong {W}>%(TEXT)</strong>',
      ...
      QA_PAGE_LANG: 'Az oldal nyelve nincs megadva! Kérjük, <a href="https://www.w3.org/International/questions/qa-html-language-declarations">declare language on HTML tag.</a>',
      ...
      QA_BLOCKQUOTE: 'Ez egy címszó? <strong {W}>%(TEXT)</strong> <hr> A blokkos idézőjeleket csak idézőjeleknél kell használni. Ha ez egy fejlécnek szánja, akkor ezt a blokkidézetet változtassa meg egy szemantikus fejlécre (pl. Fejléc 2 vagy Fejléc 3).',
