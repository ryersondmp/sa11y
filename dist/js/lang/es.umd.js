...
(function (global, factory) {
  ...
  var es = {
    ...
    strings: {
      ...
      LINK_IMAGE_BAD_ALT_MESSAGE: 'Extensión de archivo dentro del texto alternativo encontrado. Asegúrese de que el texto alternativo describa el destino del enlace, no una descripción literal de la imagen. Eliminar: <strong {r}>%(error)</strong>. <hr> <strong>Texto alternativo:</strong> %(altText)',
      LINK_IMAGE_PLACEHOLDER_ALT_MESSAGE: 'Se encontró texto alternativo no descriptivo o de marcador de posición dentro de una imagen vinculada. Asegúrese de que el texto alternativo describa el destino del enlace, no una descripción literal de la imagen. Reemplace el siguiente texto alternativo: <strong {r}>%(altText)</strong>',
      LINK_IMAGE_SUS_ALT_MESSAGE: 'Las tecnologías de asistencia ya indican que se trata de una imagen, por lo que &quot;<strong {r}>%(error)</strong>&quot; puede ser redundante. Asegúrese de que el texto alternativo describa el destino del enlace, no una descripción literal de la imagen. <hr> <strong>Texto alternativo:</strong> %(altText)',
      LINK_ALT_HAS_BAD_WORD_MESSAGE: 'Extensión de archivo dentro del texto alternativo encontrado. Si la imagen transmite una historia, un estado de ánimo o información importante, asegúrese de describir la imagen. Eliminar: <strong {r}>%(error)</strong>. <hr> <strong>Texto alternativo:</strong> %(altText)',
      ALT_PLACEHOLDER_MESSAGE: 'Se encontró texto alternativo no descriptivo o de marcador de posición. Reemplace el siguiente texto alternativo con algo más significativo: <strong {r}>%(altText)</strong>',
      ALT_HAS_SUS_WORD: 'Las tecnologías de asistencia ya indican que se trata de una imagen, por lo que &quot;<strong {r}>%(error)</strong>&quot; puede ser redundante. <hr> <strong>Texto alternativo:</strong> %(altText)',
      LINK_IMAGE_ARIA_HIDDEN: 'El enlace alrededor de la imagen tiene <code>aria-hidden=&quot;true&quot;</code> pero aún se puede enfocar con el teclado. Si tiene la intención de ocultar un enlace redundante o duplicado, agregue <code>tabindex=&quot;-1&quot;</code> también.',
      ...
      LINK_IMAGE_LONG_ALT: 'La descripción del texto alternativo en una imagen vinculada es <strong>demasiado larga</strong>. El texto alternativo en las imágenes vinculadas debe describir a dónde lo lleva el enlace, no una descripción literal de la imagen. <strong>Considere usar el título de la página a la que enlaza como texto alternativo.</strong> <hr> <strong>Texto alternativo (<span {r}>%(altLength)</span> caracteres):</strong> %(altText)',
      LINK_IMAGE_ALT_WARNING: 'El enlace de la imagen contiene un texto alternativo. <strong>¿Describe el texto alternativo a dónde lleva el enlace?</strong> Considere la posibilidad de utilizar el título de la página a la que enlaza como texto alternativo. <hr> <strong>Texto alternativo:</strong> %(altText)',
      LINK_IMAGE_ALT_AND_TEXT_WARNING: 'El enlace de la imagen contiene <strong>texto alternativo y texto del enlace que la rodea.</strong> Si esta imagen es decorativa y se usa como un enlace funcional a otra página, considere marcar la imagen como decorativa o nula; el texto del enlace que la rodea debería ser suficiente. <hr> <strong>Texto alternativo:</strong> %(altText)',
      ...
      LABELS_ARIA_LABEL_INPUT_MESSAGE: 'La entrada tiene un nombre accesible, aunque asegúrese de que también haya una etiqueta visible. <hr> El nombre accesible para esta entrada es: <strong>%(sanitizedText)</strong>',
      ...
      EMBED_MISSING_TITLE: 'El contenido incrustado requiere un nombre accesible que describa su contenido. Proporcione un atributo único <code>title</code> o <code>aria-label</code> en el elemento <code>iframe</code>. Más información sobre <a href="https://dequeuniversity.com/tips/provide-iframe-titles">iFrames.</a>',
      ...
      // Quality assurance
      QA_BAD_LINK: 'Enlace incorrecto encontrado. El enlace parece apuntar a un entorno de desarrollo. <hr> Este enlace apunta a: <br> <strong {r}>%(el)</strong>',
      ...
      QA_BLOCKQUOTE_MESSAGE: '¿Es esto un encabezado? <strong {r}>%(sanitizedText)</strong> <hr> Las comillas en bloque deben usarse solo para comillas. Si pretende ser un encabezado, cambie esta cita en bloque a un encabezado semántico (por ejemplo, Título 2 o Título 3).',
      QA_FAKE_HEADING: '¿Es esto un encabezado? <strong {r}>%(boldtext)</strong> <hr> Una línea de texto en negrita o grande puede parecer un encabezado, pero alguien que usa un lector de pantalla no puede decir que es importante o saltar a su contenido. El texto en negrita o grande nunca debe reemplazar los encabezados semánticos (Título 2 a Título 6).',
      ...
      // Contrast
      CONTRAST_ERROR: 'Este texto no tiene suficiente contraste con el fondo. La relación de contraste debe ser de al menos 4,5:1 para texto normal y 3:1 para texto grande. <hr> La relación de contraste es <strong {r}>%(cratio)</strong> para el siguiente texto: <strong {r}>%(sanitizedText)</strong>',
      CONTRAST_WARNING: 'Se desconoce el contraste de este texto y debe revisarse manualmente. Asegúrese de que el texto y el fondo tengan colores fuertes que contrasten. La relación de contraste debe ser de al menos 4,5:1 para texto normal y 3:1 para texto grande. <hr> <strong>Revise:</strong> %(sanitizedText)',
      CONTRAST_INPUT_ERROR: 'El texto dentro de esta entrada no tiene suficiente contraste con el fondo. La relación de contraste debe ser de al menos 4,5:1 para texto normal y 3:1 para texto grande. <hr> Relación de contraste: <strong {r}>%(cratio)</strong>',
    },
  };
