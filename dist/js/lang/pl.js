...
var pl = {
  ...
  strings: {
    ...
    SETTINGS: 'Ustawienia',
    ADVANCED: 'Zaawansowane kontrole',
    ...
    // Headings
    HEADING_SKIPPED_LEVEL: 'Niespójny poziom nagłówka. Zastosowane poziomy nie następują po sobie. Nagłówki nigdy nie powinny pomijać poziomów np. od <strong>Nagłówek %(prevLevel)</strong> do <strong {R}>Nagłówek %(level)</strong>.',
    ...
    LINK_ALT_FILE_EXT: 'W tekście alt znaleziono rozszerzenie nazwy pliku. Upewnij się, że tekst alternatywny opisuje miejsce docelowe łącza, a nie treść lub wygląd obrazu. Usuń wyraz(y): <strong {R}>%(ERROR)</strong> <hr> {ALT} {L} <strong {R}>%(ALT_TEXT)</strong>',
    LINK_PLACEHOLDER_ALT: 'Znaleziono nieopisowy lub zastępczy tekst alt w obrazie będącym łączem. Upewnij się, że tekst alternatywny opisuje miejsce docelowe łącza, a nie treść lub wygląd obrazu. Zastąp następujący tekst alt. <hr> {ALT} {L} <strong {R}>%(ALT_TEXT)</strong>.',
    LINK_SUS_ALT: 'Technologie wspomagające już wskazują, że jest to obraz, więc &quot;<strong {W}>%(ERROR)</strong>&quot; mogą być zbędne. Upewnij się, że tekst alternatywny opisuje miejsce docelowe łącza, a nie treść lub wygląd obrazu. <hr> {ALT} {L} <strong {W}>%(ALT_TEXT)</strong>',
    ALT_FILE_EXT: 'W tekście alt znaleziono rozszerzenie nazwy pliku. Upewnij się, że tekst alternatywny opisuje miejsce docelowe łącza, a nie treść lub wygląd obrazu. Usuń wyraz(y): <strong {R}>%(ERROR)</strong> <hr> {ALT} <strong {W}>%(ALT_TEXT)</strong>',
    ALT_PLACEHOLDER: 'Znaleziono nieopisowy lub zastępczy tekst alt. Zamień poniższy tekst alt na coś bardziej znaczącego. <hr> {ALT} <strong {R}>%(ALT_TEXT)</strong>.',
    SUS_ALT: 'Technologie wspomagające już wskazują, że jest to obraz, więc &quot;<strong {R}>%(ERROR)</strong>&quot; mogą być zbędne. <hr> {ALT} <strong {W}>%(ALT_TEXT)</strong>',
    ...
    LINK_IMAGE_ALT: 'Link do obrazu zawiera tekst alternatywny. <strong>Czy tekst alternatywny opisuje, dokąd prowadzi link?</strong> Rozważ użycie tytułu strony, do której prowadzi link, jako tekstu alternatywnego. <hr> {ALT} {L} <strong {W}>%(ALT_TEXT)</strong>',
    ...
    LABELS_ARIA_LABEL_INPUT: 'Pole danych ma dostępną nazwę (etykietę), ale upewnij się, że etykieta jest również widoczna. <hr> <strong {B}>Etykieta wejścia</strong> <strong {W}>%(TEXT)</strong>',
    ...
    QA_PAGE_LANG: 'Język strony nie zadeklarowany! <a href="https://www.w3.org/International/questions/qa-html-language-declarations">Zadeklaruj język w znaczniku HTML.</a>',
    ...
    QA_BLOCKQUOTE: 'Czy to jest nagłówek? <strong {W}>%(TEXT)</strong> <hr> Element blockquote powinien być używany tylko do cytatów. Jeśli ma to być nagłówek, zmień ten blockquote na nagłówek semantyczny (np. Nagłówek H2 lub Nagłówek H3).',
