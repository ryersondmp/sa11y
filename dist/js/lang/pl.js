...
var pl = {
  ...
  strings: {
    ...
    LINK_ALT_HAS_FILE_EXTENSION: 'W tekście alt znaleziono rozszerzenie nazwy pliku. Upewnij się, że tekst alternatywny opisuje miejsce docelowe łącza, a nie treść lub wygląd obrazu. Usuń wyraz(y): <strong {r}>%(ERROR)</strong>. <hr> <strong {eb}>ALT</strong> <strong {r}>%(ALT_TEXT)</strong>',
    LINK_IMAGE_PLACEHOLDER_ALT_MESSAGE: 'Znaleziono nieopisowy lub zastępczy tekst alt w obrazie będącym łączem. Upewnij się, że tekst alternatywny opisuje miejsce docelowe łącza, a nie treść lub wygląd obrazu. Zastąp następujący tekst alt: <strong {r}>%(ALT_TEXT)</strong>.',
    LINK_IMAGE_SUS_ALT_MESSAGE: 'Technologie wspomagające już wskazują, że jest to obraz, więc &quot;<strong {w}>%(ERROR)</strong>&quot; mogą być zbędne. Upewnij się, że tekst alternatywny opisuje miejsce docelowe łącza, a nie treść lub wygląd obrazu. <hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong>',
    ALT_HAS_FILE_EXTENSION: 'W tekście alt znaleziono rozszerzenie nazwy pliku. Upewnij się, że tekst alternatywny opisuje miejsce docelowe łącza, a nie treść lub wygląd obrazu. Usuń wyraz(y): <strong {r}>%(ERROR)</strong>. <hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong>',
    ALT_PLACEHOLDER_MESSAGE: 'Znaleziono nieopisowy lub zastępczy tekst alt. Zamień poniższy tekst alt na coś bardziej znaczącego: <strong {r}>%(ALT_TEXT)</strong>.',
    ALT_HAS_SUS_WORD: 'Technologie wspomagające już wskazują, że jest to obraz, więc &quot;<strong {r}>%(ERROR)</strong>&quot; mogą być zbędne. <hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong>',
    ...
    IMAGE_FIGURE_DUPLICATE_ALT: 'Nie używaj dokładnie tych samych słów dla tekstu alternatywnego i podpisu. Czytniki ekranu podadzą informację dwukrotnie. <ul><li>Tekst alternatywny powinien zawierać zwięzły opis tego, co znajduje się na obrazku.</li><li>Podpis powinien zwykle zawierać kontekst, aby powiązać obraz z otaczającą zawartością lub zwracać uwagę na konkretną informację.</li></ul> Ucz się więcej: <a href="https://thoughtbot.com/blog/alt-vs-figcaption#the-figcaption-element">alternatywny kontra podpis graficzny.</a> <hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong>',
    ...
    IMAGE_ALT_TOO_LONG: 'Tekst alternatywny obrazu jest <strong>zbyt długi</strong>. Tekst alternatywny powinien być zwięzły, ale znaczący jak <em>tweet</em> (około 100 znaków). Jeśli jest to złożony obraz lub wykres, należy rozważyć umieszczenie długiego opisu obrazu w tekście poniżej lub w akordeonie. <hr> <strong {wb}>ALT</strong> <strong {wb}>%(altLength) Znaków</strong> <strong {w}>%(ALT_TEXT)</strong>',
    ...
    LABELS_ARIA_LABEL_INPUT_MESSAGE: 'Pole danych ma dostępną nazwę (etykietę), ale upewnij się, że etykieta jest również widoczna. <hr> <strong {wb}>Etykieta wejścia</strong> <strong {w}>%(TEXT)</strong>',
