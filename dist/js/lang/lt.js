...
var lt = {
  ...
  strings: {
    ...
    SETTINGS: 'Nustatymai',
    ADVANCED: 'Išplėstiniai patikrinimai',
    ...
    HEADING_SKIPPED_LEVEL: 'Naudojamas ne nuoseklus antraštės lygis. Antraštės niekada neturėtų praleisti lygių arba pereiti iš <strong>Pavadinimo %(prevLevel)</strong> į <strong {R}>Pavadinimą %(level)</strong>.',
    ...
    LINK_ALT_FILE_EXT: 'Rastas failo plėtinys alt tekste. Užtikrinkite, kad alt tekstas apibūdintų nuorodos paskirties vietą, o ne pažodinį paveikslėlio aprašymą. Pašalinti: <strong {R}>%(ERROR)</strong>.<hr> {ALT} {L} <strong {R}>%(ALT_TEXT)</strong>',
    LINK_PLACEHOLDER_ALT: 'Rastas ne aprašomasis arba pakaitinis alt tekstas susietame paveikslėlyje. Užtikrinkite, kad alt tekstas apibūdintų nuorodos paskirties vietą, o ne pažodinį paveikslėlio aprašymą. Pakeiskite šį alt tekstą. <hr> {ALT} {L} <strong {R}>%(ALT_TEXT)</strong>',
    LINK_SUS_ALT: 'Pagalbinėse technologijose jau nurodoma, kad tai yra paveikslėlis, todėl &quot;<strong {W}>%(ERROR)</strong>&quot; gali būti nereikalingas. Užtikrinkite, kad alt tekstas apibūdintų nuorodos paskirties vietą, o ne pažodinį paveikslėlio aprašymą. <hr> {ALT} {L} <strong {W}>%(ALT_TEXT)</strong>',
    ALT_FILE_EXT: 'Rastas failo plėtinys alt tekste. Jei vaizdas perteikia istoriją, nuotaiką ar svarbią informaciją, būtinai aprašykite vaizdą. Pašalinti: <strong {R}>%(ERROR)</strong> <hr> {ALT} <strong {R}>%(ALT_TEXT)</strong>',
    ALT_PLACEHOLDER: 'Rastas neaprašytas arba pakaitinis alt tekstas. Pakeiskite šį alt tekstą į prasmingesnį. <hr> {ALT} <strong {R}>%(ALT_TEXT)</strong>',
    SUS_ALT: 'Pagalbinėse technologijose jau nurodoma, kad tai yra paveikslėlis, todėl &quot;<strong {W}>%(ERROR)</strong>&quot; gali būti nereikalingas. <hr> {ALT} <strong {W}>%(ALT_TEXT)</strong>',
    ...
    LINK_IMAGE_ALT: 'Paveikslėlio nuorodoje yra teksto alt. <strong>Ar alt tekste aprašoma, kur nukreipia nuoroda?</strong> Apsvarstykite galimybę kaip alt tekstą naudoti puslapio, į kurį nukreipia nuoroda, pavadinimą. <hr> {ALT} {L} <strong {W}>%(ALT_TEXT)</strong>',
    ...
    LABELS_ARIA_LABEL_INPUT: 'Įvestis turi prieinamą pavadinimą, tačiau užtikrinkite, kad būtų matoma ir etiketė. <hr> <strong {B}>Įvesties žymė</strong> <strong {W}>%(TEXT)</strong>',
    ...
    QA_PAGE_LANG: 'Puslapio kalba nedeklaruota! Prašome <a href="https://www.w3.org/International/questions/qa-html-language-declarations">deklaruoti kalbą HTML žymėje.</a>',
    ...
    QA_BLOCKQUOTE: 'Ar tai antraštė? <strong {W}>%(TEXT)</strong> <hr> Blokinės kabutės turėtų būti naudojamos tik citatoms. Jei tai turi būti antraštė, pakeiskite šią blokinę kabutę į semantinę antraštę (pvz., 2 arba 3 antraštę).',
