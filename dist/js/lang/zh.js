...
var zh = {
  ...
  strings: {
    ...
    ACC_NAME: '<strong>链接标签:</strong> %(TEXT)',
    ...
    LINK_IDENTICAL_NAME: '链接的文字与另一个链接相同, 但它指向不同的页面。<hr> 考虑使以下链接更具描述性, 以帮助将其与其他链接区分开来: <strong {C}>%(TEXT)</strong>',
    BTN_EMPTY: '按钮缺少描述其用途的可访问名称。了解如何制作<a href="https://www.sarasoueidan.com/blog/accessible-icon-buttons/">可访问按钮。</a>',
    BTN_EMPTY_LABELLEDBY: '按钮的<code>aria-labelledby</code>值为空或与页面上其他元素的<code>id</code>值不匹配。',
    ...
    LINK_IMAGE_ALT_AND_TEXT: '图片链接包含<strong>alt文本和周围的链接文本。</strong>如果该图片是装饰性的, 并被用作另一个页面的功能链接, 请考虑将该图片标记为装饰性或无效--周围的链接文本应该足够了。<hr> {ALT} <strong {C}>%(ALT_TEXT)</strong> <hr> <strong>链接标签:</strong> {L} <strong {C}>%(TEXT)</strong>',
    ...
    LABELS_ARIA_LABEL_INPUT: '输入有一个无障碍名称, 但请确保也有一个可见的标签。<hr> <strong {B}>输入标签</strong> <strong {C}>%(TEXT)</strong>',
    ...
    PAGE_LANG: '页面语言未声明!请<a href="https://www.w3.org/International/questions/qa-html-language-declarations">在HTML标签上声明语言。</a>',
