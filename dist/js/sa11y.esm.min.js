...
function je(t, e, n) {
  We();
  const s = document.querySelector("sa11y-control-panel").shadowRoot, o = s.getElementById("panel-alert"), a = s.getElementById("panel-alert-text"), i = s.getElementById("panel-alert-preview"), l = s.getElementById("close-alert"), c = s.getElementById("skip-button");
  ...
  l.addEventListener("click", r), o.onkeydown = (d) => {
    (d || window.event).key === "Escape" && o.classList.contains("active") && r();
  };
}
const u = /* @__PURE__ */ (function() {
  ...
  function n(d) {
    ...
    const b = d.checks.EMBED_VIDEO.sources, y = 'video, [src*="Video"], [src*="video"], [src*="watch"], [src*="youtube.com"], [src*="vimeo.com"], [src*="panopto.com"], [src*="wistia.com"], [src*="dailymotion.com"], [src*="brightcove.com"], [src*="vidyard.com"]';
    if (b) {
      const _ = b.split(/\s*[\s,]\s*/).map((I) => `[src*="${I}"]`);
      e.VideoSources = `${y}, ${_.join(", ")}`;
    } else
      e.VideoSources = y;
    const E = d.checks.EMBED_AUDIO.sources, v = 'audio, [src*="soundcloud.com"], [src*="simplecast.com"], [src*="podbean.com"], [src*="buzzsprout.com"], [src*="blubrry.com"], [src*="transistor.fm"], [src*="fusebox.fm"], [src*="libsyn.com"], [src*="spotify.com"], [src*="podcasts.apple.com"], [src*="castbox.fm"], [src*="megaphone.fm"], [src*="spreaker.com"], [src*="anchor.fm"], [src*="rss.com"], [src*="redcircle.com"]';
    if (E) {
...
function Z(t, e, n) {
  ...
  return s.flat().filter(Boolean)?.forEach((c) => {
    const p = c?.querySelectorAll("[data-sa11y-has-shadow-root]"), r = p ? ", [data-sa11y-has-shadow-root]" : "", d = Array.from(
      c.querySelectorAll(`:is(${t}${r}):not(${o}${i})`)
    );
    if (p.length) {
      const m = [];
      if (d.forEach((h, A) => {
...
const D = /* @__PURE__ */ (function() {
  ...
  function n(a) {
    e.Everything = Z("*", "root", u.Exclusions.Sa11yElements), e.Contrast = e.Everything.filter((r) => !u.Exclusions.Contrast.some(
      ...
      (r) => r.tagName === "IMG" && !u.Exclusions.Images.some((d) => r.matches(d))
      ...
      !u.Exclusions.Links.some((d) => r.matches(d))
    ), e.Headings = Z(
      ...
      d.length > 0 && (e.HeadingOverrideStart.set(d[0], r.previousHeading), e.HeadingOverrideEnd.set(d.pop(), r.previousHeading));
    }), e.ExcludedHeadings = e.Headings.filter(
      (r) => u.Exclusions.Headings.some((d) => r.matches(d))
    ), e.ExcludedOutlineHeadings = e.Headings.filter(
      (r) => u.Exclusions.Outline.some((d) => r.matches(d))
    ...
    e.CustomErrorLinks = i.length ? e.Links.filter(
      (r) => i.split(",").some((d) => r.matches(d.trim()))
    ) : [];
    const l = (r) => u.Root.Readability.some((d) => d.contains(r)) && !u.Exclusions.Readability.some((d) => r.matches(d));
    e.Readability = [
      ...e.Paragraphs.filter(l),
      ...e.Lists.filter(l)
    ];
    const c = a.checks.QA_NESTED_COMPONENTS.sources || '[role="tablist"], details';
    e.NestedComponents = e.Everything.filter((r) => r.matches(c)), e.TabIndex = e.Everything.filter(
...
function as(t, e = !1) {
  const n = t.element, s = Ne(t.htmlPath), o = Se(s, 600), a = `<pre><code>${ct(o)}</code></pre>`, i = (r) => {
    const d = Q(r), m = Se(d, 100);
    return d.length ? Y(m) : a;
  }, c = {
    SPAN: i,
    P: i,
    A: (r) => {
      const d = Q(r), m = Se(d, 100);
      return d.length > 1 && r.href && !r.hasAttribute("role") ? `<a href="${Fe(r.href)}">${Y(m)}</a>` : a;
    },
    IMG: (r) => {
      const d = r.closest("a[href]"), m = r.alt ? `alt="${Y(r.alt)}"` : "alt", h = ss(r);
      function A(y) {
      ...
      const b = h.startsWith("data:image/") ? h : Fe(h);
      return h ? A(b) : a;
    },
    IFRAME: (r) => {
      const d = r.src, m = r.title ? r.title : "", A = r.getAttribute("aria-label") || "";
      if (d) {
        const b = A || m;
        return `<iframe src="${Fe(d)}" aria-label="${Y(b)}"></iframe>`;
      }
      return a;
    },
    AUDIO: () => Ne(t.htmlPath),
...
function is(t, e, n = [], s) {
  let o = "";
  const a = n?.length ? n.join(",") : "", i = Array.isArray(s) ? s : null, l = (r = "") => i ? i.reduce((d, m) => d.replace(m, ""), r) : r;
...
function Zs(t, e) {
  ...
  ], r = p.length, d = t.filter(
    (m) => !p.some(
      (h) => h.dismiss === m.dismiss && (m.type === "warning" || m.type === "good")
    )
  );
  return r ? (u.Panel.dismissButton.classList.add("active"), u.Panel.dismissTooltip.innerText = f.sprintf("PANEL_DISMISS_BUTTON", r), e.object.setContent(f.sprintf("PANEL_DISMISS_BUTTON", r))) : u.Panel.dismissButton.classList.remove("active"), { dismissedIssues: n, updatedResults: d, dismissCount: r, dismissedResults: p };
}
let tn, Pt;
const ea = async (t, e, n, s) => {
  ...
  if (i.hidden = !1, a.tagName === "BUTTON" && a.hasAttribute("data-sa11y-dismiss")) {
    ...
    if (o === null && (setTimeout(() => je(f._("DISMISS_REMINDER")), 0), o = []), c.dismiss) {
      const p = a.hasAttribute("data-sa11y-dismiss-all") ? c.dismissAll : "", r = {
        key: c.dismiss,
        href: window.location.pathname,
        ...p ? { dismissAll: p } : {}
      }, d = Z(`[data-sa11y-annotation='${c.id}']`, "root"), m = d[0] ? d[0].getAttribute("data-sa11y-position") : 0;
...
function un() {
  const t = /* @__PURE__ */ new Date(), e = String(t.getDate()).padStart(2, "0"), n = String(t.getMonth() + 1).padStart(2, "0"), s = t.getFullYear(), o = (/* @__PURE__ */ new Date()).toLocaleString(), a = `${n}-${e}-${s}`, i = document.querySelector("head title"), l = !i || i.textContent.trim().length === 0, c = l ? "" : i.textContent, p = window.location.href;
  return { date: o, numericDate: a, titleCheck: l, metaTitle: c, pageURL: p };
}
async function oa(t, e) {
  ...
  const i = await a(n, "error"), l = await a(s, "warning"), c = await a(e, "dismissed"), p = un(), r = p.titleCheck ? "" : `<dt>${f._("PAGE_TITLE")}</dt><dd>${p.metaTitle}</dd>`, d = o.error !== 0 ? `<dt>${f._("ERRORS")}</dt><dd>${o.error}</dd>` : "", m = o.warning !== 0 ? `<dt>${f._("WARNINGS")}</dt><dd>${o.warning}</dd>` : "", h = o.dismiss !== 0 ? `<dt>${f._("DISMISSED")}</dt><dd>${o.dismiss}</dd>` : "";
  return `
      <!DOCTYPE html>
...
function ca(t) {
  const e = un(), n = t.filter((r) => r.type === "warning" || r.type === "error").map((r) => {
    const { type: d, content: m, htmlPath: h, cssPath: A } = r, b = m.replaceAll(/<span\s+class="visually-hidden"[^>]*>.*?<\/span>/gi, "").replaceAll('<hr aria-hidden="true">', " | ").replaceAll(/"/g, '""'), y = zs(String(b)), E = Ws(y), v = {
      Title: `"${e.metaTitle}"`,
      URL: `"${e.pageURL}"`,
      Type: `"${String(d)}"`,
      Issue: `"${E}"`,
...
`)}`, a = new Uint8Array([239, 187, 191]), i = new Blob([a, o], { type: "text/csv;charset=utf-8;" }), l = window.URL.createObjectURL(i), c = document.createElement("a");
...
function ha(t, e) {
  u.Global.showMovePanelToggle && (u.Panel.movePanelToggle.onclick = async () => {
    const l = R.getItem("sa11y-position"), [c, p] = l.includes("top") ? ["top-right", "top-left"] : ["right", "left"], r = l === c ? p : c;
    R.setItem("sa11y-position", r), [c, p].forEach((d) => {
      u.Panel.toggle.classList.replace(d, r), u.Panel.panel.classList.replace(d, r);
  ...
  a(i), n.addEventListener("change", () => {
    ...
    const c = (R.getItem("sa11y-theme") || (n.matches ? "dark" : "light")) === "dark" ? "light" : "dark";
    a(c);
  ...
  const s = () => {
    ...
    if (n.showTitleInPageOutline) {
      ...
      if (!l || l.textContent.trim().length === 0)
        ...
        const c = Q(l);
        i = `<li><span class="badge">${f._("TITLE")}</span> ${Y(c)}</li>`;
      }
      o.push(i);
    }
    e.forEach((l, c) => {
      const { element: p, headingLevel: r, text: d, type: m, dismissedHeading: h, isWithinRoot: A } = l, b = pn(p), y = b === !0 ? `<span class="hidden-icon"></span><span class="visually-hidden">${f._("HIDDEN")}</span>` : "", E = n.showHinPageOutline === !0 || n.showHinPageOutline === 1 ? "H" : "";
      let v;
      m === "error" && A === !0 ? (v = `
            <li class="outline-${r}">
...
const nt = (t, e) => {
  ...
  for (; i.nextNode() && h; ) {
    ...
    if (r && !A.closest("a") && (d === o && (o += r), r = !1, d = !1), A.ariaHidden === "true" && !(n && m < 3)) {
      Pn(i) || (h = !1);
      continue;
    }
    const y = ke(A, n);
    ...
    switch (A.tagName) {
      ...
      case "SVG":
        ...
        break;
      case "A":
        A.hasAttribute("title") ? (r = A.getAttribute("title"), d = o) : (r = !1, d = !1), o += nt(A, "");
        break;
      case "INPUT":
  ...
  return r && !d && (o += ` ${r}`), o = o.replace(/[\uE000-\uF8FF]/gu, ""), !o.trim() && t.hasAttribute("title") ? t.getAttribute("title") : o;
}, Aa = (t) => {
  ...
  const c = o && i.length ? u.Global.editImageURLofCMS + l : u.Global.editImageURLofCMS + a, p = (r) => r.includes(window.location.host) || r.startsWith(o);
...
function ba(t, e, n) {
  const s = () => {
    ...
    e.forEach((a, i) => {
      ...
      const { element: c, type: p, developer: r, dismissedImage: d } = a, m = ke(c) === "noAria" ? ct(c.getAttribute("alt")) : ke(c), h = pn(c);
      if (h) {
      ...
      const A = R.getItem("sa11y-developer"), y = (A === "Off" || A === null) && (p === "error" || p === "warning") && r === !0, E = ss(a.element), v = u.Global.editImageURLofCMS ? Aa(a) : "", L = c.hasAttribute("alt") && m === "" ? `<div class="badge">${f._("DECORATIVE")}</div>` : "", g = n.altPlaceholder?.some(
...
function Da(t) {
  ...
  return Object.assign(e.elements.popper.style, n.popper), e.styles = n, e.elements.arrow && Object.assign(e.elements.arrow.style, n.arrow), function() {
    Object.keys(e.elements).forEach(function(s) {
      var o = e.elements[s], a = e.attributes[s] || {}, i = Object.keys(e.styles.hasOwnProperty(s) ? e.styles[s] : n[s]), l = i.reduce(function(c, p) {
        return c[p] = "", c;
      }, {});
      !le(o) || !ye(o) || (Object.assign(o.style, l), Object.keys(a).forEach(function(c) {
        o.removeAttribute(c);
      }));
    });
  };
...
function Qe(t, e, n) {
  ...
  var i = Be(t) ? ae(t) : window, l = i.visualViewport, c = !ps() && n, p = (s.left + (c && l ? l.offsetLeft : 0)) / o, r = (s.top + (c && l ? l.offsetTop : 0)) / a, d = s.width / o, m = s.height / a;
  return {
    width: d,
    height: m,
    top: r,
...
function Ua(t) {
  var e, n = t.state, s = t.name, o = t.options, a = n.elements.arrow, i = n.modifiersData.popperOffsets, l = be(n.placement), c = mn(l), p = [te, de].indexOf(l) >= 0, r = p ? "height" : "width";
  if (!(!a || !i)) {
    var d = Ha(o.padding, n), m = gn(a), h = c === "y" ? ee : te, A = c === "y" ? ce : de, b = n.rects.reference[r] + n.rects.reference[c] - i[c] - n.rects.popper[r], y = i[c] - n.rects.reference[c], E = bt(a), v = E ? c === "y" ? E.clientHeight || 0 : E.clientWidth || 0 : 0, L = b / 2 - y / 2, g = d[h], _ = v - m[r] - d[A], I = v / 2 - m[r] / 2 + L, S = it(g, I, _), x = c;
...
function Dn(t) {
  var e, n = t.popper, s = t.popperRect, o = t.placement, a = t.variation, i = t.offsets, l = t.position, c = t.gpuAcceleration, p = t.adaptive, r = t.roundOffsets, d = t.isFixed, m = i.x, h = m === void 0 ? 0 : m, A = i.y, b = A === void 0 ? 0 : A, y = typeof r == "function" ? r({
    x: h,
    y: b
  }) : {
    x: h,
...
function za(t) {
  var e = t.state, n = t.options, s = n.gpuAcceleration, o = s === void 0 ? !0 : s, a = n.adaptive, i = a === void 0 ? !0 : a, l = n.roundOffsets, c = l === void 0 ? !0 : l, p = {
...
function Qa(t) {
  var e = t.state, n = t.instance, s = t.options, o = s.scroll, a = o === void 0 ? !0 : o, i = s.resize, l = i === void 0 ? !0 : i, c = ae(e.elements.popper), p = [].concat(e.scrollParents.reference, e.scrollParents.popper);
...
function Xa(t, e) {
  var n = ae(t), s = we(t), o = n.visualViewport, a = s.clientWidth, i = s.clientHeight, l = 0, c = 0;
  if (o) {
...
function Ja(t) {
  var e, n = we(t), s = An(t), o = (e = t.ownerDocument) == null ? void 0 : e.body, a = De(n.scrollWidth, n.clientWidth, o ? o.scrollWidth : 0, o ? o.clientWidth : 0), i = De(n.scrollHeight, n.clientHeight, o ? o.scrollHeight : 0, o ? o.clientHeight : 0), l = -s.scrollLeft + bn(t), c = -s.scrollTop;
  return Ie(o || n).direction === "rtl" && (l += De(n.clientWidth, o ? o.clientWidth : 0) - a), {
    width: a,
    height: i,
    x: l,
    y: c
...
function Bn(t, e, n) {
  return e === ls ? rn(Xa(t, n)) : Be(e) ? Za(e, n) : rn(Ja(we(t)));
}
function ei(t) {
  var e = rt(Ht(t)), n = ["absolute", "fixed"].indexOf(Ie(t).position) >= 0, s = n && le(t) ? bt(t) : t;
  return Be(s) ? e.filter(function(o) {
    return Be(o) && us(o, s) && ye(o) !== "body";
  }) : [];
}
function ti(t, e, n, s) {
  var o = e === "clippingParents" ? ei(t) : [].concat(e), a = [].concat(o, [n]), i = a[0], l = a.reduce(function(c, p) {
    var r = Bn(t, p, s);
    return c.top = De(r.top, c.top), c.right = Mt(r.right, c.right), c.bottom = Mt(r.bottom, c.bottom), c.left = De(r.left, c.left), c;
  }, Bn(t, i, s));
  return l.width = l.right - l.left, l.height = l.bottom - l.top, l.x = l.left, l.y = l.top, l;
}
function As(t) {
  var e = t.reference, n = t.element, s = t.placement, o = s ? be(s) : null, a = s ? Ye(s) : null, i = e.x + e.width / 2 - n.width / 2, l = e.y + e.height / 2 - n.height / 2, c;
  switch (o) {
    case ee:
      c = {
        x: i,
...
function pt(t, e) {
  e === void 0 && (e = {});
  var n = e, s = n.placement, o = s === void 0 ? t.placement : s, a = n.strategy, i = a === void 0 ? t.strategy : a, l = n.boundary, c = l === void 0 ? La : l, p = n.rootBoundary, r = p === void 0 ? ls : p, d = n.elementContext, m = d === void 0 ? st : d, h = n.altBoundary, A = h === void 0 ? !1 : h, b = n.padding, y = b === void 0 ? 0 : b, E = hs(typeof y != "number" ? y : gs(y, At)), v = m === st ? Ta : st, L = t.rects.popper, g = t.elements[A ? v : m], _ = ti(Be(g) ? g : g.contextElement || we(t.elements.popper), c, r, i), I = Qe(t.elements.reference), S = As({
    reference: I,
    element: L,
    placement: o
...
function ni(t, e) {
  e === void 0 && (e = {});
  var n = e, s = n.placement, o = n.boundary, a = n.rootBoundary, i = n.padding, l = n.flipVariations, c = n.allowedAutoPlacements, p = c === void 0 ? cs : c, r = Ye(s), d = r ? l ? On : On.filter(function(A) {
    return Ye(A) === r;
...
function ai(t) {
  ...
  if (!e.modifiersData[s]._skip) {
    for (var o = n.mainAxis, a = o === void 0 ? !0 : o, i = n.altAxis, l = i === void 0 ? !0 : i, c = n.fallbackPlacements, p = n.padding, r = n.boundary, d = n.rootBoundary, m = n.altBoundary, h = n.flipVariations, A = h === void 0 ? !0 : h, b = n.allowedAutoPlacements, y = e.options.placement, E = be(y), v = E === y, L = c || (v || !A ? [Ot(y)] : si(y)), g = [y].concat(L).reduce(function(j, q) {
    ...
    if (x)
      for (var ie = A ? 3 : 1, Ee = function(q) {
        var pe = g.find(function(ge) {
          var ue = S.get(ge);
          if (ue)
            return ue.slice(0, q).every(function(Ge) {
              return Ge;
            });
        });
        if (pe)
...
function ri(t) {
  var e = t.state, n = t.name, s = e.rects.reference, o = e.rects.popper, a = e.modifiersData.preventOverflow, i = pt(e, {
    ...
    altBoundary: !0
  }), c = Gn(i, s), p = Gn(l, o, a), r = Hn(c), d = Hn(p);
  e.modifiersData[n] = {
...
function ci(t) {
  var e = t.state, n = t.options, s = t.name, o = n.offset, a = o === void 0 ? [0, 0] : o, i = cs.reduce(function(r, d) {
    return r[d] = li(d, e.rects, a), r;
...
function hi(t) {
  var e = t.state, n = t.options, s = t.name, o = n.mainAxis, a = o === void 0 ? !0 : o, i = n.altAxis, l = i === void 0 ? !1 : i, c = n.boundary, p = n.rootBoundary, r = n.altBoundary, d = n.padding, m = n.tether, h = m === void 0 ? !0 : m, A = n.tetherOffset, b = A === void 0 ? 0 : A, y = pt(e, {
    boundary: c,
    rootBoundary: p,
    padding: d,
    altBoundary: r
  ...
  if (I) {
    if (a) {
      var $, w = g === "y" ? ee : te, O = g === "y" ? ce : de, G = g === "y" ? "height" : "width", H = I[g], K = H + y[w], ne = H - y[O], ie = h ? -x[G] / 2 : 0, Ee = v === ze ? S[G] : x[G], se = v === ze ? -x[G] : -S[G], V = e.elements.arrow, j = h && V ? gn(V) : {
        width: 0,
        height: 0
      }, q = e.modifiersData["arrow#persistent"] ? e.modifiersData["arrow#persistent"].padding : fs(), pe = q[w], ge = q[O], ue = it(0, S[G], j[G]), Ge = L ? S[G] / 2 - ie - ue - pe - k.mainAxis : Ee - ue - pe - k.mainAxis, _e = L ? -S[G] / 2 + ie + ue + ge + k.mainAxis : se + ue + ge + k.mainAxis, He = e.elements.arrow && bt(e.elements.arrow), yt = He ? g === "y" ? He.clientTop || 0 : He.clientLeft || 0 : 0, Xe = ($ = M?.[g]) != null ? $ : 0, Et = H + Ge - Xe - yt, vt = H + _e - Xe, Je = it(h ? Mt(K, Et) : K, H, h ? De(ne, vt) : ne);
      I[g] = Je, U[g] = Je - H;
    }
    if (l) {
...
function Bi(t, e) {
  ...
  return t.every(function(o) {
    var a = o.popperRect, i = o.popperState, l = o.props, c = l.interactiveBorder, p = Ci(i.placement), r = i.modifiersData.offset;
    if (!r)
      return !0;
    var d = p === "bottom" ? r.top.y : 0, m = p === "top" ? r.bottom.y : 0, h = p === "right" ? r.left.x : 0, A = p === "left" ? r.right.x : 0, b = a.top - s + d > c, y = s - a.bottom - m > c, E = a.left - n + h > c, v = n - a.right - A > c;
    return b || y || E || v;
  });
}
function Jt(t, e, n) {
...
var ji = 1, Ct = [], Zt = [];
function qi(t, e) {
  var n = Yn(t, Object.assign({}, he, Is(Wn(e)))), s, o, a, i = !1, l = !1, c = !1, p = !1, r, d, m, h = [], A = Fn(Et, n.interactiveDebounce), b, y = ji++, E = null, v = wi(n.plugins), L = {
  ...
  return He(), ie(), H(), K("onCreate", [g]), n.showOnCreate && et(), I.addEventListener("mouseenter", function() {
  ...
  function ne() {
    var T = g.props.aria;
    if (T.content) {
      var P = "aria-" + T.content, B = I.id, W = Ke(g.props.triggerTarget || t);
  ...
  function se(T) {
    if (!(me.isTouch && (c || T.type === "mousedown"))) {
  ...
  function Ge(T, P) {
    ...
    if (T === 0)
      return P();
    Jt(B, "remove", d), Jt(B, "add", W), d = W;
  }
  function _e(T, P, B) {
    B === void 0 && (B = !1);
    var W = Ke(g.props.triggerTarget || t);
    W.forEach(function(F) {
  ...
  function Et(T) {
    ...
    if (!(T.type === "mousemove" && B)) {
      var W = xe().concat(I).map(function(F) {
        var X, re = F._tippy, Ue = (X = re.popperInstance) == null ? void 0 : X.state;
        return Ue ? {
  ...
  function Lt() {
    Tt();
    var T = g.props, P = T.popperOptions, B = T.placement, W = T.offset, F = T.getReferenceClientRect, X = T.moveTransition, re = U() ? ln(I).arrow : null, Ue = F ? {
    ...
    }), Pe.push.apply(Pe, P?.modifiers || []), g.popperInstance = xi(Ue, I, Object.assign({}, P, {
      placement: B,
      onFirstUpdate: m,
      modifiers: Pe
    }));
  ...
  function xt(T) {
    if (!g.state.isDestroyed) {
      ...
      g.props = B, He(), P.interactiveDebounce !== B.interactiveDebounce && (Ee(), A = Fn(Et, B.interactiveDebounce)), P.triggerTarget && !B.triggerTarget ? Ke(P.triggerTarget).forEach(function(W) {
      ...
      m = function() {
        ...
        if (!(!g.state.isVisible || p)) {
          if (p = !0, I.offsetHeight, I.style.transition = g.props.moveTransition, U() && g.props.animation) {
            var Ft = O(), St = Ft.box, $e = Ft.content;
            Xt([St, $e], F), zn([St, $e], "visible");
          }
...
function $t(t, e = 2) {
  ...
  let r = 0, d = (t - n[p][r]) * c;
  for (r++; r < a; r++)
    l = n[p][r], l > 400 ? i[r] = l : t < 14.5 ? i[r] = 999 : t < 29.5 ? i[r] = 777 : l > 24 ? i[r] = Math.round(l - s[p][r] * d) : i[r] = l - (2 * s[p][r] * d | 0) * 0.5;
  return i;
}
function ut(t = [0, 0, 0]) {
  function e(n) {
...
function fe(t, e) {
  ...
  if (n.startsWith("rgb")) {
    ...
    c.fillStyle = n, c.fillRect(0, 0, 1, 1), [s, o, a, i] = c.getImageData(0, 0, 1, 1).data, i = (i / 255).toFixed(2);
  }
  ...
  for (; n && n.nodeType === 1; ) {
    ...
    if (a[3] !== 0 && a !== "transparent") {
      if (a[3] < 1) {
        ...
        const c = fe(l || "rgba(255, 255, 255, 1)");
        return vn(a, c);
      }
      return a;
    }
...
function Ji(t, e, n, s) {
  ...
  const a = lt(t), i = lt(e), l = a > i ? at(1, i) > o : at(0, i) < o, c = (L, g, _) => _ ? Ss(L, g) : ks(L, g);
  let p = t, r = p, d = at(a, i), m = d, h = t, A = 0.16;
  const b = 0.5, y = 0.01;
  let E = 0;
  const v = 100;
  for (; A >= y; ) {
    ...
    p = c(p, A, l);
    const L = lt(p);
    d = at(L, i), d >= o && (r = d <= m ? p : r, m = d, p = h, A *= b), h = p;
  }
  return { color: J(r) };
}
const Zi = (t, e) => {
  const n = Gt(t, [0, 0, 0, 1]), s = Gt(t, [255, 255, 255, 1]), o = Math.abs(n.ratio) > Math.abs(s.ratio), a = o ? [0, 0, 0, 1] : [255, 255, 255, 1], i = o ? n.ratio : s.ratio, l = $t(i).slice(1), c = Math.ceil(l[Math.floor(e / 100) - 1]);
  return { suggestedColor: a, size: c };
};
function er(t, e, n, s) {
  const a = n == null || s == null, i = ut(e), l = (C, k) => i <= 0.179 ? Ss(C, k) : ks(C, k);
  let c = t, p = Gt(c, e), { ratio: r } = p, d = null, m = r, h = null, A, b, y;
  ...
  const x = 50;
  for (; g >= I && S < x; )
    S += 1, c = l(c, g), p = Gt(c, e), r = p.ratio, (a ? v() : E()) && ((Math.abs(r) <= Math.abs(m) || !h) && (h = c, m = r), c = L, g *= _), L = c;
  return h ? { color: J(h), size: null } : !a && d ? {
    color: J(d.suggestedColor),
    size: d.size
  } : { color: J(t), size: null };
}
function tr(t, e, n, s, o, a, i) {
  const { ratio: l, blendedColor: c } = ft(e, n), p = s >= 24 || s >= 18.67 && o >= 700;
  let r;
  if (i === "AAA")
    r = p ? l < 4.5 : l < 7;
  else {
...
function nr(t, e, n, s, o, a, i) {
  const { ratio: l, blendedColor: c } = ft(e, n, i), p = $t(l).slice(1), r = Math.floor(o / 100) - 1, d = p[r];
  return s < d ? {
    $el: t,
    ratio: Ln(l),
    color: c,
...
function sr(t) {
  const { sanitizedText: e, color: n, background: s, fontWeight: o, fontSize: a, ratio: i, textUnderline: l } = t, c = s && s.type !== "image", p = c ? J(s) : "#000000", r = n ? J(n) : "#000000", d = o ? `font-weight:${o};` : "", m = a ? `font-size:${a}px;` : "", h = l ? `text-decoration:${l};` : "", A = n ? "" : 'class="unknown"', b = s && s.type !== "image" ? "" : 'class="unknown"', y = n ? "" : `<span id="fg-input-unknown" class="visually-hidden">(${f._("UNKNOWN")})</span>`, E = s ? "" : `<span id="bg-input-unknown" class="visually-hidden">(${f._("UNKNOWN")})</span>`;
  let v;
...
function ar(t, e) {
  ...
  const { fontSize: s, fontWeight: o, type: a, isLargeText: i } = e, l = t.querySelector("#contrast"), c = t.querySelector("#contrast-preview"), p = t.querySelector("#fg-input"), r = t.querySelector("#bg-input"), d = t.querySelector("#value"), m = t.querySelector("#good"), h = (y, E) => {
    y.forEach((v) => {
    ...
    if (setTimeout(() => {
      ...
      c.style.color = S ? "" : E, c.style.backgroundColor = x ? "" : v, c.style.backgroundImage = x ? "" : "none";
    ...
    if (g === "APCA") {
      ...
      switch (a) {
        ...
        default: {
          $ ? (h([d, l], O), m.hidden = !O) : (h([d, l], w), m.hidden = !w);
          break;
        }
      }
    }
...
function ir(t) {
  ...
  if (n && s && s.type !== "image" && (l === "text" || l === "svg-error" || l === "input")) {
    const c = u.Global.contrastAlgorithm === "APCA" ? er(n, s, o, a) : Ji(
      n,
      s,
      i,
      u.Global.contrastAlgorithm
    ...
    let p;
    const r = '<hr aria-hidden="true">', d = J(t.background), h = `<button id="suggest" class="badge" style="${`color:${c.color};background-color:${d};`}">${c.color}</button>`, A = `<button id="suggest-size" class="normal-badge">${c.size}px</button>`;
    u.Global.contrastAlgorithm === "AA" || u.Global.contrastAlgorithm === "AAA" ? c.color === null ? p = `${r} ${f._("NO_SUGGESTION")}` : p = `${r} ${f._("CONTRAST_COLOR")} ${h}` : c.color && c.size ? p = `${r} ${f._("CONTRAST_APCA")} ${h} ${A}` : c.color ? p = `${r} ${f._("CONTRAST_COLOR")} ${h}` : c.size && (p = `${r} ${f._("CONTRAST_SIZE")} ${A}`), e = document.createElement("div"), e.id = "advice";
  ...
  connectedCallback() {
    ...
    const s = Le(ws, {
      ...
      onShow(o) {
        ...
        if (o.popper.addEventListener("keydown", c), !o.popper.hasAttribute("contrast-tools-initialized")) {
        ...
        const d = () => {
          i.removeEventListener("click", l), o.popper.removeEventListener("keydown", c), o.popper.removeEventListener("hidden", d);
        };
        o.popper.addEventListener("hidden", d);
      },
      onTrigger(o, a) {
...
const pr = () => {
  ...
  if (t.offsetTop === 0) {
    ...
    if (je(`${f._("NOT_VISIBLE")}`, c, l), pr(), o) {
...
function br(t, e) {
  const n = e.susAltStopWords ? e.susAltStopWords.split(",").map((c) => c.trim().toLowerCase()).filter(Boolean) : f._("SUS_ALT_STOPWORDS"), s = new Set(f._("PLACEHOLDER_ALT_STOPWORDS")), o = Re(e.altPlaceholder, !0), a = Re(e.linkIgnoreStrings), i = e.extraPlaceholderStopWords.split(",").map((c) => c.trim().toLowerCase()).filter(Boolean), l = (c) => {
    const p = c.toLowerCase(), r = p.replace(/\d+/g, "").trim(), d = [null, null, null];
    for (const m of Ar)
      if (p.includes(m)) {
        d[0] = m;
        break;
    ...
    if ((s.has(p) || s.has(r)) && (d[2] = c), i.length)
      for (const m of i) {
        const h = p.indexOf(m);
        if (h > -1 && h < 6) {
          d[2] = m;
  ...
  return D.Found.Images.forEach((c) => {
    const p = ke(c) === "noAria" ? c.getAttribute("alt") : ke(c);
    if (c.height < 2 && c.width < 2 && (Me(c) || p === ""))
      return;
    const r = c.closest(
      e.imageWithinLightbox ? `a[href]:not(${e.imageWithinLightbox})` : "a[href]"
    ), d = c.getAttribute("src") ? c.getAttribute("src") : c.getAttribute("srcset"), m = r ? qe(r, u.Exclusions.LinkSpan).textContent.replace(
      a,
      ""
    ) : "", h = Ae(m).length;
    if (r && r.getAttribute("aria-hidden") === "true") {
      const w = r.getAttribute("tabindex") === "-1";
      e.checks.HIDDEN_FOCUSABLE && !w && t.push({
        ...
        dismiss: N(`IMGHIDDENFOCUSABLE${d}`),
    ...
    if (e.checks.MISSING_ALT && y && b === "") {
      t.push({
        ...
        type: e.checks.MISSING_ALT.type || "error",
        content: f.sprintf(e.checks.MISSING_ALT.content || "MISSING_ALT"),
        dismiss: N(`IMGNOALTARIA${d}`),
        dismissAll: e.checks.MISSING_ALT.dismissAll ? "MISSING_ALT" : !1,
        developer: e.checks.MISSING_ALT.developer || !1
      });
      return;
    }
    const E = p === "", v = c.closest("figure"), L = v?.querySelector("figcaption"), g = L ? Q(L) : "", _ = e.checks.LINK_IMAGE_LONG_ALT.maxLength || 250, I = e.checks.IMAGE_ALT_TOO_LONG.maxLength || 250, S = p.match(o)?.[0];
    if (E || S) {
      const w = e.checks.IMAGE_DECORATIVE_CAROUSEL.sources, O = w ? c.closest(w) : "";
      if (O) {
        ...
        dismiss: N(`DECIMAGE${d}`),
        dismissAll: e.checks.IMAGE_DECORATIVE.dismissAll ? "IMAGE_DECORATIVE" : !1,
        developer: e.checks.IMAGE_DECORATIVE.developer || !1
      });
      return;
    }
    const x = r ? e.checks.LINK_ALT_UNPRONOUNCEABLE : e.checks.ALT_UNPRONOUNCEABLE;
    if (x && p.replace(/"|'|\?|\.|-|\s+/g, "") === "" && h === 0) {
      const w = r ? "LINK_ALT_UNPRONOUNCEABLE" : "ALT_UNPRONOUNCEABLE";
      t.push({
        test: w,
        element: c,
        type: x.type || "error",
        content: f.sprintf(x.content || w, b),
        dismiss: N(`UNPRONOUNCEABLE${d}`),
        dismissAll: x.dismissAll ? "ALT_UNPRONOUNCEABLE" : !1,
        developer: x.developer || !1
      });
      return;
    }
    const C = l(b), k = r ? e.checks.LINK_ALT_MAYBE_BAD : e.checks.ALT_MAYBE_BAD, M = new RegExp(`^\\S{${k.minLength || 15},}$`), U = /[^\p{L}\-,.!?]/u.test(p);
    if (C[0] !== null) {
      const w = r ? e.checks.LINK_ALT_FILE_EXT : e.checks.ALT_FILE_EXT, O = r ? "LINK_ALT_FILE_EXT" : "ALT_FILE_EXT";
      w && t.push({
        test: O,
        element: c,
        type: w.type || "error",
      ...
      const w = r ? e.checks.LINK_PLACEHOLDER_ALT : e.checks.ALT_PLACEHOLDER, O = r ? "LINK_PLACEHOLDER_ALT" : "ALT_PLACEHOLDER";
      w && t.push({
        test: O,
        element: c,
        type: w.type || "error",
      ...
      const w = r ? e.checks.LINK_SUS_ALT : e.checks.SUS_ALT, O = r ? "LINK_SUS_ALT" : "SUS_ALT";
      w && t.push({
        test: O,
        element: c,
        type: w.type || "warning",
    ...
    } else if (k && M.test(p) && U) {
      const w = r ? "LINK_ALT_MAYBE_BAD" : "ALT_MAYBE_BAD";
      t.push({
        test: w,
        element: c,
        type: k.type || "error",
    ...
    } else if (r ? p.length > _ : p.length > I) {
      const w = r ? e.checks.LINK_IMAGE_LONG_ALT : e.checks.IMAGE_ALT_TOO_LONG, O = r ? "LINK_IMAGE_LONG_ALT" : "IMAGE_ALT_TOO_LONG", G = Se(b, 600);
      w && t.push({
        test: O,
        element: c,
        type: w.type || "warning",
        content: f.sprintf(w.content || O, p.length, G),
        dismiss: N(`${O + d + b}`),
        dismissAll: w.dismissAll ? O : !1,
        developer: w.developer || !1
      });
      ...
      if (w) {
        const G = Te(r), H = Ae(G), K = Y(H), ne = h === 0 ? f.sprintf("LINK_IMAGE_ALT", b) : `${f.sprintf("LINK_IMAGE_ALT_AND_TEXT", b, K)} ${f.sprintf("ACC_NAME_TIP")}`;
        t.push({
          test: O,
          element: c,
          type: w.type || "warning",
...
function yr(t, e, n) {
  ...
  return D.Found.Headings.forEach((i, l) => {
    const p = Te(i, u.Exclusions.HeaderSpan).replace(a, ""), r = Ae(p), d = Y(r), m = u.Root.areaToCheck.some((C) => C.contains(i)), h = u.Root.areaToCheck.some(
    ...
    _ && g && t.push({
      ...
      developer: I,
      margin: x
    }), s = y, o = d, D.Found.OutlineIgnore.includes(i) || n.push({
      element: i,
      headingLevel: y,
      text: d,
      type: g,
...
function xr(t, e) {
  const n = e.linkStopWords ? e.linkStopWords.split(",").map((r) => r.toLowerCase().trim()) : [], s = /* @__PURE__ */ new Set([...f._("PARTIAL_ALT_STOPWORDS"), ...n]), o = new Set(e.linkIgnoreStrings.map((r) => r.toLowerCase())), a = Re(f._("CLICK")), i = Re(f._("NEW_WINDOW_PHRASES")), l = Re(Er), c = Re(e.linkIgnoreStrings), p = {};
  return D.Found.Links.forEach((r) => {
    const d = qs(r), m = r.getAttribute("title"), h = r.getAttribute("aria-hidden") === "true", A = r.getAttribute("tabindex") === "-1", b = r.getAttribute("target")?.toLowerCase() === "_blank", y = r.getAttribute("aria-label"), E = r.getAttribute("aria-labelledby"), v = E ? null : r.querySelector("[aria-labelledby]"), L = E || v, g = L || y || r.querySelector("[aria-label]"), _ = Ae(
      Te(r, u.Exclusions.LinkSpan)
    ), I = _.replace(c, ""), S = I.toLowerCase(), x = Wt(S), C = Q(r).toLowerCase(), k = S.match(i)?.[0] || C.match(i)?.[0], M = S.match(l)?.[0] || C.match(l)?.[0], U = r.matches(vr);
    if (!r.querySelector("img")) {
      ...
      if (g && I.length !== 0) {
        const V = Y(I), j = qe(r, u.Exclusions.LinkSpan), q = Q(j).replace(c, ""), pe = Wt(q), ge = en(pe, s), ue = is(
          r,
          _,
...
function Nr(t, e) {
  ...
  for (let a = 0; a < D.Found.Contrast.length; a++) {
    const i = D.Found.Contrast[a], l = getComputedStyle(i), c = parseFloat(l.opacity), p = fe(l.color, c), r = parseFloat(l.fontSize), d = l.fontWeight, m = Xn(d), h = Rt(i, u.Global.shadowDetection), A = Ks(i), y = Me(i) || A || c === 0 || r === 0, v = Array.from(i.childNodes).filter((g) => g.nodeType === 3).map((g) => g.textContent).join("").trim(), L = ["SELECT", "INPUT", "TEXTAREA"].includes(i.tagName);
    if (v.length !== 0 || L) {
  ...
  return D.Found.Svg.forEach((a) => {
    const i = { $el: a, type: "svg-warning" }, l = Rt(a, u.Global.shadowDetection), c = l !== "unsupported" && l.type !== "image", p = a.querySelectorAll("path, rect, circle, ellipse, polygon, text, use"), r = a.querySelectorAll(
    ...
    if (a.placeholder && a.placeholder.length !== 0) {
      const i = getComputedStyle(a, "::placeholder"), l = fe(i.getPropertyValue("color")), c = parseFloat(i.fontSize), p = Xn(i.fontWeight), r = Rt(a, u.Global.shadowDetection), d = parseFloat(i.opacity);
      if (l === "unsupported")
    ...
    if (e.contrastAlgorithm === "APCA")
      ...
      c = Object.values(p).map((r) => ({
        ...r[0]
      }));
    }
    return [...c, ...l];
  })(n).forEach((a) => {
    const { $el: i, ratio: l } = a, c = a, p = i.tagName === "OPTION" ? i.closest("datalist, select, optgroup") : i, r = qe(p, ["option:not(option:first-child)"]), d = Q(r), m = Se(d, 80), h = Y(m);
    let A;
    a.type === "placeholder" || a.type === "placeholder-unsupported" ? A = Y(i.placeholder) : a.type === "svg-error" || a.type === "svg-warning" ? A = "" : A = h, c.sanitizedText = A;
    let b = "";
...
function Sr(t, e) {
  return e.formLabelsPlugin && D.Found.Inputs.forEach((n) => {
    ...
    if (Me(n) || s && o)
      return;
    const i = Te(n), l = Ae(i), c = n.getAttribute("alt"), p = n.getAttribute("type"), r = n.getAttribute("title"), d = n.placeholder && n.placeholder !== 0, m = n.getAttribute("aria-label") || n.getAttribute("aria-labelledby");
    if (p === "submit" || p === "button" || p === "hidden")
      return;
    if (p === "image") {
      e.checks.LABELS_MISSING_IMAGE_INPUT && (!c || c.trim() === "") && !m && !r && t.push({
...
`, ""), L.length <= 3)
    ...
    const A = Number(h.toFixed(1)), b = Number((l / p).toFixed(1)), y = Math.round(100 * ((l - (d + m)) / l));
    let E;
  ...
  if (["sv", "fi", "da", "no", "nb", "nn"].includes(e)) {
    ...
    }, l = a(), c = l.length, p = l.filter((b) => b.length > 6).length, r = i().length || 1, d = Math.round(c / r + p * 100 / c), m = Number((c / r).toFixed(1)), h = Math.round(100 * (p / c));
    let A;
...
function Cr(t, e) {
  ...
  return e.checks.EMBED_AUDIO && D.Found.Audio.forEach((s) => {
    ...
    const o = ["presentation", "none"].includes(s.getAttribute("role")), a = Me(s), i = s.tagName === "VIDEO" || s.tagName === "AUDIO", l = s.getAttribute("aria-hidden") === "true", c = s.getAttribute("tabindex") === "-1";
    if (!(a || i || l && c || o)) {
      if (c) {
    ...
    Me(s) || a && i || o || s.tagName === "VIDEO" || s.tagName === "AUDIO" || t.push({
      test: "EMBED_GENERAL",
      element: s,
...
function Pr(t, e) {
  if (e.checks.QA_BAD_LINK && D.Found.CustomErrorLinks.forEach((i) => {
    ...
    if (i.hasAttribute("href")) {
      ...
      if (e.checks.QA_IN_PAGE_LINK) {
        const r = Q(i).length !== 0, d = i.ariaHidden === "true" && i.getAttribute("tabindex") === "-1", m = i.hasAttribute("role") || i.hasAttribute("aria-haspopup") || i.hasAttribute("aria-expanded") || i.hasAttribute("onclick") || i.hasAttribute("disabled") || i.closest('nav, [role="navigation"]');
        if ((l.startsWith("#") || l === "") && r && !d && !m) {
    ...
    if (Me(i) === !1) {
      ...
      }), e.checks.TABLES_SEMANTIC_HEADING && c.length > 0 && c.forEach((r) => {
        t.push({
    ...
    const i = (d, m) => {
      ...
      const m = getComputedStyle(d).fontSize.replace("px", ""), h = Q(d), A = h.match(/[.;?!"]/) === null, b = h.length >= 4 && h.length <= 120;
      if (m >= 24 && !d.closest(c) && b && A && !l(d)) {
        const y = Y(h);
        i(d, y);
      }
    }, r = (d) => {
      if (/^<\s*(strong|b)(\s+[^>]*)?>/i.test(d.innerHTML.trim()) && !d.closest(c)) {
        const h = d.querySelector("strong, b"), A = Q(h), b = A.match(/[.:;?!"']/) === null;
        if (A.length >= 3 && A.length <= 120 && b) {
          const E = qe(d, ["strong", "b"]).textContent.trim().length;
          if (E !== 0 && E <= 250)
            return;
          const v = Y(A);
          i(h, v);
    ...
    D.Found.Paragraphs.forEach((d) => {
      p(d), r(d);
    });
  }
  if (e.checks.QA_FAKE_LIST) {
    const i = new RegExp(/(([023456789][\d\s])|(1\d))/, ""), l = new RegExp(/(^[aA1αаΑ]|[^p{Alphabetic}\s])[-\s.)]/, "u"), c = new RegExp(/\p{Extended_Pictographic}/, "u"), p = ["a", "A", "α", "Α", "а", "А", "1"], r = /[([{#]/, d = {
      2: "1",
      b: "a",
      B: "A",
      β: "α",
      ...
      б: "а",
      Б: "А"
    }, m = (y) => y.replace(/^b|^B|^б|^Б|^β|^В|^2/, (E) => d[E]);
    let h = "", A = "", b = !1;
    D.Found.Paragraphs.forEach((y, E) => {
      let v = !1, L = !1;
      A = A || Q(y).replace("(", "");
      const g = A.substring(0, 2), _ = g.match(l), I = g.match(i), S = g.match(c), x = r.test(g.charAt(0));
      if (g.length > 0 && g !== h && !I && (_ || S || x)) {
        ...
        if (C) {
          if (v = Q(C).replace("(", "").substring(0, 2), p.includes(v?.toLowerCase().trim()))
            return;
          const k = m(v);
          _ ? g !== "A " && g === k && (L = !0) : S && !b && k.match(c) && (L = !0, b = !0);
        }
        if (!L) {
          let k = y?.querySelector("br")?.nextSibling?.nodeValue;
          k && (k = k.replace(/<\/?[^>]+(>|$)/g, "").trim().substring(0, 2), (r.test(k.charAt(0)) || g === m(k) || !b && k.match(c)) && (L = !0));
        }
        L ? (t.push({
          test: "QA_FAKE_LIST",
          element: y,
  ...
  if (e.checks.QA_UPPERCASE) {
    const i = (l) => {
      ...
      const p = /([A-Z]{2,}[ ])([A-Z]{2,}[ ])([A-Z]{2,}[ ])([A-Z]{2,})/g, r = c.match(p);
  ...
  const n = (i) => {
    ...
    const l = getComputedStyle(i), { textDecorationLine: c, textAlign: p, fontSize: r } = l, d = 'a[href], button, abbr, [role="link"], [role="button"], [tabindex="0"], [onclick]';
...
function Or(t, e) {
    ...
    if (n) {
      ...
      if (s) {
        const o = s.split(",").reduce((i, l) => {
          const [c, p] = l.split("=").map((r) => r.trim());
          return i[c] = p, i;
        }, {});
  ...
  return e.checks.META_REFRESH && document.querySelector('meta[http-equiv="refresh"]') && t.push({
    ...
    const o = /* @__PURE__ */ new Set(), a = (l, c) => {
      l.forEach((p) => {
        const { id: r } = p;
  ...
  constructor(e) {
    ...
    this.initialize = () => {
      ...
      if ((s || n.externalDeveloperChecks === !0) && (this.results = this.results.filter((a) => a.developer !== !0)), s && (this.results = this.results.filter((a) => a.external !== !0)), this.results.forEach((a, i) => {
        const l = n.selectorPath ? Qs(a.element) : "", c = a.element?.outerHTML.replace(/\s{2,}/g, " ").trim() || "";
        ...
        if (Ea(i.error, i.warning), R.getItem("sa11y-panel") === "Opened") {
          ...
          this.results.forEach((p) => {
            ...
            if (p.element && !p.margin) {
              ...
              r = { ...p, margin: `${d * 20 + m}px` };
            }
            lr(r, n);
          }), D.initializeAnnotations();
          const c = new Zn();
          document.body.appendChild(c), na(this.results, this.dismissed, this.checkAll, this.resetAll), ma(this.dismissedPageResults, this.headingOutline, n), n.showImageOutline && ba(this.dismissedPageResults, this.imageResults, n), ya(a.dismissCount, i.error, i.warning), gr(this.results), n.exportResultsPlugin && da(this.results, a.dismissedResults), gt(u.Panel.pageIssuesList, u.Panel.pageIssuesContent);
        }
