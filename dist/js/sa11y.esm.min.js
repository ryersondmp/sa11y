...
function xr(t, e) {
  ...
  return D.Found.Links.forEach((r) => {
    ...
    if (!r.querySelector("img")) {
      ...
      const G = en(x, s, i), H = x.match(a)?.[0] || C.match(a)?.[0], K = S.match(Lr)?.[0], ie = S.startsWith("www.") || S.startsWith("http") ? "URL Prefix" : S.match(Tr)?.[0], Ee = I.length === 1 && Ir.test(I), se = S.match(_r)?.[0];
      if (G)
        w(r, G);
      else if (K)
