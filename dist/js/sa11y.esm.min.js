...
async function ia(t, e) {
  ...
  async function i(b, E) {
    ...
    if (!(b.length > 0))
      return "";
    let x = `<h2>${L}</h2>`, _ = `<ol class="${E}">`, k = "</ol>";
    E === "dismissed" && (_ = `<details><summary>${f.sprintf("PANEL_DISMISS_BUTTON", r.dismiss)}</summary><ol>`, k = "</details>"), x += _;
    const C = b.map(async (I) => {
      let S = "";
      if (I.element) {
        const R = ["IMG", "IFRAME", "AUDIO", "VIDEO"], G = await ns(I, !0);
        R.includes(I.element.tagName) ? S = `<li><strong>${f._("PREVIEW")}:</strong> ${G}</li><li><strong>${f._("ELEMENT")}:</strong> <pre><code>${lt(I.htmlPath)}</code></pre></li>` : S = `<li><strong>${f._("ELEMENT")}:</strong> <pre><code>${lt(I.htmlPath)}</code></pre></li>`;
      }
      const w = I.cssPath ? `<li><strong>${f._("PATH")}:</strong> <pre><code>${I.cssPath}</code></pre></li>` : "";
      return `<li>${I.content} <ul>${S}${w}</ul></li>`;
    }), D = await Promise.all(C);
    return x += D.join(""), x += k, x;
  }
...
class xn extends HTMLElement {
  connectedCallback() {
    ...
    S.setAttribute("id", "container"), S.setAttribute("role", "region"), S.setAttribute("data-sa11y-version", "dev-4.4.0"), S.setAttribute("lang", f._("LANG_CODE")), S.setAttribute("aria-label", f._("CONTAINER_LABEL")), S.setAttribute("dir", u.Global.langDirection), a === "top-left" || a === "top-right" ? S.innerHTML = `
        ${A}
        ...
        <div id="panel" class="${a}" part="panel">
          ...
          ${k}
          ${I}
        </div>`, this.shadowRoot.appendChild(S), u.initializePanelSelectors();
  }
}
function ua(t, e) {
...
function ha(t, e, n) {
  const s = () => {
    ...
    e.forEach((l, d) => {
      ...
      if (h.id = `sa11y-h${d}`, y) {
        const _ = ft(p, "display", "none"), k = _?.previousElementSibling || _?.parentNode;
        k?.insertAdjacentElement("beforebegin", h), k?.setAttribute("data-sa11y-parent", `h${d}`);
      } else
...
function ma(t, e, n) {
  const s = () => {
    ...
    e.forEach((i, a) => {
      ...
      ), x = n.imageWithinLightbox ? `a[href]:not(${n.imageWithinLightbox})` : "a[href]", _ = d.closest(x) ? `<div class="badge"><span class="link-icon"></span><span class="visually-hidden">${f._("LINKED")}</span></div>` : "", k = g === !0 ? `<div class="badge"><span class="hidden-icon"></span><span class="visually-hidden">${f._("HIDDEN")}</span></div>` : "";
      let C;
      if (p === "error" && !b) {
...
function Ga(t) {
  ...
  if (!(!i || !a)) {
    var c = Ba(r.padding, n), m = fn(i), g = d === "y" ? J : Z, A = d === "y" ? re : oe, y = n.rects.reference[o] + n.rects.reference[d] - a[d] - n.rects.popper[o], b = a[d] - n.rects.reference[d], E = At(i), v = E ? d === "y" ? E.clientHeight || 0 : E.clientWidth || 0 : 0, L = y / 2 - b / 2, h = c[g], x = v - m[o] - c[A], _ = v / 2 - m[o] / 2 + L, k = at(h, _, x), C = d;
    n.modifiersData[s] = (e = {}, e[C] = k, e.centerOffset = k - _, e);
  }
}
function Ha(t) {
...
function Pn(t) {
  ...
  if (p) {
    var _ = At(n), k = "clientHeight", C = "clientWidth";
    if (_ === ee(n) && (_ = Se(n), Le(_).position !== "static" && l === "absolute" && (k = "scrollHeight", C = "scrollWidth")), _ = _, r === J || (r === Z || r === oe) && i === ct) {
      h = re;
      var D = c && _ === x && x.visualViewport ? x.visualViewport.height : (
        // $FlowFixMe[prop-missing]
        _[k]
      );
      y -= D - s.height, y *= d ? 1 : -1;
    }
  ...
  if (g = w.x, y = w.y, d) {
    var R;
    return Object.assign({}, S, (R = {}, R[h] = v ? "0" : "", R[L] = E ? "0" : "", R.transform = (x.devicePixelRatio || 1) <= 1 ? "translate(" + g + "px, " + y + "px)" : "translate3d(" + g + "px, " + y + "px, 0)", R));
  }
  return Object.assign({}, S, (e = {}, e[h] = v ? y + "px" : "", e[L] = E ? g + "px" : "", e.transform = "", e));
}
function Ka(t) {
...
function ni(t) {
  ...
  if (!e.modifiersData[s]._skip) {
    ...
    }, []), x = e.rects.reference, _ = e.rects.popper, k = /* @__PURE__ */ new Map(), C = !0, D = h[0], I = 0; I < h.length; I++) {
      var S = h[I], w = he(S), R = Ye(S) === We, G = [J, re].indexOf(w) >= 0, $ = G ? "width" : "height", W = dt(e, {
        placement: S,
        boundary: o,
        rootBoundary: c,
        altBoundary: m,
...
function ui(t) {
  var e = t.state, n = t.options, s = t.name, r = n.mainAxis, i = r === void 0 ? !0 : r, a = n.altAxis, l = a === void 0 ? !1 : a, d = n.boundary, p = n.rootBoundary, o = n.altBoundary, c = n.padding, m = n.tether, g = m === void 0 ? !0 : m, A = n.tetherOffset, y = A === void 0 ? 0 : A, b = dt(e, {
    ...
    padding: c,
    altBoundary: o
  }), E = he(e.placement), v = Ye(e.placement), L = !v, h = hn(E), x = pi(h), _ = e.modifiersData.popperOffsets, k = e.rects.reference, C = e.rects.popper, D = typeof y == "function" ? y(Object.assign({}, e.rects, {
    placement: e.placement
  })) : y, I = typeof D == "number" ? {
    mainAxis: D,
    altAxis: D
    ...
    mainAxis: 0,
    altAxis: 0
  }, D), S = e.modifiersData.offset ? e.modifiersData.offset[e.placement] : null, w = {
    x: 0,
    y: 0
  };
  if (_) {
    if (i) {
      var R, G = h === "y" ? J : Z, $ = h === "y" ? re : oe, W = h === "y" ? "height" : "width", H = _[h], F = H + b[G], Q = H - b[$], te = g ? -C[W] / 2 : 0, le = v === We ? k[W] : C[W], ne = v === We ? -C[W] : -k[W], ke = e.elements.arrow, me = g && ke ? fn(ke) : {
        width: 0,
        height: 0
      }, ce = e.modifiersData["arrow#persistent"] ? e.modifiersData["arrow#persistent"].padding : ps(), Ae = ce[G], Me = ce[$], be = at(0, k[W], me[W]), Be = L ? k[W] / 2 - te - be - Ae - I.mainAxis : le - be - Ae - I.mainAxis, Te = L ? -k[W] / 2 + te + be + Me + I.mainAxis : ne + be + Me + I.mainAxis, Ge = e.elements.arrow && At(e.elements.arrow), bt = Ge ? h === "y" ? Ge.clientTop || 0 : Ge.clientLeft || 0 : 0, qe = (R = S?.[h]) != null ? R : 0, Et = H + Be - qe - bt, yt = H + Te - qe, Xe = at(g ? Dt(F, Et) : F, H, g ? Pe(Q, yt) : Q);
      _[h] = Xe, w[h] = Xe - H;
    }
    if (l) {
      var Je, vt = h === "x" ? J : Z, Lt = h === "x" ? re : oe, Ee = _[x], Ie = x === "y" ? "height" : "width", Ze = Ee + b[vt], we = Ee - b[Lt], et = [J, Z].indexOf(E) !== -1, Tt = (Je = S?.[x]) != null ? Je : 0, It = et ? Ze : Ee - k[Ie] - C[Ie] - Tt + I.altAxis, _t = et ? Ee + k[Ie] + C[Ie] - Tt - I.altAxis : we, xt = g && et ? Ma(It, Ee, _t) : at(g ? It : Ze, Ee, g ? _t : we);
      _[x] = xt, w[x] = xt - Ee;
    }
    e.modifiersData[s] = w;
  }
        ...
        if (!m) {
          ...
          if (Hn(v, L)) {
            ...
            for (var h = 0; h < o.orderedModifiers.length; h++) {
              ...
              var x = o.orderedModifiers[h], _ = x.fn, k = x.options, C = k === void 0 ? {} : k, D = x.name;
...
function Qi(t, e) {
  ...
  if (!n.render)
    return h;
  var x = n.render(h), _ = x.popper, k = x.onUpdate;
  ...
  function Je(T) {
    return ue.isTouch ? S() !== T.type.indexOf("touch") >= 0 : !1;
  }
  function vt() {
    Lt();
    var T = h.props, O = T.popperOptions, B = T.placement, K = T.offset, U = T.getReferenceClientRect, j = T.moveTransition, se = w() ? rn(_).arrow : null, He = U ? {
  ...
  function _t(T) {
    if (!h.state.isDestroyed) {
      ...
      }) : B.triggerTarget && t.removeAttribute("aria-expanded"), te(), H(), k && k(O, B), h.popperInstance && (vt(), Ie().forEach(function(K) {
  ...
  function Os() {
    ...
    if (!(T || O || B || K) && !R().hasAttribute("disabled") && (F("onShow", [h], !1), h.props.onShow(h) !== !1)) {
      if (h.state.isVisible = !0, w() && (_.style.visibility = "visible"), H(), ce(), h.state.isMounted || (_.style.transition = "none"), w()) {
      ...
      m = function() {
        ...
        if (!(!h.state.isVisible || p)) {
          if (p = !0, _.offsetHeight, _.style.transition = h.props.moveTransition, w() && h.props.animation) {
  ...
  function Ps() {
    ...
    if (!(T || O || B) && (F("onHide", [h], !1), h.props.onHide(h) !== !1)) {
      if (h.state.isVisible = !1, h.state.isShown = !1, p = !1, a = !1, w() && (_.style.visibility = "hidden"), le(), Ae(), H(!0), w()) {
        var U = $(), j = U.box, se = U.content;
        h.props.animation && (qt([j, se], K), Kn([j, se], "hidden"));
      }
      Q(), te(), h.props.animation ? w() && Me(K, h.unmount) : h.unmount();
    }
  }
  function Rs(T) {
...
function Ji(t, e, n, s) {
  const i = n == null || s == null, a = pt(e), l = (D, I) => a <= 0.179 ? xs(D, I) : Ns(D, I);
  ...
  const C = 50;
  for (; h >= _ && k < C; )
    k += 1, d = l(d, h), p = Bt(d, e), o = p.ratio, (i ? v() : E()) && ((Math.abs(o) <= Math.abs(m) || !g) && (g = d, m = o), d = L, h *= x), L = d;
...
function nr(t, e) {
  if (t?.querySelector("#contrast-tools")) {
    const { fontSize: s, fontWeight: r, type: i, isLargeText: a } = e, l = t.querySelector("#contrast"), d = t.querySelector("#contrast-preview"), p = t.querySelector("#fg-input"), o = t.querySelector("#bg-input"), c = t.querySelector("#value"), m = t.querySelector("#good"), g = (b, E) => {
      ...
      const E = p.value, v = o.value, L = p.classList.contains("unknown"), h = o.classList.contains("unknown"), x = A();
      if (d.style.color = L ? "" : E, d.style.backgroundColor = h ? "" : v, d.style.backgroundImage = h ? "" : "none", b.target.classList.remove("unknown"), b.target.parentElement.classList.remove("unknown"), L || h)
        return;
      const _ = ut(
        de(E),
        de(v),
      ...
      if (u.Global.contrastAlgorithm === "APCA") {
        const C = _.ratio;
        c.textContent = yn(C);
        const D = Ut(C).slice(1), I = C >= 45 && D[0] >= 0 && D[0] <= 777;
        let S;
        switch (i) {
          case "svg-error":
          case "svg-warning": {
            m.hidden = !I, S = I, g(k, S);
            break;
          }
          default: {
            const w = D[Math.floor(r / 100) - 1];
            S = x >= w, g(k, S), m.hidden = !S;
            break;
          }
        }
      }
      if (u.Global.contrastAlgorithm === "AA" || u.Global.contrastAlgorithm === "AAA") {
        const C = _.ratio;
        c.textContent = vn(C);
        const D = u.Global.contrastAlgorithm === "AAA", I = 3, S = D ? 7 : 4.5, w = D ? 4.5 : 3, R = C >= I, G = x >= 24 || x >= 18.66 && r >= 700 || a, $ = C >= S, W = C >= w;
        switch (i) {
          case "svg-error":
          case "svg-text":
          case "svg-warning": {
          ...
          default: {
            G ? (g([c, l], W), m.hidden = !W) : (g([c, l], $), m.hidden = !$);
            break;
          }
        }
      }
...
function rr(t, e) {
  ...
  if (n) {
    ...
    const _ = e.ignoreHiddenOverflow ? e.ignoreHiddenOverflow.split(",").flatMap((C) => [...document.querySelectorAll(C)]) : [], k = ft(n, "overflow", "hidden");
    k && !_.includes(k) && k.setAttribute("data-sa11y-overflow", "");
  } else {
...
const ws = () => {
  ...
  const e = M.Annotations.Array;
  ae <= 0 && (ae = e.length);
  const n = M.Annotations.Array[ae - 1].shadowRoot.querySelector("button"), s = ks(n, t);
  window.scrollTo({
    top: s,
    behavior: `${u.Global.scrollBehaviour}`
  }), n.offsetTop !== 0 && (n.focus(), n.click()), ae -= 1, ae === -1 && (ae = M.Annotations.Array.length - 1);
};
function ur(t, e) {
  M.Annotations.Array.length && !u.Panel.skipButton.hasAttribute("disabled") && (t.altKey && (t.code === "KeyS" || t.code === "Period") ? (t.preventDefault(), Cs(e)) : t.altKey && (t.code === "KeyW" || t.code === "Comma") && (t.preventDefault(), pr(e)));
}
let on, ln;
function fr(t) {
  on = (e) => {
...
function gr(t, e) {
  ...
  return M.Found.Images.forEach((s) => {
    ...
    if (!(s.height < 2 && s.width < 2 && (Re(s) || r === ""))) {
      ...
      if (r === null)
        ...
        if (y || x) {
          const I = e.checks.IMAGE_DECORATIVE_CAROUSEL.sources, S = I ? s.closest(I) : "";
          if (S) {
            const w = S.querySelectorAll("img"), R = w.length === 1 ? e.checks.IMAGE_DECORATIVE : e.checks.IMAGE_DECORATIVE_CAROUSEL, G = w.length === 1 ? "IMAGE_DECORATIVE" : "IMAGE_DECORATIVE_CAROUSEL";
            R && t.push({
              test: G,
              element: s,
              type: R.type || "warning",
        ...
        const k = i ? e.checks.LINK_ALT_MAYBE_BAD : e.checks.ALT_MAYBE_BAD, C = new RegExp(`^\\S{${k.minLength || 15},}$`), D = /[^\p{L}\-,.!?]/u.test(r);
        if (m[0] !== null) {
          const I = i ? e.checks.LINK_ALT_FILE_EXT : e.checks.ALT_FILE_EXT, S = i ? "LINK_ALT_FILE_EXT" : "ALT_FILE_EXT";
          I && t.push({
            test: S,
            element: s,
            type: I.type || "error",
            content: f.sprintf(I.content || S, m[0], c),
            dismiss: N(`${S + a + c}`),
            dismissAll: I.dismissAll ? S : !1,
            developer: I.developer || !1
          });
        } else if (m[2] !== null) {
          const I = i ? e.checks.LINK_PLACEHOLDER_ALT : e.checks.ALT_PLACEHOLDER, S = i ? "LINK_PLACEHOLDER_ALT" : "ALT_PLACEHOLDER";
          I && t.push({
            test: S,
            element: s,
            type: I.type || "error",
            content: f.sprintf(I.content || S, c),
            dismiss: N(`${S + a + c}`),
            dismissAll: I.dismissAll ? S : !1,
            developer: I.developer || !1
          });
        } else if (m[1] !== null) {
          const I = i ? e.checks.LINK_SUS_ALT : e.checks.SUS_ALT, S = i ? "LINK_SUS_ALT" : "SUS_ALT";
          I && t.push({
            test: S,
            element: s,
            type: I.type || "warning",
            content: f.sprintf(I.content || S, m[1], c),
            dismiss: N(`${S + a + c}`),
            dismissAll: I.dismissAll ? S : !1,
            developer: I.developer || !1
          });
        } else if (k && C.test(r) && D) {
          const I = i ? "LINK_ALT_MAYBE_BAD" : "ALT_MAYBE_BAD";
          t.push({
            test: I,
            element: s,
            type: k.type || "warning",
            content: f.sprintf(k.content || I, c),
            dismiss: N(`${I + a + c}`),
            dismissAll: k.dismissAll ? I : !1,
            developer: k.developer || !1
          });
        } else if (i ? r.length > L : r.length > h) {
          const I = i ? e.checks.LINK_IMAGE_LONG_ALT : e.checks.IMAGE_ALT_TOO_LONG, S = i ? "LINK_IMAGE_LONG_ALT" : "IMAGE_ALT_TOO_LONG", w = xe(c, 600);
          I && t.push({
            test: S,
            element: s,
            type: I.type || "warning",
            content: f.sprintf(I.content || S, r.length, w),
            dismiss: N(`${S + a + c}`),
            dismissAll: I.dismissAll ? S : !1,
            developer: I.developer || !1
          });
        } else if (i) {
          const I = p === 0 ? e.checks.LINK_IMAGE_ALT : e.checks.LINK_IMAGE_ALT_AND_TEXT, S = p === 0 ? "LINK_IMAGE_ALT" : "LINK_IMAGE_ALT_AND_TEXT";
          if (I) {
            const w = ve(i), R = fe(w), G = Y(R), $ = p === 0 ? f.sprintf("LINK_IMAGE_ALT", c) : `${f.sprintf("LINK_IMAGE_ALT_AND_TEXT", c, G)} ${f.sprintf("ACC_NAME_TIP")}`;
            t.push({
              test: S,
              element: s,
              type: I.type || "warning",
...
function mr(t, e, n) {
  ...
  return M.Found.Headings.forEach((i, a) => {
    ...
    let v = null, L = null, h = null, x = null, _ = null, k = null;
    if (b === 0)
      if (i.querySelectorAll("img").length) {
        ...
        i.querySelector("img") && (!C || C.trim() === "") && e.checks.HEADING_EMPTY_WITH_IMAGE && (v = "HEADING_EMPTY_WITH_IMAGE", L = e.checks.HEADING_EMPTY_WITH_IMAGE.type || "error", h = f.sprintf(
          ...
          y
        ), x = e.checks.HEADING_EMPTY_WITH_IMAGE.developer || !1, _ = e.checks.HEADING_EMPTY_WITH_IMAGE.dismissAll ? "HEADING_EMPTY_WITH_IMAGE" : !1, k = "-15px 30px");
      } else e.checks.HEADING_EMPTY && (v = "HEADING_EMPTY", L = e.checks.HEADING_EMPTY.type || "error", h = f.sprintf(e.checks.HEADING_EMPTY.content || "HEADING_EMPTY", y), x = e.checks.HEADING_EMPTY.developer || !1, _ = e.checks.HEADING_EMPTY.dismissAll ? "HEADING_EMPTY" : !1, k = "0");
    ...
    h && L && t.push({
      ...
      developer: x,
      margin: k
    }), s = y, r = o, M.Found.OutlineIgnore.includes(i) || n.push({
      element: i,
      headingLevel: y,
      text: o,
      type: L,
...
function Ar(t, e) {
  ...
  return M.Found.Links.forEach((a) => {
    ...
    ), k = f._("CLICK").some((G) => new RegExp(`\\b${G}\\b`, "i").test(a.textContent)), I = [
      "pdf",
      "doc",
      "docx",
      "word",
    ...
    if (!a.querySelectorAll("img").length) {
      ...
      (c[1] !== null || k) && e.checks.LINK_CLICK_HERE && t.push({
    ...
    if (e.linksAdvancedPlugin && w.length !== 0) {
      if (i[w] && !i[l]) {
        ...
        i[w] = !0, i[l] = !0;
      a.getAttribute("target")?.toLowerCase() === "_blank" && !S && !_ && e.checks.LINK_NEW_TAB && t.push({
        test: "LINK_NEW_TAB",
        element: a,
      ...
      }), S && !I && e.checks.LINK_FILE_EXT && t.push({
        test: "LINK_FILE_EXT",
        element: a,
...
function br(t, e) {
  ...
  for (let i = 0; i < M.Found.Contrast.length; i++) {
    const a = M.Found.Contrast[i], l = getComputedStyle(a), d = parseFloat(l.opacity), p = de(l.color, d), o = parseFloat(l.fontSize), c = l.fontWeight, m = jn(c), g = Pt(a, u.Global.shadowDetection), A = Fs(a), b = Re(a) || A || d === 0 || o === 0, v = Array.from(a.childNodes).filter((h) => h.nodeType === 3).map((h) => h.textContent).join("").trim(), L = ["SELECT", "INPUT", "TEXTAREA"].includes(a.tagName);
    if (v.length !== 0 || L) {
  ...
  return M.Found.Svg.forEach((i) => {
    ...
    if ((p.length === 1 || c) && o.length === 0) {
      ...
      const h = Math.min(v, L) < 50 ? 1 : 3, x = A && E >= h && A !== "none", _ = g && g !== "none" && !g.startsWith("url("), k = de(g === "currentColor" ? getComputedStyle(p[0]).color : g, b), C = de(A === "currentColor" ? getComputedStyle(p[0]).color : A, b);
      if (![k, C].includes("unsupported") && d) {
        let I, S = !1, w = !1;
        if (_ && (I = ut(
          k,
          l,
          e.contrastAlgorithm
        ), S = e.contrastAlgorithm === "APCA" ? I.ratio >= 45 : I.ratio >= 3), x && (I = ut(
          C,
          l,
          e.contrastAlgorithm
        ), w = e.contrastAlgorithm === "APCA" ? I.ratio >= 45 : I.ratio >= 3), _ && x && !S && !w || _ && !x && !S || !_ && x && !w) {
          const W = q(l), H = q(k), F = q(C);
          if (H === W && !x || F === W && !_)
            return;
          n.push({
            $el: i,
        ...
        _ && k !== "unsupported" ? a.color = k : x && C !== "unsupported" && (a.color = C), d && (a.background = l), n.push(a);
    } else
...
function Tr(t, e) {
  ...
  if (e.checks.QA_FAKE_LIST) {
    ...
    M.Found.Paragraphs.forEach((b, E) => {
      let v = !1, L = !1;
      A = A || V(b).replace("(", "");
      const h = A.substring(0, 2), x = h.match(l), _ = h.match(a), k = h.match(d), C = o.test(h.charAt(0));
      if (h.length > 0 && h !== g && !_ && (x || k || C)) {
        const D = M.Found.Paragraphs[E + 1];
        if (D) {
          if (v = V(D).replace("(", "").substring(0, 2), p.includes(v?.toLowerCase().trim()))
            return;
          const I = m(v);
          x ? h !== "A " && h === I && (L = !0) : k && !y && I.match(d) && (L = !0, y = !0);
        }
        if (!L) {
