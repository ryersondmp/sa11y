...
export default {
  ...
  strings: {
    ...
    LABELS_ARIA_LABEL_INPUT: 'Intrarea are un nume accesibil, deși vă rugăm să vă asigurați că există și o etichetă vizibilă. <hr> <strong {B}>Etichetă</strong> <strong {C}>%(TEXT)</strong>',
    ...
    QA_DUPLICATE_ID: 'S-a găsit <strong>un ID duplicat</strong>. Se știe că erorile de ID duplicat cauzează probleme pentru tehnologiile asistive atunci când acestea încearcă să interacționeze cu conținutul. <hr> Vă rugăm să eliminați sau să modificați următorul ID: <strong {C}>%(id)</strong>',
    ...
    QA_UNCONTAINED_LI: 'Toate elementele de listă <code>&lt;li&gt;</code> trebuie să fie plasate în interiorul elementelor <code>&lt;ul&gt;</code> neordonate sau <code>&lt;ol&gt;</code> ordonate. Această structură ajută cititoarele de ecran să anunțe lista și elementele sale cu precizie.',
    QA_META_SCALABLE: 'Eliminați parametrul <code>user-scalable="no"</code> din elementul <code>&lt;meta name="viewport"&gt;</code> pentru a permite mărirea.',
    QA_META_MAX: 'Asigurați-vă că parametrul <code>maximum-scale</code> din elementul <code>&lt;meta name="viewport"&gt;</code> nu este mai mic de 2.',
    QA_JUSTIFY: 'Evitați utilizarea textului justificat, care se aliniază atât la marginea stângă, cât și la cea dreaptă. Acest lucru poate fi dificil de citit pentru unii oameni din cauza spațiilor inegale între cuvinte. Utilizați text aliniat la stânga pentru o mai bună lizibilitate.',
