...
export default {
  ...
  strings: {
    ...
    QA_DUPLICATE_ID: '<strong>Yinelenen kimlik</strong> bulundu. Yinelenen kimlik hatalarının, içerikle etkileşime girmeye çalışan yardımcı teknolojiler için sorunlara neden olduğu bilinmektedir. <hr> Lütfen aşağıdaki kimliği kaldırın veya değiştirin: <strong {C}>%(id)</strong>',
    ...
    QA_UNCONTAINED_LI: 'Tüm <code>&lt;li&gt;</code> liste öğeleri, <code>&lt;ul&gt;</code> sırasız veya <code>&lt;ol&gt;</code> sıralı öğeler içinde yer almalıdır. Bu yapı, ekran okuyucularının listeyi ve öğelerini doğru bir şekilde duyurmasına yardımcı olur.',
    QA_META_SCALABLE: 'Yakınlaştırmayı etkinleştirmek için <code>&lt;meta name="viewport"&gt;</code> öğesindeki <code>user-scalable="no"</code> parametresini kaldırın.',
    QA_META_MAX: ' <code>&lt;meta name="viewport"&gt;</code> öğesindeki <code>maximum-scale</code> parametresinin 2\'den az olmadığından emin olun.',
