...
export default {
  ...
  strings: {
    ...
    LINK_IMAGE_BAD_ALT_MESSAGE: 'Alt metin içinde dosya uzantısı bulundu. Alt metnin, görüntünün gerçek bir tanımını değil, bağlantının hedefini açıkladığından emin olun. Kaldırın: <strong {r}>%(error)</strong>.<hr><strong>Alt metin:</strong> %(altText)',
    ...
    LINK_ALT_HAS_BAD_WORD_MESSAGE: 'Bulunan alt metin içinde dosya uzantısı. Görsel bir hikaye, ruh hali veya önemli bir bilgi aktarıyorsa, görseli tanımladığınızdan emin olun. Kaldırın: <strong {r}>%(error)</strong>.<hr><strong>Alt metin:</strong> %(altText)',
    ...
    LINK_IMAGE_ARIA_HIDDEN: 'Resmin etrafındaki bağlantı <code>aria-hidden=&quot;true&quot;</code> özelliğine sahiptir ancak yine de klavye ile odaklanabilir. Gereksiz veya yinelenen bir bağlantıyı gizlemek istiyorsanız, <code>tabindex=&quot;-1&quot;</code> da ekleyin.',
    ...
    LINK_IMAGE_ALT_AND_TEXT_WARNING: 'Resim bağlantısı <strong>hem alt metin hem de çevresindeki bağlantı metnini içerir.</strong> Bu resim dekoratifse ve başka bir sayfaya işlevsel bir bağlantı olarak kullanılıyorsa, resmi dekoratif veya boş olarak işaretlemeyi düşünün - çevresindeki bağlantı metni yeterli olmalıdır. <hr> <strong>Alt metin:</strong> %(altText)',
    ...
    LABELS_ARIA_LABEL_INPUT_MESSAGE: 'Girdinin erişilebilir bir adı vardır, ancak lütfen görünür bir etiket olduğundan da emin olun. <hr> Bu girdi için erişilebilir ad şöyledir: <strong>%(sanitizedText)</strong>',
    ...
    EMBED_MISSING_TITLE: 'Gömülü içerik, içeriğini açıklayan erişilebilir bir ad gerektirir. Lütfen <code>iframe</code> öğesinde benzersiz bir <code>title</code> veya <code>aria-label</code> özniteliği sağlayın. <a href="https://dequeuniversity.com/tips/provide-iframe-titles">iFrames.</a> hakkında daha fazla bilgi edinin',
