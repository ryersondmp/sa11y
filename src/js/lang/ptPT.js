...
export default {
  ...
  strings: {
    ...
    LABELS_ARIA_LABEL_INPUT: 'A entrada tem um nome acessível, mas certifique-se de que também existe uma etiqueta visível. <hr> <strong {B}>Rótulo</strong> <strong {C}>%(TEXT)</strong>',
    ...
    QA_DUPLICATE_ID: 'Encontrada <strong>identificação duplicada</strong>. Os erros de ID duplicada são conhecidos por causar problemas às tecnologias de assistência quando estas estão a tentar interagir com o conteúdo. <hr> Remova ou altere o seguinte ID: <strong {C}>%(id)</strong>',
    ...
    QA_UNCONTAINED_LI: 'Todos os itens de lista <code>&lt;li&gt;</code> devem ser colocados dentro dos elementos <code>&lt;ul&gt;</code> não ordenados ou <code>&lt;ol&gt;</code> ordenados. Esta estrutura ajuda os leitores de ecrã a anunciar a lista e os seus itens com precisão.',
    ...
    QA_JUSTIFY: 'Evite utilizar texto justificado, que se alinha tanto às margens esquerda como direita. Isto pode ser difícil de ler para algumas pessoas devido aos espaços desiguais entre palavras. Utilize texto alinhado à esquerda para uma melhor legibilidade.',
