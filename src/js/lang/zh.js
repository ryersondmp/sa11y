...
export default {
  ...
  strings: {
    ...
    HEADING_LONG: '标题很长!标题应被用来组织内容和传达结构。它们应该是简短的、信息丰富的和独特的。请将标题保持在160个字符以内 (不超过一个句子) 。<hr> <strong {wb}>%(HEADING_LENGTH) 字符</strong>。',
    ...
    LINK_ALT_HAS_FILE_EXTENSION: '在alt文本中发现文件扩展名。确保alt文本描述链接的目的地, 而不是图片的字面描述。删除: <strong {r}>%(ERROR)</strong>. <hr> <strong {eb}>ALT文本</strong> <strong {r}>%(ALT_TEXT)</strong>',
    LINK_IMAGE_PLACEHOLDER_ALT_MESSAGE: '发现链接图片中的非描述性或占位符的alt文本。确保alt文本描述了链接的目的地, 而不是图像的字面描述。替换以下alt文本: <strong {r}>%(ALT_TEXT)</strong>',
    LINK_IMAGE_SUS_ALT_MESSAGE: '辅助技术已经表明这是一张图片, 所以&quot;<strong {w}>%(ERROR)</strong>&quot; 可能是多余的。确保alt文本描述了链接的目的地, 而不是图像的字面描述。<hr> <strong {wb}>ALT文本</strong> <strong {w}>%(ALT_TEXT)</strong>',
    ALT_HAS_FILE_EXTENSION: '在alt文本内找到文件扩展名。如果图片传达了一个故事、情绪或重要信息--一定要描述图片。删除: <strong {r}>%(ERROR)</strong> <hr> <strong {eb}>ALT文本</strong> <strong {r}>%(ALT_TEXT)</strong>',
    ALT_PLACEHOLDER_MESSAGE: '发现非描述性或占位符的alt文本。用更有意义的内容替换下面的alt文本: <strong {r}>%(ALT_TEXT)</strong>。',
    ALT_HAS_SUS_WORD: '辅助技术已经表明这是一张图片, 所以&quot;<strong {w}>%(ERROR)</strong>&quot; 可能是多余的。<hr> <strong {wb}>ALT文本</strong> <strong {w}>%(ALT_TEXT)</strong>',
    ...
    LINK_IMAGE_LONG_ALT: '链接图片的alt文本描述<strong>太长</strong>。链接图片的alt文本应该描述链接的位置, 而不是图片的字面描述。<strong>考虑使用它所链接的页面的标题作为alt文本。</strong> <hr> <strong {wb}>ALT文本</strong> <strong {wb}>%(altLength) 字符</strong> <strong {w}>%(ALT_TEXT)</strong>',
    LINK_IMAGE_ALT_WARNING: '图片链接包含alt文本。符号文本是否描述了该链接的位置？可以考虑使用它所链接的页面的标题作为alt文本。 <hr> <strong {wb}>ALT文本</strong> <strong {w}>%(ALT_TEXT)</strong>',
    LINK_IMAGE_ALT_AND_TEXT_WARNING: '图片链接包含<strong>alt文本和周围的链接文本。</strong>如果该图片是装饰性的, 并被用作另一个页面的功能链接, 请考虑将该图片标记为装饰性或无效--周围的链接文本应该足够了。<hr> <strong {wb}>ALT文本</strong> <strong {w}>%(ALT_TEXT)</strong> <hr> <strong>链接标签:</strong> <strong {w}>%(TEXT)</strong>',
    ...
    IMAGE_FIGURE_DUPLICATE_ALT: '不要在 alt 文本和标题文本中使用完全相同的词语。屏幕阅读器会重复宣读信息。<ul><li>alt 文本应提供对图像内容的简洁描述。</li><li>标题通常应提供上下文以将图像与周围内容联系起来，或者关注特定信息。</li></ul>了解更多：<a href="https://thoughtbot.com/blog/alt-vs-figcaption#the-figcaption-element">alt 与 figcaption 的区别。</a> <hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong>',
    IMAGE_DECORATIVE: '图片被标记为<strong>装饰性</strong>, 将被辅助技术所忽略。如果图片传达了一个故事、情绪或重要的信息--请务必添加alt文本。',
    IMAGE_ALT_TOO_LONG: 'Alt文本描述<strong>太长</strong>。Alt文本应该是简洁的, 但又像<em>tweet</em>一样有意义 (大约100个字符) 。如果这是一张复杂的图片或图表, 可以考虑将图片的长篇描述放在下面的文字或手风琴组件中。<hr> <strong {wb}>ALT文本</strong> <strong {wb}>%(altLength) 字符</strong> <strong {w}>%(ALT_TEXT)</strong>',
    IMAGE_PASS: '<strong {b}>ALT文本</strong> %(ALT_TEXT)',
    ...
    LABELS_ARIA_LABEL_INPUT_MESSAGE: '输入有一个无障碍名称, 但请确保也有一个可见的标签。<hr> <strong {wb}>输入标签</strong> <strong {w}>%(TEXT)</strong>',
    ...
    CONTRAST_ERROR: '这个文本与背景的对比度不够。普通文本的对比度至少应该是4.5:1, 大文本的对比度应该是3:1。<hr> <strong {eb}>对比度比率</strong> <strong {eb}>%(RATIO)</strong> <strong {r}>%(TEXT)</strong>',
    CONTRAST_WARNING: '该文本的对比度不明, 需要人工审查。确保文字和背景有强烈的颜色对比。正常文本的对比度应至少为4.5:1, 大型文本为3:1。<hr> 请审查: <strong {w}>%(TEXT)</strong>',
    CONTRAST_INPUT_ERROR: '该输入的文字与背景的对比度不够。普通文本的对比度应该至少为4.5:1, 大文本的对比度应该为3:1。<hr> <strong {eb}>对比度比率</strong> <strong {eb}>%(RATIO)</strong>',
  },
};
