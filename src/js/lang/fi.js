...
export default {
  ...
  strings: {
    ...
    LABELS_ARIA_LABEL_INPUT: 'Syötteellä on helppokäyttöinen nimi, mutta varmista, että myös merkintä on näkyvissä. <hr> <strong {B}>Syötelabeli</strong> <strong {C}>%(TEXT)</strong>',
    ...
    QA_DUPLICATE_ID: 'Löytyi <strong>kaksoistunniste</strong>. Kaksoistunnusvirheiden tiedetään aiheuttavan ongelmia avustaville teknologioille, kun ne yrittävät toimia vuorovaikutuksessa sisällön kanssa. <hr> Poista tai vaihda seuraava ID: <strong {C}>%(id)</strong>',
    ...
    QA_UNCONTAINED_LI: 'Kaikki <code>&lt;li&gt;</code> luetteloelementit on sijoitettava <code>&lt;ul&gt;</code> järjestämättömien tai <code>&lt;ol&gt;</code> järjestettyjen elementtien sisään. Tämä rakenne auttaa ruudunlukuohjelmia ilmoittamaan luettelon ja sen kohteet tarkasti.',
    QA_META_SCALABLE: 'Poista parametri <code>user-scalable="no"</code> elementistä <code>&lt;meta name="viewport"&gt;</code> salliaksesi zoomauksen.',
    QA_META_MAX: 'Varmista, että parametri <code>maximum-scale</code> elementissä <code>&lt;meta name="viewport"&gt;</code> ei ole pienempi kuin 2.',
    QA_JUSTIFY: 'Vältä oikeuttamista, joka kohdistaa tekstin sekä vasemmalle että oikealle marginaalille. Tämä voi olla vaikeaa joillekin ihmisille, koska sanojen välinen etäisyys on epätasaista. Käytä paremman luettavuuden vuoksi vasemmalle kohdistettua tekstiä.',
