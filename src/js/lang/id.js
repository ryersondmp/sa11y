...
export default {
  ...
  strings: {
    ...
    LABELS_ARIA_LABEL_INPUT: 'Input memiliki nama yang dapat diakses, meskipun pastikan ada label yang terlihat juga. <hr> <strong {B}>Label masukan</strong> <strong {C}>%(TEXT)</strong>',
    ...
    QA_DUPLICATE_ID: 'Ditemukan <strong>duplikat ID</strong>. Kesalahan ID duplikat diketahui dapat menyebabkan masalah pada teknologi bantuan saat mencoba berinteraksi dengan konten. <hr> Harap hapus atau ubah ID berikut ini: <strong {C}>%(id)</strong>',
    ...
    QA_UNCONTAINED_LI: 'Semua item daftar <code>&lt;li&gt;</code> harus diletakkan di dalam elemen <code>&lt;ul&gt;</code> yang tidak terurut atau <code>&lt;ol&gt;</code> yang terurut. Struktur ini membantu pembaca layar mengumumkan daftar dan item-itemnya dengan akurat.',
    QA_META_SCALABLE: 'Hapus parameter <code>user-scalable="no"</code> pada elemen <code>&lt;meta name="viewport"&gt;</code> untuk mengizinkan zoom.',
    QA_META_MAX: 'Pastikan parameter <code>maximum-scale</code> pada elemen <code>&lt;meta name="viewport"&gt;</code> tidak kurang dari 2.',
    QA_JUSTIFY: 'Hindari penggunaan teks yang diratakan, yang sejajar dengan margin kiri dan kanan. Ini dapat sulit dibaca oleh beberapa orang karena jarak yang tidak merata antara kata-kata. Gunakan teks yang rata kiri untuk keterbacaan yang lebih baik.',
