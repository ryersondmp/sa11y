...
export default {
  ...
  strings: {
    ...
    HEADING_LONG: 'Smerovanie je dlhé! Nadpisy by sa mali používať na usporiadanie obsahu a vyjadrenie štruktúry. Mali by byť stručné, informatívne a jedinečné. Prosím, aby nadpisy mali menej ako 160 znakov (nie viac ako jednu vetu). <hr> <strong {wb}>%(HEADING_LENGTH) Znakov</strong>',
    ...
    LINK_LABEL: '<strong {wb}>Popis</strong> %(TEXT)',
    ...
    LINK_ALT_HAS_FILE_EXTENSION: 'Nájdená prípona súboru v texte alt. Uistite sa, že text alt opisuje cieľ odkazu, nie doslovný opis obrázka. Odstráňte: <strong {r}>%(ERROR)</strong> <hr> <strong {eb}>ALT</strong> <strong {r}>%(ALT_TEXT)</strong>',
    LINK_IMAGE_PLACEHOLDER_ALT_MESSAGE: 'Nájdený text alt bez popisu alebo zástupný text v rámci prepojeného obrázka. Uistite sa, že text alt opisuje cieľ odkazu, nie doslovný opis obrázka. Nahraďte nasledujúci text alt: <strong {r}>%(ALT_TEXT)</strong>',
    LINK_IMAGE_SUS_ALT_MESSAGE: 'Asistenčné technológie už indikujú, že ide o obrázok, takže &quot;<strong {w}>%(ERROR)</strong>&quot; môže byť zbytočné. Uistite sa, že alt text opisuje cieľ odkazu, nie doslovný opis obrázka. <hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong>',
    ALT_HAS_FILE_EXTENSION: 'Nájdená prípona súboru v texte alt. Ak obrázok vyjadruje príbeh, náladu alebo dôležitú informáciu, nezabudnite ho opísať. Odstráňte: <strong {r}>%(ERROR)</strong> <hr> <strong {eb}>ALT</strong> <strong {r}>%(ALT_TEXT)</strong>',
    ALT_PLACEHOLDER_MESSAGE: 'Nájdený text alt bez popisu alebo zástupného textu. Nahraďte nasledujúci alt text niečím zmysluplnejším: <strong {r}>%(ALT_TEXT)</strong>',
    ALT_HAS_SUS_WORD: 'Asistenčné technológie už indikujú, že ide o obrázok, takže &quot;<strong {w}>%(ERROR)</strong>&quot; môže byť zbytočné. <hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong>',
    ...
    LINK_IMAGE_LONG_ALT: 'Popis Alt textu na prepojenom obrázku je <strong>predlžený</strong>. Text alt na prepojených obrázkoch by mal popisovať, kam vás odkaz zavedie, nie doslovný opis obrázka. <strong>Zvážte použitie názvu stránky, na ktorú odkazuje, ako alt textu.</strong> <hr> <strong {wb}>ALT</strong> <strong {wb}>%(altLength) Znakov</strong> <strong {w}>%(ALT_TEXT)</strong>',
    LINK_IMAGE_ALT_WARNING: 'Odkaz na obrázok obsahuje text alt. Opisuje text alt, kam vás odkaz zavedie? <strong>Zvážte použitie názvu stránky, na ktorú odkazuje, ako alt textu.</strong> <hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong>',
    LINK_IMAGE_ALT_AND_TEXT_WARNING: 'Odkaz na obrázok obsahuje <strong>obidva texty alt a okolitý text odkazu.</strong> Ak je tento obrázok dekoratívny a používa sa ako funkčný odkaz na inú stránku, zvážte označenie obrázka ako dekoratívneho alebo nulového - okolitý text odkazu by mal stačiť. <hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong> <hr> <strong {wb}>Označenie odkazu</strong> <strong {w}>%(TEXT)</strong>',
    ...
    IMAGE_FIGURE_DUPLICATE_ALT: 'Nepoužívajte presne tie isté slová pre text alt aj nadpis. Čítačky obrazovky budú informáciu oznamovať dvakrát. <ul><li>Textu alt by mal poskytovať stručný opis toho, čo je na obrázku.</li><li>Titul by mal zvyčajne poskytovať kontext, aby súvisel s okolitým obsahom, alebo upozorniť na konkrétnu informáciu.</li></ul> Ďalšie informácie: <a href="https://thoughtbot.com/blog/alt-vs-figcaption#the-figcaption-element">alt verzus titulok obrázku.</a> <hr> <strong {wb}>ALT</strong> <strong {w}>%(ALT_TEXT)</strong>',
    ...
    IMAGE_ALT_TOO_LONG: 'Alt text popisu je <strong>príliš dlhý</strong>. Alt text by mal byť stručný, ale výstižný ako <em>tweet</em> (približne 100 znakov). Ak ide o zložitý obrázok alebo graf, zvážte umiestnenie dlhého popisu obrázka do textu pod ním alebo do akordeónového komponentu. <hr> <strong {wb}>ALT</strong> <strong {wb}>%(altLength) Znakov</strong> <strong {w}>%(ALT_TEXT)</strong>',
    ...
    LABELS_ARIA_LABEL_INPUT_MESSAGE: 'Vstup má prístupný názov, ale dbajte na to, aby bol viditeľný aj štítok. <hr> <strong {wb}>Štítok vstupu</strong> <strong {w}>%(TEXT)</strong>',
    ...
    CONTRAST_ERROR: 'Tento text nie je dostatočne kontrastný s pozadím. Kontrastný pomer by mal byť aspoň 4,5:1 pre normálny text a 3:1 pre veľký text. <hr> <strong {eb}>Kontrastný pomer</strong> <strong {eb}>%(RATIO)</strong> <strong {r}>%(TEXT)</strong>',
    ...
    CONTRAST_INPUT_ERROR: 'Text v tomto vstupe nie je dostatočne kontrastný s pozadím. Kontrastný pomer by mal byť aspoň 4,5:1 pre normálny text a 3:1 pre veľký text. <hr> <strong {eb}>Kontrastný pomer</strong> <strong {eb}>%(RATIO)</strong>',
  },
};
