...
export default {
  ...
  strings: {
    ...
    LINK_IDENTICAL_NAME: 'Saitei ir identisks teksts kā citai saitei, lai gan tā norāda uz citu lapu. Vairākas saites ar vienādu tekstu var radīt neskaidrības cilvēkiem, kuri izmanto ekrānlasītājus.<hr>Padomājiet, vai šādu saiti nevajadzētu padarīt aprakstošāku, lai palīdzētu to atšķirt no citām saitēm: <strong {C}>%(TEXT)</strong>',
    ...
    LABELS_ARIA_LABEL_INPUT: 'Ievadei ir pieejams nosaukums, tomēr pārliecinieties, ka ir redzams arī marķējums. <hr> <strong {B}>Ievades etiķete</strong> <strong {C}>%(TEXT)</strong>',
    ...
    QA_DUPLICATE_ID: 'Atrasts <strong>duplicate ID</strong>. Ir zināms, ka dubultā ID kļūdas rada problēmas palīgtehnoloģijām, kad tās mēģina mijiedarboties ar saturu. <hr> Lūdzu, noņemiet vai mainiet šādu ID: <strong {C}>%(id)</strong>',
    ...
    QA_UNCONTAINED_LI: 'Visiem <code>&lt;li&gt;</code> saraksta elementiem jābūt ievietotiem <code>&lt;ul&gt;</code> nesakārtotas vai <code>&lt;ol&gt;</code> sakārtotas elementus. Šī struktūra palīdz ekrāna lasītājiem precīzi paziņot sarakstu un tā elementus.',
    QA_META_SCALABLE: 'Noņemiet parametru <code>user-scalable="no"</code> <code>&lt;meta name="viewport"&gt;</code> elementā, lai atļautu pietuvināšanu.',
    QA_META_MAX: 'Pārliecinieties, ka <code>maximum-scale</code> parametrs <code>&lt;meta name="viewport"&gt;</code> elementā nav mazāks par 2.',
    QA_JUSTIFY: 'Izvairieties no izmantotā teksta, kas ir izkārtots gan pa kreisi, gan pa labi. Tas var būt grūti lasāms dažiem cilvēkiem, jo ​​vārdu atstarpes ir nevienmērīgas. Lai uzlabotu lasāmību, izmantojiet tekstu, kas izlīdzināts pa kreisi.',
