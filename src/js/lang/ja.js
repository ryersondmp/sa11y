export default {
  ...
  strings: {
    ...
    HEADING_NON_CONSECUTIVE_LEVEL: '連続しない見出しレベルが使用されています。見出しはレベルを飛ばしたり、<strong>見出し %(prevLevel)</strong> から <strong {r}>Heading %(level)</strong> になってはなりません。',
    HEADING_EMPTY: '空の見出しが見つかりました！修正するには、この行を削除するか、<strong {r}>見出し %(level)</strong> から <strong>Normal</strong> または <strong>Paragraph</strong> にフォーマットを変更してください。',
    HEADING_LONG: '見出しが長い！見出しは、内容を整理し、構造を伝えるために使用されるべきです。また、簡潔で情報量が多く、ユニークであるべきです。見出しは160文字以内（1文以内）に収めてください。<hr> 文字数です：<strong {r}>%(見出しの長さ)</strong>。',
    ...
    HEADING_EMPTY_WITH_IMAGE: '見出しにはテキストがなく、画像が含まれています。これが見出しでない場合は、そのフォーマットを<strong {r}>見出し %(レベル)</strong>から<strong>通常</strong>または<strong>パラグラフ</strong>に変更してください。それ以外の場合、画像に装飾がない場合は、altテキストを追加してください。',
    ...
    LINK_ALT_HAS_BAD_WORD_MESSAGE: '見つかったaltテキスト内のファイル拡張子。画像がストーリーや雰囲気、重要な情報を伝えている場合は、必ず画像を説明してください。削除してください：<strong {r}>%(error)</strong>.<hr><strong>Alt text:</strong> %(altText)',
    ...
    QA_BAD_LINK: '不正なリンクが見つかりました。リンクは開発環境を指しているようです。<hr> このリンクは、次の場所を指しています：<br> <strong {r}>%(エル)</strong>。',
