...
export default {
  ...
  strings: {
    ...
    LINK_IDENTICAL_NAME: 'Lingi tekst on identne teise lingiga, kuigi see viitab teisele lehele. Mitu sama tekstiga linki võib tekitada segadust inimestele, kes kasutavad ekraanilugejaid. <hr> Võtke arvesse, et järgmine link oleks kirjeldavam, et aidata seda teistest linkidest eristada: <strong {C}>%(TEXT)</strong>',
    ...
    LABELS_ARIA_LABEL_INPUT: 'Sisendil on juurdepääsetav nimi, kuigi palun veenduge, et ka silt on nähtav. <hr> <strong {B}>Sisendmärgis</strong> <strong {C}>%(TEXT)</strong>',
    ...
    QA_DUPLICATE_ID: 'Leiti <strong>korduv ID</strong>. Teadaolevalt põhjustavad dubleeritud ID-vead probleeme tugitehnoloogiatele, kui need üritavad sisuga suhelda. <hr> Palun eemaldage või muutke järgmine ID: <strong {C}>%(id)</strong>',
    ...
    QA_UNCONTAINED_LI: 'Kõik <code>&lt;li&gt;</code> nimekirjaelemendid peavad olema paigutatud <code>&lt;ul&gt;</code> järjestamata või <code>&lt;ol&gt;</code> järjestatud elementide sisse. See struktuur aitab ekraani lugemistarkvaral nimekirja ja selle elemente täpselt kuulutada.',
    QA_META_SCALABLE: 'Eemaldage parameeter <code>user-scalable="no"</code> elemendis <code>&lt;meta name="viewport"&gt;</code>, et võimaldada suumimist.',
    QA_META_MAX: 'Veenduge, et <code>maximum-scale</code> parameeter elemendis <code>&lt;meta name="viewport"&gt;</code> pole väiksem kui 2.',
    QA_JUSTIFY: 'Vältige õigustatud teksti kasutamist, mis joondub nii vasaku kui parema servaga. See võib mõne inimese jaoks olla raske lugeda, kuna sõnade vahemaa on ebaühtlane. Kasutage parema loetavuse saavutamiseks vasakule joondatud teksti.',
